...
impl Dense {
    ...
    pub(super) fn score(&self, order: &[usize]) -> u64 {
        let rows = order.iter().copied().max().map_or(0, |v| v + 1);
        let mut a = self.a[..rows * self.w].to_vec();
        let mut deg = self.deg[..rows].to_vec();
        ...
        let mut cost = 0;
        for &v in order {
            cost += ((deg[v] + 1) as u64).pow(2);
            active[v] = false;
            words.clear();
            words.extend(
                a[v * self.w..(v + 1) * self.w]
                    .iter()
                    .copied()
                    .enumerate()
                    .filter(|&(_, b)| b != 0),
            ...
            for &(j, mut bits) in &words {
                ...
                while bits != 0 {
                    ...
                    bits &= bits - 1;
                    if u < rows && active[u] {
                        let added = fill_row(&mut a[u * self.w..(u + 1) * self.w], &words, u, v);
                        deg[u] = deg[u] + added as usize - 2;
                    }
                }
            }
        }
...
impl<'a> ProfileState<'a> {
    fn new(atoms: &'a Atoms) -> Self {
        Self {
            ...
            words: Vec::with_capacity(atoms.w),
        }
    }
    ...
    pub(super) fn profile(&self, order: &[usize], v: usize) -> Vec<u64> {
        ...
        for &u in &rest {
            ...
            if b.has(u, v) {
                let mut du = 0u64;
                for j in 0..b.w {
                    du += self.weight(b.a[u * b.w + j] | b.a[v * b.w + j], j);
                }
                du -= b.wt[u] + b.wt[v];
