...
pub(super) fn refine(pattern: &Pattern, p: &[usize]) -> Option<Vec<usize>> {
    ...
    refine_impl(pattern, p, pattern.n, 2_000_000, 32)
}
pub(super) fn boundary(pattern: &Pattern, p: &[usize], free: usize) -> Option<Vec<usize>> {
    boundary_with_budget(pattern, p, free, 500_000, 128)
}
/// Boundary-fixed lifts, visited by descending column count.
pub(super) fn boundary_with_budget(
    ...
    refine_impl(pattern, p, free, work, depth)
}
fn refine_impl(
    pattern: &Pattern,
    p: &[usize],
    ...
    {
        let mut scan: Vec<_> = (0..n).collect();
