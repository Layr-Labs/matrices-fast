...
impl<'a> Tree<'a> {
    ...
    pub(super) fn build_limited(
        ...
        Self::from_prepared(pattern, (p, counts, parent), &mut build_work)
    }

    pub(super) fn from_prepared(
        pattern: &'a Pattern,
        prepared: (Vec<usize>, Vec<u32>, Vec<i32>),
        build_work: &mut usize,
    ) -> Option<Tree<'a>> {
        let n = pattern.n;
        let (p, counts, parent) = prepared;
        charge(build_work, 8 * n + pattern.nnz())?;
        ...
        for v in 0..n {
            ...
            charge(build_work, entries + b.len() * (b.len().max(1).ilog2() as usize + 1))?;
            b.sort_unstable();
...
fn propose(
    ...
    if cfg.natives >= 1 {
        candidates.push(super::quotient::order(
            &graph,
            10.0,
            true,
            true,
    ...
    if let Some(atoms) = &atoms {
        if atoms.k <= cfg.max_atoms && atoms.n <= 128 && atoms.k * 5 < k * 4 {
...
pub(super) fn recovered(
    ...
    propose_regions(&tree, cfg, &mut 64_000_000, &mut proposals);
    assemble(&tree, &proposals)
}

pub(super) fn propose_regions(
    tree: &Tree,
    cfg: &RecoveryCfg,
    work: &mut usize,
    ...
    let n = tree.p.len();
    if charge(work, n * (n.max(1).ilog2() as usize + 8)).is_none() {
        return;
    }
    ...
    for &cap in &cfg.caps {
        for (root_number, &root) in roots.iter().enumerate() {
            ...
            for &mode in &cfg.skeleton_modes {
                ...
                let membership_work = 8 * cap + if mode == 11 { 100_000 } else { 0 };
                if charge(work, membership_work).is_none() {
                    continue;
                }
    ...
    let mut full: Vec<usize> = {
        ...
        let allowance = cfg.patch_work.min((*work / 4) / tasks.len().max(1));
        let mut cheap = Vec::with_capacity(tasks.len());
        let mut map = Vec::new();
        for (root, vertices) in &tasks {
            let total = vertices.len() + tree.bag[*root].len();
            let reserve = allowance + 3 * total * total.div_ceil(64);
            cheap.push(if charge(work, reserve).is_some() {
                propose_cheap(tree, *root, vertices, cfg, allowance, &mut map)
            } else {
                None
            });
        }
    ...
    // Reserve bounded solver work and structural setup/scoring before dispatch.
    full.retain(|&index| {
        let Some((_, graph, total)) = &prepared[index] else {
            return false;
        };
        let entries: usize = graph.iter().map(|word| word.count_ones() as usize).sum();
        let k = tasks[index].1.len();
        let w = total.div_ceil(64);
        let replay_bound = (total * (w + 1) + k * (2 * total - k + 1) / 2 * (w + 8))
            .min(4_000_000);
        let reserve = 8 * graph.len() + 32 * (total + entries)
            + cfg.natives * super::quotient::WORK
            + if cfg.atoms { 4_000_000 } else { 0 }
            + if k <= 512 {
                cfg.modes.len() * (cfg.greedy_work + replay_bound) + cfg.rotation_work
            } else { 0 };
        charge(work, reserve).is_some()
    });
    ...
    for pr in results.into_iter().flatten() {
        if pr.cost < pr.old {
...
pub(super) fn refine(pattern: &Pattern, incumbent: &[usize]) -> Option<Vec<usize>> {
    ...
    for &cap in &[512usize, 1024, 256] {
        for &root in &roots {
            ...
            if vertices.len() + tree.bag[root].len() > 4096 {
                continue;
            }
