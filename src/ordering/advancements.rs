...
pub(super) fn finish(pattern: &Pattern, scoring: &ScoringPattern, best: &mut Candidate) {
    ...
    let refine_core = core.cost.saturating_mul(100) < best.cost.saturating_mul(95);
...
pub(super) fn extra_cores(
    ...
    for threshold in [2, 3, 4, 5, 6, 12] {
        ...
        let scores = map_indexed(&orders, |_, order| {
...
fn core_candidates(
    ...
    map_indexed(&[0, 1], |_, &source| {
        if source == 0 {
            quotient::order(graph, 10.0, true, true, quotient::WORK)
        } else {
...
pub(super) fn complete(
    ...
    let n = pattern.n;
    if n > 100_000 || pattern.nnz() > 1_500_000 {
        return;
    }
    ...
    for round in 0..8 {
        ...
        if let Some(candidates) = completion::peo_candidates(
            ...
            accept_orders(sp, candidates, best, cost);
        }
        if *cost == before {
            break;
        }
    }
    if n > 30_000 || pattern.nnz() > 500_000 {
        return;
    }
    let counts = column_counts(sp, best);
    let limits = completion::CompletionLimits {
