...
/// under a deterministic work budget.
pub(super) fn minimalized_peo_candidates(
    ...
    let mut sorted = completion.sorted;
    ...
    let deleted = minimalize(&mut sorted, &completion.fill, &scan_order, budget);
...
fn reconstruct(
    ...
    Some(Completion { adj, sorted, fill })
}
...
fn eight_variants(
    ...
    let bases = [basis_a, basis_b];
    for basis in &bases[..if basis_a == basis_b { 1 } else { 2 }] {
