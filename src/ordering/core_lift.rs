...
fn reduce_impl<const CHECK_PAIRS: bool>(
    ...
    while let Some(Reverse((d, v))) = heap.pop() {
        ...
        for &w in &nbrs[vu] {
            let wu = w as usize;
            // `edges` only removes keys incident to the pivot being eliminated,
            // and marks that pivot dead immediately afterwards. Therefore no
            // live-live edge is ever deleted: after filtering dead stale
            // neighbours, membership in `nbrs` is enough.
            if wu != vu && alive[wu] {
                live.push(w);
            }
        }
    ...
    for &v in core_ids.iter() {
        ...
        for &w in &nbrs[v] {
            let wu = w as usize;
            // Same live-live invariant as above; sort/dedup preserves the
            // existing duplicate tolerance while avoiding one hash lookup per
            // residual adjacency entry.
            if wu != v && alive[wu] {
                row.push(pos_of[wu]);
            }
        }
...
mod tests {
    ...
    fn assert_same_lift(actual: Option<CoreLift>, expected: Option<CoreLift>, label: &str) {
        match (actual, expected) {
            (None, None) => {}
            (Some(a), Some(e)) => {
                assert_eq!(a.prefix, e.prefix, "{label}: prefix");
                assert_eq!(a.prefix_flops, e.prefix_flops, "{label}: prefix flops");
                assert_eq!(a.core_ids, e.core_ids, "{label}: core ids");
                assert_eq!(a.core_col_ptr, e.core_col_ptr, "{label}: core col ptr");
                assert_eq!(a.core_row_idx, e.core_row_idx, "{label}: core row idx");
                assert_symmetric_core(&a);
            }
            (actual, expected) => panic!(
                "{label}: presence differs actual={} expected={}",
                actual.is_some(),
                expected.is_some()
            ),
        }
    ...
    fn reduce_reference(
        ...
        reduce_reference_impl::<false>(sp, max_row_deg, max_core_n, max_core_edges, 0)
    }

    fn reduce_reference_checked(
        sp: &ScoringPattern,
        max_row_deg: usize,
        max_core_n: usize,
        max_core_edges: usize,
    ...
    fn reduce_reference_impl<const CHECK_PAIRS: bool>(
        sp: &ScoringPattern,
        max_row_deg: usize,
        max_core_n: usize,
        max_core_edges: usize,
    ...
    fn compare_to_reference(sp: &ScoringPattern, label: &str) {
        let max_core_edges = sp.n.saturating_mul(sp.n.saturating_sub(1)) / 2;
        for depth in 0..=sp.n.min(6) {
            let case = format!("{label} depth={depth} unchecked");
            assert_same_lift(
                reduce(sp, depth, sp.n, max_core_edges),
                reduce_reference(sp, depth, sp.n, max_core_edges),
                &case,
            );
            for budget in [0, 1, 2, 3, 6, 16, u64::MAX] {
                let case = format!("{label} depth={depth} budget={budget}");
                assert_same_lift(
                    reduce_checked(sp, depth, sp.n, max_core_edges, budget),
                    reduce_reference_checked(sp, depth, sp.n, max_core_edges, budget),
                    &case,
                );
            }
        }
    ...
    fn graph_from_mask(n: usize, mask: u64) -> ScoringPattern {
        let pairs = all_edges(n);
        let edges: Vec<_> = pairs
            .into_iter()
            .enumerate()
            .filter_map(|(i, e)| ((mask & (1u64 << i)) != 0).then_some(e))
            .collect();
        pattern(n, &edges)
    }

    fn collocation_like_repeated_blocks() -> ScoringPattern {
    ...
    #[test]
    fn live_live_membership_fast_path_matches_reference_exhaustive() {
        for n in 1..=5 {
            let edge_count = n * (n - 1) / 2;
            for mask in 0..(1u64 << edge_count) {
                let sp = graph_from_mask(n, mask);
                compare_to_reference(&sp, &format!("n={n} mask={mask:#x}"));
            }
        }
    }

    #[test]
    fn live_live_membership_fast_path_matches_reference_synthetic() {
        let mut fill_path = all_edges(9)
            .into_iter()
            .filter(|&(a, b)| {
                !((a == 0 && b >= 4)
                    || (a == 8 && b <= 4)
                    || matches!((a, b), (1, 2) | (1, 3) | (2, 3) | (5, 6) | (5, 7) | (6, 7)))
            })
            .collect::<Vec<_>>();
        fill_path.extend([(0, 1), (0, 2), (0, 3), (5, 8), (6, 8), (7, 8)]);
        let cases = [
            pattern(9, &fill_path),
            collocation_like_repeated_blocks(),
            pattern(
                12,
                &[
                    (0, 1),
        ...
        for (i, sp) in cases.iter().enumerate() {
            compare_to_reference(sp, &format!("synthetic={i}"));
        }
    }
