...
fn scored_source(
    ...
    incumbent: &[usize],
    cost: u64,
    produce: impl FnOnce() -> OrderResult,
    ...
    if !is_bijection(&order, graph.n) { return None; }
    let cost = if order == incumbent { cost } else { flops_of(graph, &order) };
    Some((order, cost))
}

fn accept_sources(sources: Vec<Option<(Vec<usize>, u64)>>, best: &mut Vec<usize>, cost: &mut u64) {
    for (order, value) in sources.into_iter().flatten() {
        if value < *cost { *best = order; *cost = value; }
    }
}
...
fn relabelled_order(graph: &ScoringPattern, seed: u64, metis: bool) -> OrderResult {
    ...
    } else { feral_amd::amd_order(&core)? };
    Ok(order.into_iter().map(|v| q[v as usize] as i32).collect())
}

fn native_order(core: &feral_ordering_core::CscPattern<'_>, graph: &ScoringPattern, source: usize) -> OrderResult {
    match source {
        0 => feral_amd::amd_order(core),
        1..=3 => {
            let (aggressive, dense_alpha) = match source {
                1 => (true, 2.0), 2 => (false, -1.0), _ => (false, 10.0),
            };
            feral_amd::amd_order_opts(core, &feral_amd::AmdOptions { aggressive, dense_alpha }).map(|(p, ..)| p)
        }
        4 | 5 => feral_amf::amf_order_opts(core, &feral_amf::AmfOptions {
            dense_alpha: if source == 4 { 5.0 } else { -1.0 }, ..Default::default()
        }).map(|(p, ..)| p),
        6..=8 => {
            let options = match source {
                7 => feral_metis::MetisOptions { niparts: 4, fm_passes: 20, nd_to_amd_switch: 100, max_imbalance: 0.10, ..Default::default() },
                8 => feral_metis::MetisOptions { niparts: 16, fm_passes: 10, nd_to_amd_switch: 400, max_imbalance: 0.30, ..Default::default() },
                _ => feral_metis::MetisOptions::default(),
            };
            feral_metis::metis_order_full(core, &options).map(|(p, ..)| p)
        }
...
pub(super) fn refine(pattern: &Pattern, scoring_pat: &ScoringPattern, best: &mut Candidate) -> Option<ladder::Core> {
    let n = pattern.n;
    let mut core3 = None;
    if n >= 20 {
        ...
        for &goal in thresholds {
            ...
            let exported = ladder.export();
            if goal == 3 { core3 = Some(exported.clone()); }
            ...
            let mut core_best_perm: Vec<usize> = best
                ...
                .collect();
            debug_assert!(is_bijection(&core_best_perm, k));
            ...
            if entries == k + m / 2 {
                ...
                if exported.prefix_flops + core_best_flops < best.cost {
                    ...
                    best.consider(scoring_pat, Some(full));
                }
                continue;
            }
            ...
            let sources: Vec<_> = (0..10).filter(|&source| {
                let eligible = match source {
                    4 | 5 => k < AMF_MAX_N && m < AMF_MAX_NNZ && (source == 4 || !large_core_n),
                    6..=9 => k < CORE_METIS_MAX_N && m < CORE_METIS_MAX_NNZ
                        && (!large_core_n || source == 6),
                    _ => true,
                };
                eligible && core_pass_ledger.try_charge(core_pass_cost)
            }).collect();
            let results = parallel::map_indexed(&sources, 4, |_, &source| {
                scored_source(&core_sp, &core_best_perm, core_best_flops, || {
                    native_order(&core_core, &core_sp, source)
                })
            });
            let amd_flops = sources.iter().zip(&results).find_map(|(&source, result)| {
                (source == 0).then(|| result.as_ref().map(|(_, cost)| *cost)).flatten()
            });
            accept_sources(results, &mut core_best_perm, &mut core_best_flops);
            ...
            let mut core_restarts = if large_core_n { 0 } else {
            ...
            // change the schedule.
            if core_restarts > 1 && amd_flops.is_some_and(|f| f as f64 > 1.1 * core_best_flops as f64) {
                core_restarts = 1;
            }

            let restarts: Vec<_> = (0..core_restarts)
                .take_while(|_| core_pass_ledger.try_charge(core_pass_cost)).collect();
            let results = parallel::map_indexed(&restarts, 4, |_, &r| {
                scored_source(&core_sp, &core_best_perm, core_best_flops, || {
                    relabelled_order(&core_sp, r as u64 + 1, false)
                })
            });
            accept_sources(results, &mut core_best_perm, &mut core_best_flops);
            ...
            if k <= 3500 && m < 300_000 && (k <= 1800 || split_competitive) {
                ...
                for &mode in modes {
                    ...
                    let perm =
                        ladder::dense_greedy(k, &exported.col_ptr, &exported.row_idx, mode, 7);
            ...
            if exported.prefix_flops + core_best_flops < best.cost {
                ...
                if is_bijection(&spliced, n) {
                    let f = flops_of(scoring_pat, &spliced);
                    if f < best.cost {
                        best.cost = f;
                        best.perm = spliced;
                    }
