...
pub(super) fn minfill_order(pattern: &Pattern) -> Vec<i32> {
    ...
    if g.init_def(&mut work).is_none() {
        return Vec::new();
    }
    ...
    for _ in 0..n {
        ...
        order.push(v as i32);
        if g.defelim(v, &mut work).is_none() {
            return Vec::new();
        }
    }
    order
...
impl Graph {
    pub(super) fn new(pattern: &Pattern) -> Self {
        let n = pattern.n;
        let adj: Vec<_> = (0..n)
            .map(|v| pattern.row_idx[pattern.col_ptr[v]..pattern.col_ptr[v + 1]].to_vec())
            .collect();
...
pub(super) fn nd_order(graph: &Graph) -> Vec<i32> {
    nested_dissection(graph, false)
}
pub(super) fn ndfm_order(graph: &Graph) -> Vec<i32> {
    nested_dissection(graph, true)
}

fn nested_dissection(graph: &Graph, fm: bool) -> Vec<i32> {
    let n = graph.adj.len();
...
pub(super) fn rcm_order(graph: &Graph) -> Vec<i32> {
    let n = graph.adj.len();
