...
pub(crate) fn run(sp: &ScoringPattern, ledger: u64) -> Option<(u64, Vec<usize>)> {
    ...
    // iter1083a STRUCTURAL: widen colour deepen to mid-hub band
    // (15k..26k)&nnz<90k — admits faclay30/gasprod/gabriel09/nd_netgen/emfl
    // while lee4_09/10 stay out (nnz≥100k). Not an idle-cut twin.
    if (15_000..26_000).contains(&n) && nnz < 90_000 {
    ...
    // iter1083a: metis_k=3 on mid-hub structural band
    let metis_k = if nnz >= 300_000 { 2 }
        else if (15_000..26_000).contains(&n) && nnz >= 40_000 && nnz < 90_000 { 3 }
    ...
    for (i, (lc, amd_total, _)) in cores.iter().enumerate() {
        ...
        if metric_ok[i] && cn <= METRIC_CORE_MAX_N && cnnz <= METRIC_CORE_MAX_NNZ {
            ...
            // iter1083a: mid-hub structural metric portfolio
            let mid_hub = (15_000..26_000).contains(&n) && nnz < 90_000;
            let vs: &[V] = if mid_hub {
