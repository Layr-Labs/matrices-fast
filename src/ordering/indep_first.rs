...
pub(crate) fn run(sp: &ScoringPattern, ledger: u64) -> Option<(u64, Vec<usize>)> {
    ...
    // iter1061a: edgecross-ONLY colour deepen (15k..18k). Zero lee4_06 contact.
    if (15_000..18_000).contains(&n) && nnz < 80_000 {
    ...
    // died in that band.
    // iter1061a: metis_k=3 edgecross-only
    let metis_k = if nnz >= 300_000 { 2 }
        else if (15_000..18_000).contains(&n) && nnz >= 40_000 && nnz < 80_000 { 3 }
    ...
    for (i, (lc, amd_total, _)) in cores.iter().enumerate() {
        ...
        if metric_ok[i] && cn <= METRIC_CORE_MAX_N && cnnz <= METRIC_CORE_MAX_NNZ {
            ...
            let edgecross = (15_000..18_000).contains(&n) && nnz < 80_000;
            let vs: &[V] = if edgecross {
