...
pub(crate) fn run(sp: &ScoringPattern, ledger: u64) -> Option<(u64, Vec<usize>)> {
    ...
    for (i, (lc, amd_total, _)) in cores.iter().enumerate() {
        ...
        // 0157: KaHIP on the single best-AMD core when that core is tiny
        // (census: waternd2 0.9983). ONE call, top-1 core only — the local
        // slay09h cap death was ~13 calls at ~0.12 s each.
