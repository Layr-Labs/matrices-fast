...
/// Cheap summaries only narrow the equality checks: equal size and pair work
/// do not imply equal selected vertices or an equal residual core.
fn set_was_admitted(
    admitted: &[Vec<bool>],
    summaries: &[(usize, u64)],
    candidate: &[bool],
...
pub(crate) fn run(sp: &ScoringPattern, ledger: u64) -> Option<(u64, Vec<usize>)> {
    ...
    // and exact scorings, i.e. 5·nnz + 9·pairs ≤ ledger. Size and pair sum
    // cheaply filter duplicates; exact membership confirms equality before
    // the Schur complement is skipped.
    let share = ledger;
    ...
    // the set is new, and they are skipped when exact membership matches a
    ...
    for mut in_x in candidates {
        ...
        if set_was_admitted(&admitted, &seen_sizes, &in_x, (xs, pairs)) {
            continue;
        }
...
fn run_sequential_180(sp: &ScoringPattern, ledger: u64) -> Option<(u64, Vec<usize>)> {
    ...
    // and two exact scorings, i.e. 5·nnz + 9·pairs ≤ ledger. Size and pair sum
    // cheaply filter duplicates; exact membership confirms equality before
    // the Schur complement is skipped.
    let share = ledger;
    let max_pairs = share.saturating_sub(5 * nnz as u64) / 9;
...
mod exact_set_dedup_tests {
    ...
    #[test]
    fn equal_set_summaries_do_not_hide_different_schur_complements() {
