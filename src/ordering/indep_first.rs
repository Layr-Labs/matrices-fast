    ...
    let eval_set = |in_x: &[bool]| -> Option<(u64, Vec<usize>)> {
        ...
        let run_pass = |k: usize| -> Option<(u64, Vec<usize>)> {
            ...
            let p: Vec<i32> = match k {
                ...
                _ => {
                    // iter162a: AMF α2
                    let o = feral_amf::AmfOptions { dense_alpha: 2.0, ..Default::default() };
                    feral_amf::amf_order_opts(&ccore, &o).ok()?.0
                }
            };
        ...
        if use_amf {
            pass_ids.push(1);
            if cn <= 8_000 && cnnz <= 120_000 {
                pass_ids.push(3); // AMF α5
            }
            if cn <= 5_000 && cnnz <= 80_000 {
                pass_ids.push(4); // AMF α2 iter162a
            }
        }
        if use_metis {
            pass_ids.push(2);
        ...
        if use_amf && cn <= 3_000 && cnnz <= 30_000 {
            ...
            for seed in 1u64..=4 { // iter164b seed4 only
