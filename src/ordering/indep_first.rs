...
impl ExtraCores {
    pub(crate) fn refine_search<S>(&self,p:&[usize],f:u64,score:S)->Option<Vec<usize>>
    where S:Fn(&[usize])->u64 {
        #[cfg(test)]
        if !search_enabled() {return None;}
        extra_search::refine(self,p,f,score)
    }
...
#[cfg(test)]
thread_local! {static SEARCH_ENABLED:std::cell::Cell<bool>=std::cell::Cell::new(true);}
#[cfg(test)]
pub(super) fn set_search_enabled(enabled:bool) {SEARCH_ENABLED.with(|c|c.set(enabled));}
#[cfg(test)]
fn search_enabled()->bool {SEARCH_ENABLED.with(|c|c.get())}
