...
#[test]
#[ignore]
fn probe_compact_core_search() {
    let mut cache=read_cache("/tmp/matrices-fast-early32-campaign-seeds.tsv");
    let mut sums=[[0.0;3];2];let mut counts=[0;3];let mut wins=0;
    let mut seconds=0.0;let mut maximum=0.0f64;let mut next_cache=String::new();
    for (name,pat) in crate::corpus::corpus() {
        ...
        let base=score(&p);let factor=ws.borrow().nnz_l();let mut best=p.clone();let mut f=base;
        if pat.n>=32 &&pat.n<=18_000 &&pat.nnz()<=80_000 &&factor<=100_000 &&base<=20_000_000_000 {
            if let Some((_,_,cores))=run_with_cores(&sp,8_000_000) {
                let t=Instant::now();
                if let Some(p)=cores.refine_search(&p,base,&score) {
                    best=crate::ordering::post_core::refine(&pat,&p,1,200_000,&score,
                        |p|builder.borrow_mut().permute(p),||ws.borrow().nnz_l()).unwrap_or(p);
                    f=score(&best);assert!(f<base);
                }
                let elapsed=t.elapsed().as_secs_f64();seconds+=elapsed;maximum=maximum.max(elapsed);
            }
        }
        assert!(crate::ordering::is_bijection(&best,pat.n));wins+=usize::from(f<base);
        next_cache.push_str(&name);next_cache.push('\t');
        for (i,v) in best.iter().enumerate() {if i!=0 {next_cache.push(',');}next_cache.push_str(&v.to_string());}next_cache.push('\n');
        let cp:Vec<i32>=pat.col_ptr.iter().map(|&v|v as i32).collect();let ri:Vec<i32>=pat.row_idx.iter().map(|&v|v as i32).collect();
        let raw=feral_ordering_core::CscPattern::new(pat.n,&cp,&ri).unwrap();
        let amd:Vec<usize>=run_pass(&raw,Pass::Amd).unwrap().into_iter().map(|v|v as usize).collect();let reference=score(&amd);
        let b=if pat.n<1_000 {0} else if pat.n<10_000 {1} else {2};counts[b]+=1;
        sums[0][b]+=(base as f64/reference as f64).ln();sums[1][b]+=(f as f64/reference as f64).ln();
        println!("COMPACT_ROW\t{name}\t{}\t{}\t{reference}\t{base}\t{f}\t{factor}",pat.n,pat.nnz());
    }
    assert!((aggregate(&sums[0],&counts)-0.791635371701).abs()<1e-11);
    std::fs::write("/tmp/matrices-fast-compact-core-campaign-seeds.tsv",next_cache).unwrap();
    println!("COMPACT_TOTAL score={:.12} wins={wins} seconds={seconds:.6} max={maximum:.6}",aggregate(&sums[1],&counts));
}

#[test]
#[ignore]
fn probe_retained_exact_search() {
    use crate::ordering::{rgreedy,post_core};
    let mut cache=read_cache("/tmp/matrices-fast-early32-campaign-seeds.tsv");
    let mut sums=[[0.0;3];9];let mut counts=[0;3];let mut wins=[0;8];
    let mut seconds=[0.0;8];let mut maxima=[0.0f64;8];
    for (name,pat) in crate::corpus::corpus() {
        let p=cache.remove(&name).unwrap();
        let sp=ScoringPattern{n:pat.n,col_ptr:pat.col_ptr.clone(),row_idx:pat.row_idx.clone()};
        let ws=std::cell::RefCell::new(scoring_ws::ScoreWorkspace::new(pat.n,pat.nnz()));
        ...
        let reference=score(&amd);let mut row=[base;8];let mut costs=[0.0;8];
        ...
        if pat.n>=32 &&pat.n<=18_000 &&pat.nnz()<=80_000 &&factor<=200_000 &&base<=20_000_000_000 {
            if let Some((_,_,cores))=run_with_cores(&sp,8_000_000) {
                for (rank,(amd,lc)) in cores.cores.iter().enumerate() {
                    if amd.saturating_mul(2)>base.saturating_mul(3) ||lc.core_pat.n>12_000 {continue;}
                    let mut inverse=vec![usize::MAX;pat.n];
                    for (i,&v) in lc.il.core_ids.iter().enumerate() {inverse[v]=i;}
                    let seed:Vec<usize>=p.iter().filter_map(|&v|if inverse[v]!=usize::MAX {Some(inverse[v])} else {None}).collect();
                    assert!(crate::ordering::is_bijection(&seed,lc.core_pat.n));
                    let mut core_ws=scoring_ws::ScoreWorkspace::new(lc.core_pat.n,lc.core_pat.row_idx.len());
                    let cf=core_ws.flops(&lc.core_pat,&seed);
                    let projected=splice(&lc.il,&seed);let projected_f=score(&projected);
                    assert_eq!(projected_f,cf+lc.il.prefix_flops);
                    for f in &mut row {*f=(*f).min(projected_f);}
                    if projected_f<union_f {union_f=projected_f;union=projected;}
                    if core_ws.nnz_l()==(lc.core_pat.n+lc.core_pat.row_idx.len()/2) as u64 {
                        println!("CORE_SEARCH_CERTIFIED\t{name}\t{rank}\t{cf}\t{projected_f}");
                        continue;
                    }
                    let adj=rgreedy::Game::build_adj(lc.core_pat.n,&lc.il.core_col_ptr,&lc.il.core_row_idx).unwrap();
                    for arm in 0..6 {
                        let budget=[80_000_000,160_000_000,240_000_000,160_000_000,160_000_000,240_000_000][arm];
                        let rng=if arm==3 ||arm==5 {0xd1b54a32d192ed03} else {0x9e3779b97f4a7c15};
                        let mut params=rgreedy::Params::DEFAULT;
                        if arm>=4 {params.phase_a_num=1;params.phase_a_den=1;params.lns=false;}
                        let t=Instant::now();let result=rgreedy::search_with(lc.core_pat.n,&adj,&seed,cf,budget,rng,params);
                        let mut f=u64::MAX;
                        ...
                        println!("CORE_SEARCH_CANDIDATE\t{name}\t{rank}\t{arm}\t{cf}\t{f}\t{elapsed:.6}");
                    }
                }
            }
            row[6]=union_f;costs[6]=costs[..6].iter().sum();row[7]=union_f;costs[7]=costs[6];
            if union_f<base {
                let t=Instant::now();
                if let Some(p)=post_core::refine(&pat,&union,1,200_000,&score,
                    |p|builder.borrow_mut().permute(p),||ws.borrow().nnz_l()) {row[7]=score(&p);}
                costs[7]+=t.elapsed().as_secs_f64();
            }
        }
        let b=if pat.n<1_000 {0} else if pat.n<10_000 {1} else {2};counts[b]+=1;
        sums[0][b]+=(base as f64/reference as f64).ln();
        print!("CORE_SEARCH_ROW\t{name}\t{}\t{}\t{reference}\t{base}\t{factor}",pat.n,pat.nnz());
        for arm in 0..8 {
            assert!(row[arm]<=base);sums[arm+1][b]+=(row[arm] as f64/reference as f64).ln();wins[arm]+=usize::from(row[arm]<base);
            seconds[arm]+=costs[arm];maxima[arm]=maxima[arm].max(costs[arm]);print!("\t{}",row[arm]);
        } println!();
    }
    assert!((aggregate(&sums[0],&counts)-0.791635371701).abs()<1e-11);
    for arm in 0..8 {println!("CORE_SEARCH_TOTAL arm={arm} score={:.12} wins={} seconds={:.6} max={:.6}",
