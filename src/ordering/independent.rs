...
fn lift(pattern: &Pattern, selected: &[bool]) -> Option<Core> {
    ...
    let mut cp = Vec::with_capacity(k + 1);
    let mut ri = Vec::new();
    cp.push(0);
    for mut row in rows {
        row.sort_unstable();
...
fn metric_order(graph: &feral_ordering_core::CscPattern<'_>) -> Option<Vec<i32>> {
    ...
    let mut native_work = super::quotient::WORK;
    while ws.nel < ws.n {
        ...
        native_work = native_work.checked_sub(super::quotient::front_work(&ws, start, end))?;
...
fn donors(pattern: &Pattern, incumbent: u64) -> Vec<Vec<usize>> {
    ...
    for (second, cap) in [
        ...
        let Ok(order) = super::quotient::order(&graph, 10.0, true, false, super::quotient::WORK)
        else {
            continue;
        };
    ...
    cores.truncate(2);
    let candidates = super::map_indexed(&cores, |i, (core, cp, ri, ..)| {
        let graph = feral_ordering_core::CscPattern::new(core.graph.n, cp, ri)?;
        let order: Vec<_> = metric_order(&graph)?
            .into_iter()
            .map(|v| v as usize)
            .collect();
...
pub(super) fn refine(pattern: &Pattern, scoring: &ScoringPattern, best: &mut Candidate) {
    ...
    let mut pool = super::decomposition::WholePool::new(scoring);
