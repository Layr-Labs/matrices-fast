...
**The value gates are now met; the remaining question is the cap, and it is a judgment call, not a
measurement.** Against frontier `bbf58495` (hidden **0.840623**) the bar is ≈1e-4 absolute, and this
package's dev reading is 1.09–1.28e-4 depending on frame. That is thin, and this exact value package
has never completed a hidden run: `e1e6c7f2` (fork+twin) and `e4302dab` (fork alone) were both
cap-killed in the old whole-pipeline form, while the close variant `3587d1b` completed at
**0.840545** (−7.8e-5) — under the bar.

What the iter66 package changes about that record is cost, not value: the fork no longer duplicates
the prefix, and on the rows it gates the added wall is ~0 at the median (tail: up to +72 % on cheap
rows where stage 4 accepts). Whether that is enough to clear the hidden cap is unknown, and no local
frame can answer it.

Submit only after all of these are true:

1. A candidate gains at least **1.1e-4 dev** over a same-binary frontier arm — **met** in the
   production-worker frame (−1.279e-4; the harness-frame reading is ≈−1.12e-4).
2. Candidate and frontier production workers are measured in the same session on every admitted dev
   row, with per-row output and timing archived — **met**; receipts in
   `evidence/0276-graded-frame-{shared-fork,decision-gated,reps2}.log`.
3. Both release and test builds pass, followed by the full 126-test suite and a two-run permutation
   determinism check — **met** (`SSI_GRADED_REPS=2`, all 130 rows × 2 arms).
4. The submission note states the remaining cap delta and cites an archived remote receipt, and does
   not claim that a redacted failure isolates a device — **written** as
   `evidence/0276-submission-note.md`; no score is claimed, because no local harness run completed in
   this window (it stopped on `rsyn0810m04m`, outside both gates, where the frontier control fails
   identically).

If the cap risk is not acceptable, the cheap half of the package can ship alone: the bounded
`10/4/4` twin is worth ≈2.2e-5 dev and adds no new gate class.
