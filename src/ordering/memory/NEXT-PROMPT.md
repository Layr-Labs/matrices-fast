...
## CURRENT OVERRIDE — iter82, 2026-09-15 — THE FORK IS BACK, AND ITS CAP EXCUSE IS GONE

**Iter82 re-armed the shared-prefix basin fork.** The iter78–iter81 sequence is
the receipt: iter78 (`782a26d`), iter79 (`26f00f8`) and iter80 (`65c2e9d`) all
cap-failed at 84.1–84.7 s with the fork **absent**, while iter81's restore of
the four `rgreedy.rs` `Game` buffer-image behaviours **completed** the Benchmark
step in 8 min 02 s. The fork was retired on a cap argument that the tree no
longer supports. Public **0.790253862267 → 0.790161 (−9.3e-5 dev, −0.93 bips)**,
**4 movers, 0 regressions**, all in `lt_1k`; 300/300 `COUNTS`; determinism
re-run byte-identical; release suite 127 passed / 0 failed / 56 ignored. See
[0291](experiments/0291-basin-fork-rearm.md).

Shipped constants, all three in `mod.rs` and all **unconditional** (the retired
form's `#[cfg(test)]`-off / `#[cfg(not(test))]`-false split was a probe-vs-graded
divergence trap and is gone):

```
SHARED_BASIN_FORK_MAX_N    = 1_000     (was 600)
SHARED_BASIN_FORK_MAX_NNZ  = 10_000    (was 5_000)
SHARED_BASIN_FORK_MARGIN_PCT = 20
gate: n >= 6 && n <= MAX_N && nnz <= MAX_NNZ
      && best_flops * 100 < amd_flops * (100 - MARGIN_PCT)
```

The fence is priced row by row, not guessed: of the eight rows of the same shape
immediately outside the ported fence (`615..=969` vertices, `2 580..=7 260`
nonzeros, all already below ratio 0.80) exactly two move —
`multiplants_mtg1b` −0.312 % and `sonet24v5` −0.103 % — taking the device from
−8.6e-5 to −9.3e-5 dev. Margin 0 measures **0.790165** against margin 20's
**0.790168** (a 3.4e-6 difference bought by the single row `gancns`, the largest
wall in the band for 97 flops), so the margin is kept. The two rows the original
wall receipt charged, `himmel11` (ratio 1.0000) and `syn15hfsg` (0.9915), are
outside the fence by construction in **both** bands.

**Sealed in the same pass — do not retry:**

* **The simplicial reduction at any degree.** New exact `core_lift` rule
  (eliminate a vertex whose live neighbourhood is already a clique, at ANY
  degree, charged `d(d-1)/2` before it is attempted). It fires on most rows and
  shrinks cores by **20–98 %** (`squfl030-150` 13 680 → 180, `emfl050_5_5`
  13 175 → 650) and **300/300 flop records are bit-identical**: SCORE 0.790254
  both ways. Widening `order_core`'s small-core quality gate from `1000..10_000`
  to `n <= 60_000` on top of it is also identical. **A smaller core is not a
  better ordering**; the core-size axis is closed as a value lever. See
  [0290](experiments/0290-simplicial-reduction-negative.md).
* **The five `gt_10k` anchor ties are optima, not search failures.**
  `probe_tie_forensics` shows all five carry fill, but `probe_tie_headroom`
  finds 21 diverse candidates (six AMD option pairs, three AMF α, METIS
  default/tuned/high-trial, Scotch, RCM, Sloan, ND, six custom metrics) all
  landing on **exactly 1.00000000**. Read `fillfree = 0` as evidence about
  chordality, never as headroom.
* **`probe_eval_audit` is still `leak_rows = 0`, `recoverable_bips = 0.00`**
  (300 rows, 3121 scored candidates) — the stale-`best_flops` family has no dev
  headroom.

**Open, and the honest read of this submission:** the fork's value is 100 %
inside `lt_1k` and concentrated in **4 rows**, so it is a band-shaped device and
the lane's transfer law (`band-confined value transfers ~0`) applies to it. The
counter-evidence is the public analogue `3587d1b` (PR #689), the same fence,
which **completed** a hidden corpus at 0.840545 — a −7.8e-5 hidden gain on a
−8.3e-5 dev gain, transfer ≈0.94. Nothing local separates those two readings, so
the verdict is the experiment. If it lands short, the next lever is a **second
lineage inside the fork** (a different withheld stage, or a different seed set
for the alt suffix) rather than a wider fence, which is now priced out.

---

## PRECEDING OVERRIDE — iter81, 2026-09-15 — THE CAP KILL IS FIXED

**Iter81 completed the hidden Benchmark step.** Submitted as `849643ab` / PR #747,
workflow `34997790290`: Benchmark entered `16:55:45.0196689Z`, finished
`17:03:47.0960286Z` — **8 min 02 s**, no kill. Verdict: scored **0.840510**
against the frontier **0.840511**, `improves but fell short of the required 1 bip
improvement`, so **rejected by 1e-6**.

This is the lane's **first completion since iter72** (`0e1edc92`, 14 min 02 s).
The eight submissions between them were all killed at the 2.0 s per-matrix cap
inside the first ≈85 s. The change that fixed it was restoring the promoted `Game`
buffer lifecycle: release the 1 GiB sparse image at the promoted 160 MB pool
ceiling, give `new_sparse` a fresh zeroed image, drop
`new_reset_first_with_degrees`/`assemble(…, initialize_adj)`, and drop the
`sparse_adj_is_pristine` first-reset fast path (net **−55** lines, public score
unchanged at 0.790253862267, 300/300 `COUNTS`, suite 127/0/56). **Do not
reintroduce any of those four behaviours** — they cost >8 minutes of Benchmark wall
on the binding row and were invisible locally. See
[0289](experiments/0289-restore-promoted-buffer-lifecycle.md).

**The remaining problem is the score, and it looks architectural.** 0.840510 vs
0.840511 reproduces iter72's near-tie to 1e-6 on a tree that differs from it by
dozens of search devices. Two independent completions now land within 1e-6 of the
frontier while differing by ~1e-4 on dev: value work inside the exact-window family
is not transferring. The ledger lever (2 GiB → 1 GiB) was **not** needed as a cap
measure and is untested, not exonerated.

`MAX_N` is refuted as a wall lever — do not retry it. Running the release probe with
`SSI_PROBE_PHASES=1 SSI_MARK_NOSCORE=1` on the five dev rows above 80 000 vertices
gives `22.win == final` on every one, i.e. **zero** terminal-region seconds; their
wall is `1.portfolio` (1.60 s of `gabriel10`'s 3.38 s), which no change since iter72
touches.

**Submission mechanics note:** submit with `--model "DeepSeek-V41-Flash"` (the exact
string the grader expects).

---

## PRECEDING OVERRIDE — iter80, 2026-09-15

Iter80 was submitted (the handoff had not recorded it) as `65c2e9d` / PR #746 and
cap-failed at **84.1 s** with the same `order() exceeded the 2.0s per-matrix cap`
kill. All four same-day endpoints now sit in an **84.1–84.7 s** band, so nothing
in this sequence orders by tree cost.

Fetching the promoted base `05fa99b`'s ordering sources settles what had actually
diverged: `MAX_N = 1 << 17`, the sparse `Pristine` representation, `new_sparse`,
`XCH_ALLOC` and the ledger are **identical** in both trees. `mod.rs` differs only
in `#[cfg(test)]` seam defaults (plus `peo_alt_seeds()`), and `window_dp.rs` only
in test instrumentation plus `PRODUCTION_XCH_ALLOC`. The single functional
divergence was four `rgreedy.rs` buffer-image changes, so iter81 restores the
promoted `Game` lifecycle: release the 1 GiB sparse image at the promoted 160 MB
pool ceiling, let `new_sparse` own a fresh zeroed image, and drop
`new_reset_first_with_degrees` / `assemble(…, initialize_adj)` and the
`sparse_adj_is_pristine` first-reset fast path (net **−55** lines). Public score
is unchanged at **0.790253862267** with 300/300 `COUNTS` records; the release
suite is 127 passed / 0 failed / 56 ignored. The next action is hidden submission
of this exact tree. See [0289](experiments/0289-restore-promoted-buffer-lifecycle.md).

**Refuted and reverted — do not retry.** `MAX_N` is *not* the wall lever. Running
the release probe with `SSI_PROBE_PHASES=1 SSI_MARK_NOSCORE=1` on the five dev
rows above 80 000 vertices (`cont6-qq`, `acopf_case9241pegase_qcqp`, `faclay75`,
`gabriel10`, `unitcommit_200_100_1_mod_8`) gives `22.win == final` on every one of
them, i.e. **zero** terminal-region seconds: `terminal_exchange` never fires there.
Their wall is `1.portfolio` (1.60 s of `gabriel10`'s 3.38 s), which no change since
iter72 touches. A one-line narrowing to `80_000` was measured and reverted.

The ledger endpoint is **not** needed as a cap measure and stays untested, not
exonerated: iter81 completed, so the cap cause is the buffer lifetime.

---

## PRECEDING OVERRIDE — iter79, 2026-09-15

Twin-free iter79 was submitted as `26f00f80` / PR #745 and cap-failed after
about 84.360 s, again the same hidden position. Both newly composed search
devices are now exonerated on the binding row.

An exact comparison with promoted base `05fa99b` found global residue from the
retired fork: every row's complete suffix still lived inside the fork closure
and used converted scorer/runner-up ownership even though production admission
was constant false. Iter80 deletes that scaffolding and restores the promoted
direct phase-4-through-terminal control flow. Search behavior and public score
remain unchanged at **0.790253862267**. See
[0288](experiments/0288-restore-direct-suffix-control-flow.md).
Production candidate and trusted parent builds clean; final focused records are
unchanged; release suite 127 passed / 0 failed / 56 ignored. The next action is
hidden submission of this exact tree.
...
ignored. Hidden submission `26f00f80` cap-failed after about 84.360 s.
