...
`PRODUCTION_EXCHANGE_LEDGER` 2G → **4G** (the step `0237` priced at −1.03e-4 dev and refused because it
loads `crudeoil_lee4_10`; re-priced in-frame on this tree below).
...
Ledger 2G → 4G, re-priced in-frame on this tree (`SSI_EXCHANGE_LEDGER` seam, everything else fixed),
moves three rows — `crudeoil_lee4_10` **−1.10 %**, `crudeoil_lee4_09` **−0.59 %**, `nuclear10a`
−0.001 % — and further improves two band rows from the table above (`crudeoil_pooling_dt3` 0.7056 →
**0.7021**, `arki0013` 0.3993 → **0.3975**):

```
SCORE 0.790425 -> 0.790295   (-1.30e-4)   shared engine + ceiling 45 000   [full 300-row probe]
SCORE 0.790295 -> ~0.79006   (-2.37e-4)   + ledger 4G                      [its five measured movers]
buckets  0.8873 / 0.8373 / 0.6826  ->  0.8873 / 0.8373 / 0.6820
```

The two steps are disjoint: the ceiling acts only on rows the class never reached
(25 000 < n ≤ 45 000), the ledger only on rows that already ran the exchange. Their sum is the
composition of separately measured deltas, not a single end-to-end run — the full 300-row probe was
taken before the ledger step and no further full run was possible in this session's window.
...
* **Superseded:** the ledger 2G → 4G step (−1.03e-4 dev) — it is now shipped, on the re-price in §3: the shared
  engine takes ~0.1–0.15 s OFF every class row (nine of the eleven builds at n = 17 809 are ~14.9 M
  word-ops each), the same order as this step's added work, and an interleaved min-of-3 timing A/B over
  six class rows (including the corpus's slowest, `chimera_selby-c16-02`) reads −1.19 s total with no
  row systematically worse.
