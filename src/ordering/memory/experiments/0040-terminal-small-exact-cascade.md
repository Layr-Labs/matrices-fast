...
**Private status:** **FAILED validation** as submission `1fbb1a08`. The public
CLI did not expose a failure category. Because the candidate passed the full
local cap/determinism/correctness run and the new terminal search was the only
added work, it is the primary hidden-cap suspect; the entire terminal addition
was removed before experiment 0041.
...
That local conclusion did not survive private validation: submission `1fbb1a08`
failed without a public score. Do not retry the terminal serial/parallel search
on this gate. The fixed-work subtree allocation itself is independently safe—it
was promoted in submission `26932eb`—but the additive whole-graph work is not.
