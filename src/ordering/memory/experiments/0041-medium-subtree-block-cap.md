...
**Status:** **FAILED private timing** as submission `fd357537`. The public
workflow reported: `hidden matrix: order() exceeded the 2.0s per-matrix cap and
was killed`. The failure occurred in GitHub Actions run `33788875544`. See 0042
for the measured runtime diagnosis and bounded-block repair.
...
This revision adds no candidate or configured operation ceiling. That fact did
**not** make it runtime-neutral. Lowering `max_s` changed which disjoint blocks
were eligible; the 32-block first round could fill more of its slots and steer
later conditional rounds into a much slower basin. A post-failure direct timing
probe measured the promoted frontier at 1.081 s worst on this machine and this
candidate at 1.606 s, with `rsyn0810m04m` becoming the candidate's worst case.

The lesson is that a cap on *requested* work is not a cap on actual portfolio
runtime when it changes the incumbent and therefore the conditional chain.
`max_s=256` remains a useful score mechanism, but it must be paired with a
first-round block cap chosen from end-to-end timing. Experiment 0042 does that.
...
The medium-only cap is a real public-score improvement: **0.849487 → 0.849194**
with both non-target buckets unchanged. It is **not shippable as written**:
submission `fd357537` exceeded the hidden 2 s cap. The original safety argument
mistook an unchanged requested-work ceiling for unchanged realized work.
