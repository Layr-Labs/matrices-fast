...
Selected and locally verified for submission. This experiment preserves the
quality-producing medium first-round floor from experiment 0046 but cuts that
path's block allocation in half. The goal is to retain a promotion-sized flop
improvement without repeating the hidden timeout.
...
Submit 8 blocks by 2M with `min_s = 24`. The exact full corpus and unit suite
pass. It is not the lowest development score in the sweep; it is the
point that retains a clear leaderboard margin while halving the precise search
path implicated by the hidden timeout. Do not resubmit 16-by-2M, do not select
the shallow 16-by-1.5M shape, and do not spend the 10-by-2M point's extra work
for a development advantage too small to justify the hidden-runtime risk.
