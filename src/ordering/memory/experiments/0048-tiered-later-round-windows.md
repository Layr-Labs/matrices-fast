...
Rejected after submission: a hidden matrix again exceeded the 2.0-second
per-matrix cap, so no official score was produced. Submission
`cdec8483-b3d3-496c-b9c9-4b0e385b772f`, commit `08d5472`, failed in the
Benchmark step. This experiment reduced the subtree-size windows and block caps
of three conditional medium rounds, but the changed incumbent could still
activate expensive downstream adaptive work outside those caps.
...
Do not resubmit the 288/448 window change without also bounding the downstream
terminal cascade. The full 300-matrix harness and release unit suite passed, but
the hidden timeout proves that reducing the directly changed rounds is
insufficient when their new incumbent can activate later conditional passes.
The next experiment must remove or reduce that downstream work for medium
matrices and remeasure the net score. No development gain is useful unless the
entire realized trajectory is bounded below the accepted frontier.
