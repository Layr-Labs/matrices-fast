...
Rejected by hidden timing. This experiment combined the
...
Submission `9a16b5b8-cd26-49a2-868c-b6f0274f0cbb` failed in workflow
`33810603225` with the same terminal receipt as the preceding three attempts:

```text
RUN FAILED: hidden matrix: order() exceeded the 2.0s per-matrix cap and was killed
```

The reduced terminal cascade did not repair the hidden failure. Close this
window-size direction and do not retry it: four independently bounded variants
have now produced the same hidden timeout.
