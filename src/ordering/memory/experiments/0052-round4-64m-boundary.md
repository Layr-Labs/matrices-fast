...
Rejected by hidden timing. Starting from the hidden-accepted 0051 source,
...
Submission `de541fe9-909c-4725-8792-c1f344cb2299` failed in workflow
`33815111328`. The hidden benchmark stopped only nine seconds after it began:

```text
RUN FAILED: hidden matrix: order() exceeded the 2.0s per-matrix cap and was killed
```

This sharply identifies the private boundary: round 4 at 32M passed and
promoted; the otherwise identical 64M setting fails almost immediately. Restore
32M and do not test any intermediate or larger raw budget. Further work must be
cost-neutral or reallocate already accepted work.
