...
Rejected by hidden timing. Submission
`bf35c66a-8396-4129-9bcc-282c2b989fda` failed in workflow `33818670149`:

```text
RUN FAILED: hidden matrix: order() exceeded the 2.0s per-matrix cap and was killed
```

The promoted 0053 source was restored immediately. This experiment starts from the promoted 0053
...
The hidden timeout activates the predeclared stopping rule. Restore promoted
0053 immediately and close both 128M and the upper-medium 64M extension. The
0.814-second public worst was not sufficient evidence for an unseen medium
matrix, and another cutoff retry would be timing overfit rather than robust
optimization. The confirmed official leader remains 0053 at **0.871239**.
