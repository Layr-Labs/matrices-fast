...
### 3.2 Failed: stack trial (`96b612df`, hidden timeout) and no-ceiling bundle (`273fff1`, rejected)
...
*Stopping guidance (resolved 2026-09-04, updated after bundle3).* Stack
**timed out**; bundle3 (no ceiling raised anywhere: H4 + 16x32M swap +
tail-gate + ND leaves, dev −1.15) **scored but rejected** at hidden 0.871015
(−0.17 bips, 0.15x translation). Timing is now fixable; transfer is not:
stacked weak positives (<1 bip each) do not survive the hidden refresh even
when perfectly additive on dev. H2 restored exactly. Nothing in inventory
clears the bar alone (best: ND-leaf −0.56, R5-tier −0.41, H4 −0.31, swap
−0.27). Next submittable candidate must be a SINGLE >1-bip mechanism with
drop-top breadth proof — currently only mover-targeted exact search
qualifies structurally, and only as pure reallocation.
