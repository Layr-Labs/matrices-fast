...
- **Status:** **FAILED hidden** as `d6e006a` / `53ce411`. CLI `failed`, not score-rejected. α=0.5 as a *global* replacement for 16.0 is slower (fewer dense deferrals) on the nnz≈120k cap owner `crudeoil_lee4_10`. Reverted. Do not retry a global 0.5 swap. Narrow-band 0059 is the one remaining retry.
