...
- **Candidate Development Score:** 0.843657 (fill 0.943813)
- **Delta:** −0.000321 (−3.21 basis points vs base on full 300 dev corpus)
...
- Course correction: Replaced the additive pass with **substitutive re-tiering** within the existing `SUBTREE_SEARCH_WORK_LIMIT = 32M` and `TERMINAL_SUBTREE_SEARCH_WORK_LIMIT = 16M` bounds:
  ...
  - Extended chained terminal passes 2, 3, and 4 to sparse below-anchor `gt_10k` matrices ($nnz \le 60k$).
  - Added a 3rd stream to medium exact LNS.
- Measured dev score: **0.843657** (−3.21 bips dev gain).
- Worst local execution time: **1.352 s** (faster than base `1.358 s` on `crudeoil_lee4_10`).
- Total dev suite runtime: **116.3 s** (down from `165 s` on base).
...
|---|---:|---:|---:|---:|
| **Aggregate Score** | **0.843978** | **0.843657** | **−0.000321** | **−3.21 bips** |
| `gt_10k` (weight 0.40, count 45) | 0.791954 | **0.791924** | −0.000030 | −0.30 bips |
| `1k_10k` (weight 0.30, count 108) | 0.866998 | **0.866176** | −0.000822 | −8.22 bips |
| `lt_1k` (weight 0.30, count 147) | 0.890324 | **0.890328** | +0.000004 | 0.00 bips |
| **Worst `order()` time** | **1.358 s** | **1.352 s** | −0.006 s | safely under 2.0 s cap |
