...
- **Status:** LOCAL WIN, **HIDDEN FAIL** (submission `395313f1`, 2026-09-05). No hidden score — the 2 s cap. Extra relabel on `n >= 10k` at the new 0.90 band is the suspected cause. See [0063](0063-drop-large-090-extra-relabel.md).
...
Submission `395313f1-209c-4e5f-b57e-bda7f18f942b` **failed** (no score). Same signature as the 0060 additive large-graph timeout. The new work that can hit `n >= 10k` is extra relabel at ratio 0.80–0.90 (`12` AMF+AMD seeds, `nnz <= 100k`). The second miss-retry is `n < 10k`. Extra LNS is `n <= 6k`. Those two stay.
