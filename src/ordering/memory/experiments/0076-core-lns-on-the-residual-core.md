...
- **Score:** dev 0.831772 → **0.831500** (−3.3 bips); lt_1k 0.889730 → **0.888824**, `1k_10k` and gt_10k identical to the frontier
- **Status:** small-rows-only time-neutral shape: submitted for hidden validation (full trusted 300-matrix run OK, `results.tsv` 1788689790; 66 tests). Earlier shapes: ADDITIVE (−6.1 bips dev, up to +0.11 s on 0.8 s rows) **failed hidden validation** (time) as `f118a2c5`; time-neutral on small AND medium rows (−3.8 bips dev) **passed the cap but was rejected on hidden score** (+1.2 bips) as `35f129c9`
...
- **Reduce once, decide early:** for 50 ≤ n < 10k the terminal block's K = 3 `core_lift::reduce` runs
  ...
  n ≤ 1000 LNS gate (small rows only; the medium-gate trade was dropped after `35f129c9`).
- **Pay first:** on paid rows `trim_streams` removes Σ`CORE_LNS_STREAMS` = 60M ops from the TAIL of
  the row's full-graph LNS stream list (the fifth 100M stream → 40M; with the sixth well-below stream
  present, the sixth goes and the fifth → 90M), so no row runs more exact-search ops than the
  frontier; every row with n > 1000 is bit-identical to the frontier.
...
Small-rows-only time-neutral shape, full trusted run (`results.tsv` 1788689790, 300 OK):
**0.831772 → 0.831500** (−3.3 bips), lt_1k 0.889730 → 0.888824, `1k_10k` / gt_10k identical.
**7 better / 1 worse / 292 identical**: pooling_adhya4tp 0.8595 → 0.8044, wastewater05m1 0.7274 → 0.6938,
waterund11 0.7284 → 0.7151, st_e31 0.9491 → 0.9389, pooling_adhya4pq 0.6843 → 0.6809, korcns 0.9292 →
0.9268, graphpart_3g-0244-0244 0.9719 → 0.9709; worse multiplants_mtg1b 0.6632 → 0.6642. Timing (2-core
pin, alone): mean row −0.5 ms, 163 rows faster, slowest small rows +0.014–0.019 s (noise on an unchanged
op budget), rows with n > 1000 byte-identical code.

Earlier shapes, same box: additive (`f118a2c5`) 0.831264 (−6.1 bips, 14 / 0 / 286, up to +0.11 s on
0.8 s rows) — failed the hidden cap; time-neutral on small AND medium rows (`35f129c9`) 0.831457
(−3.8 bips, 12 / 5 / 283; lt_1k 0.888824, `1k_10k` 0.863503) — passed the cap, hidden 0.861309 vs
0.861203 (+1.2 bips), rejected. The hidden loss cannot be attributed from dev; the medium trade
(60M off a 150M budget, the two largest dev regressions) is the hypothesis this shape tests.
...
- Rows with an eligible core but no full-graph LNS (6000 < n < 10k, or nnz > 30k) get nothing;
  priced with `probe_core_lns_unpaid`: 8 such rows on dev, 1 mover, −0.03 bips — closed. (The
  additive tree's rsyn0830m04m / rsyn0820m04m movers came from the widened full `refine_core` chain
  on cores > 1000, not from the walk.)
