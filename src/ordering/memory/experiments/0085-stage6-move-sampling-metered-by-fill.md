...
incumbent already permits, and the two hygiene items strictly *return* time. Submission `f8941f7f`
re-ran the candidate **without** the stage-6 sets as a bisection, and returned both answers:

- **`rejected`, hidden 0.859571** — it ran to completion, so the `failed` is attributable to the
  stage-6 sets, and the pool deepening plus the two hygiene items are cap-safe.
- **0.61 dev bips bought 0.02 absolute hidden bips: a translation of 0.03**, against `0084`'s 0.64
  for *introducing* the same mechanism an hour earlier.

That second number is worth more than everything else in this page. *Introducing* a mechanism fires
wherever its precondition holds, which is a property of patterns in general, so it transfers (0.58
and 0.64 for the two introductions on record). *Deepening* one only fires where the shallow version
ran out — rarer and far more corpus-specific: 4 dev rows in 300, and essentially nothing hidden.
**The promotion floor is therefore ~1.35 dev bips for an introduction and ~26 for a deepening**, and
a constant sweep, a budget increase, a pool depth, an extra seed set and a wider gate are all
deepenings. That is the arithmetic reason five rounds of constant-tuning across this benchmark have
produced so few promotions relative to attempts.
...
**Build a mechanism, do not sweep a constant.** Everything this page swept - stream sets, pool depth,
gate width - is a deepening, and a deepening translates at 0.03. `0084`'s own next step - seeds from the stages that assign `best_perm` directly
