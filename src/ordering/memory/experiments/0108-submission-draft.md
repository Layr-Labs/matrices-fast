# DRAFT — bounded chordal completion exchanges

DO NOT SUBMIT: integrated capped public verification is PENDING. Replace this
warning and the pending receipt section only after inspecting actual results.
...
improvements. This is a calculation from independent screen outputs, not yet
an integrated production or hidden-corpus measurement. The block result adds
...
## Integrated verification receipt — PENDING
...
PENDING: record baseline/candidate trusted capped grader outcomes, exact
integrated score and all 300 integer counts, targeted test results, full profile
timing, runner identity, any failures, and the submission decision. Do not
replace these fields with an expected score or a prior screen's result.
