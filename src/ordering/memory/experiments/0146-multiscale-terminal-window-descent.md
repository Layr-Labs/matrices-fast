...
Submission `42491bf1` failed the hidden benchmark because one hidden matrix
exceeded the 2.0-second `order()` cap. The workflow reached the benchmark step
normally and failed after about 86 seconds of hidden evaluation; no score was
uploaded. The local score evidence remains valid, but the broad ten-pass gate
is closed. The structurally reduced retry is documented in
[0147](0147-two-tier-window-runtime-envelope.md).
