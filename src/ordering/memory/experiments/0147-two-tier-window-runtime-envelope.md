...
Submission `a1f928e2` failed the hidden benchmark at the same 2.0-second
per-matrix cap as 0146. Workflow `34444803770` entered the benchmark normally
and was killed 106 seconds after the grader started. Since this version removed
the ten-pass chain from medium rows but kept it on `n <= 3,000`, the repeated
failure rules out the two-tier policy as a safe runtime envelope. The much
smaller fixed-work successor is documented in [0148](0148-low-width-terminal-budget.md).
...
This version did reach the hidden cap. The nearly identical failure point to
0146 implicates work retained by this retry, so keeping the full small tier
alone is not a sound fallback. The successor replaces it with three low-width
8M passes and removes the sparse-medium tier entirely.
