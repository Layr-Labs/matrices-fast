...
already has substantial per-row work. No local measurement can prove remote
headroom. This retry responds to both observed failures by deleting their
high-budget paths and adding only a small fixed allowance where development
gains were measured. If even this package times out at the same hidden corpus
position, the defensible conclusion is that the affected hidden row has almost
no headroom beyond the promoted parent; the next retry should restrict new work
to a disjoint tiny dimension band or remove terminal windows entirely.
