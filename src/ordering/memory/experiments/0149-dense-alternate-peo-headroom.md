...
The hidden corpus and machine remain unavailable locally, so a public timing
margin cannot prove that every hidden row stays below two seconds. This package
does address the strongest evidence from the three failures: it creates about
90 ms of margin on the slowest matching public rows, reduces the measured
worst by roughly 11%, and preserves every public flop count. If another hidden
timeout occurs, the next useful step is another final-count lineage cut in an
inherited large-row stage rather than deleting the already tiny, disjoint
terminal budget that produces the score improvement.
