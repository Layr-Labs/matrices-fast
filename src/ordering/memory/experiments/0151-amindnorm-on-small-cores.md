...
- **Status:** win on dev; **submitted as `151a98ce` and graded 0.842857 —
  bit-identical to the frontier on the hidden corpus (0.00 % diff, same fill),
  i.e. the hidden corpus contains no row where the gated AmindNorm pass wins,
  so the submission was rejected as score-neutral.** It DID clear the hidden
  2 s cap. The change stays in the tree: it is output-neutral where it does
  not win and keeps the dev gain.
...
- **Metric variants on the REDUCE core (0151b, `probe_reduce_core_metrics`):
  zero wins on all 227 corpus rows whose K=3 core builds.** The production
  reduce candidate only ever wins through downstream polish; RAW reduce-core
  totals (AMD/AMF × α, all 11 metric variants, METIS) never beat the polished
  pipeline final — e.g. risk2bpb's best raw core total is 1.62× the final.
  The reduce-core pass menu is not the limiting factor; do not widen it.
- **`dense_alpha` sweep on the lifted-core metric passes (0151c,
  `SSI_SETS_ALPHA` on `probe_indep_sets`).** Production runs every core
  metric at α=10; α ∈ {2.5, 5} on {ddnsw, dpd, amind, ddnd, sqpure} × all
  sets finds exactly two rows: edgecross24-115 at 0.9695 of the *old* final
  (= 0.7924 of AMD — WORSE than the shipped α=10 AmindNorm path, 0.7960 raw
  → 0.7555 polished) and ringpack_30_2 at 0.9999 (noise-level). The α
  dimension adds nothing; the initial census summary hid the edgecross rows
  because α variants on *shipped* sets raise the reference — the patched
  probe records `ALPHA` hit lines explicitly. Do not sweep further α.
