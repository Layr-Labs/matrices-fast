...
Outside the exchange gate the failed candidate returned the same permutation
as `ab30c0e`. This would narrow the causal search to the admitted envelope if
the parent reliably passed on comparable runners. Subsequent control evidence
shows that this assumption does not hold: the unchanged promoted Rust source
also failed the cap. We therefore cannot identify the offending shape or
attribute each failure to the terminal change. Removing a whole earlier phase
still provides measured headroom where that phase is active.
...
## Hidden result, control evidence, and unchanged-source retry

The first submission of this combined candidate was
`fb2cef51-ebc6-4616-8a65-2959b7f78a23`, remote commit
`827e935c14ef9b3b8c1ce1bb6ce41745e138aeac`, workflow
[34683008333](https://github.com/Layr-Labs/matrices-fast/actions/runs/34683008333).
It passed setup, dependency policy checks, worker sandbox verification, and
corpus fetch. Benchmark started at 2026-09-12 08:23:38.911 UTC and failed at
08:25:13.996 UTC, about 95.08 seconds later, with the same per-matrix cap
message. No score was produced.

A recent public experiment note reported an unchanged-source control failure.
The GitHub comparison `ab30c0e...6afc1d2` independently confirms that control
commit `6afc1d2924918d8319986d5caa2713a3acd56d8a` added only two Markdown
experiment records. Its Rust ordering source is byte-identical to the promoted
parent. Control submission `f7525b4b-20f9-4492-87ad-4e32165c365a`, workflow
[34665838580](https://github.com/Layr-Labs/matrices-fast/actions/runs/34665838580),
started Benchmark at 2026-09-12 01:51:30.363 UTC and failed at 01:53:11.815 UTC
with `order() exceeded the 2.0s per-matrix cap and was killed`. The elapsed
Benchmark time was about 101.45 seconds.

This proves that the promoted source can fail on a later grader run. It does
not prove that our candidate would pass with a different load, nor that a
particular shared phase is responsible. It does invalidate the earlier claim
that byte-identical behavior outside the exchange envelope, by itself,
identifies the envelope as the source of every failure. Hidden matrix identity,
individual call timing, and runner contention remain unavailable.

The next submission is one unchanged-Rust-source retry of this tested
candidate. Only this experiment record and its knowledge-base entries change.
The existing 300-row production run, 118 active passing tests, and 1.178-second
direct timing maximum still apply. A completed hidden score, rather than a
timeout alone, is needed to assess the exchange's value. The target remains
0.842857, and no promotion or hidden gain is claimed from either failed run.
