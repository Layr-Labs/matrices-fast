...
**Official outcome: FAILED hidden 2.0 s cap**, no hidden score. Submission
d895008f in workflow 34720094107, Benchmark 102.81 s. Complete remote ordering
6a1cdea matches tested/uploaded 7946bcf. The original-AMD guard hypothesis does
not resolve this run; recorded public quality does not establish runtime safety.
Campaign now **2/4 rejected, 5 failed workflows, 0 promotions**.
...
Submitted **d895008f-037d-4dc8-a7e6-28400a392d03**, status validating, exact
tested/uploaded local ordering source **7946bcf8d3875f8fb0c520c48095c55d461027f2**.
The worker source is frozen and the required sandboxed public score is
**0.790705 / fill 0.923840**. Command:

```sh
yukon submit --note-file src/ordering/memory/experiments/0214-amd-work-guard-and-larger-factors.md \
  --claimed-score 0.790705 --model 'GPT 6' --harness 'Codex'
```

Hidden grade remains unknown. Campaign **2/4 rejected, 4 failed workflows,
0 promotions**. Later research is isolated from this immutable submitted tree.

The official workflow subsequently failed with explicit hidden
`order() exceeded the 2.0s per-matrix cap`, Benchmark
**21:32:45.367755 -> 21:34:28.176409 UTC**, **102.81 seconds**. Remote
**6a1cdea71bce22ea506c47dd341ec2ec869b0b02** is byte-identical across the whole
ordering archive to submitted **7946bcf8d3875f8fb0c520c48095c55d461027f2**.
No hidden score or offending identity is supplied. The work-bound inference is
not confirmed. Current promoted best remains 07f0e8a2 / 0.842377, best valid raw
candidate remains de17cd31 / 0.842374. Campaign **2/4 rejected, 5 failed,
0 promotions**. The next runtime investigation replaces stale MCS heap entries
with one eager indexed item per vertex and measures exact equivalence before
another submission; the failed source stays preserved independently.
