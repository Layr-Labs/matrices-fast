...
**BOTH DOSES WERE KILLED — the band is OFF in production.** `fe17562d`
(`14/5/14`) and `873f23f0` (`14/5/8`) both died at the hidden per-matrix cap.
The band survives as `EXCHANGE_WIDE_ENABLED = false` plus the seams, and the
shipped tree keeps only the fork and the twin. The two doses and their walls:
...
**Shipped dose.** `14/5/8` inside the same band. Dose and wall, one binary,
...
Shipped constants: `EXCHANGE_WIDE_MAX_N = 10 000`,
...
cap-killed. The shipped `14/5/8` stack measures **0.790095** (evidence
...
(That table is the `14/5/14` bat; the shipped `14/5/8` bat moves the same two
...
The first bat of this family lost its slot to the cap, and that is the most
expensive datum here: **a +11…+35 % wall change on `n <= 10 000` class rows is
lethal on the hidden corpus, while the fork's +22 % on `n <= 600 && nnz <= 5 000`
is not** (submission `3587d1b` completed hidden with it). The band was therefore
re-dosed to +5.4 %, and the lesson generalises: on this benchmark the wall
budget is charged per *row class*, and a device is priced by the wall it adds to
the class the hidden cap already owns — not by its dev score.
