...
- **Status:** in the tree, **not shipped**. The shared-prefix fork has no hidden receipt in this form.
...
## Cap accounting (the reason this is not shipped)

| tree | added wall on the rows it gates | hidden receipt |
|---|---|---|
| fork, whole pipeline duplicated (`e4302dab`, `e1e6c7f2`) | ≈ +22 % on its band | **2/2 killed** on the 2.0 s cap |
| fork, prefix shared, both suffixes (run A) | +9.0 % aggregate, 139 rows | none |
| fork, prefix shared, stage-4 gated (runs B/C) | +1.9 % (139 rows) / median **+0.2 %** (119 fork rows) | none |
...
What is *not* established: any hidden-side margin for this form. The close variant `3587d1b`
completed a hidden corpus at **0.840545** (−7.8e-5, sub-bar) with the *old* duplicated fork, and the
two later submissions of the same value package were killed. Against the current best **0.840623**
the bar is ≈1e-4 absolute, so even a clean completion is not obviously promotable.
