...
- **Status:** both submissions cap-failed on hidden matrices: no-margin
  `48e929aa` (PR #725) and 10%-margin `c4613ce` (PR #726). Superseded by the
  output-preserving sparse setup reductions in [0283](0283-sparse-first-reset-and-buffer-retention.md).
...
## First hidden receipt and cap correction

Submission `48e929aa-c579-48d4-b96f-5f92df74b110` failed because a hidden
matrix exceeded the two-second `order()` cap. This is evidence against the one
production difference with repeated matching provenance: the no-margin basin
fork. PR #719 completed, and the iter72 two-image tree without the fork also
completed; multiple earlier no-margin fork trees cap-failed. Conversely, the
fork's 10% anchor-margin form completed once. Hidden row identity is redacted,
so this is a causal inference from controlled tree differences rather than a
claim about the hidden matrix.

The follow-up changes only `SHARED_BASIN_FORK_MARGIN_PCT` from 0 to 10. A second
suffix is now paid only after the common pipeline has already beaten AMD by at
least 10%. The public fork movers have incumbent ratios 0.3516, 0.8407, and
0.7347, so all remain admitted. A focused run confirmed their exact records are
unchanged at 70,488 / 58,202 / 78,229; the three dense-twin records also remain
2,560,253 / 2,487,272 / 492,146. The known expensive near-anchor non-movers
`himmel11` (1.0000) and `syn15hfsg` (0.9915) remain output-identical but no
longer qualify for the second suffix. Thus the claimed public score stays
`0.790142`; only fork wall exposure is reduced.

That follow-up was submitted as `c4613ce` and also exceeded the cap. Its final
test build completed at 16:53:15.433Z and the hidden kill printed at
16:54:49.228Z, about **93.79 seconds** later. This receipt was not reflected in
the original handoff; the Yukon ledger and PR #726 establish that `1a8bb63` had
already reached submission and failed. The score composition is retained, but
the next attempt first removes redundant sparse-image work in 0283.
