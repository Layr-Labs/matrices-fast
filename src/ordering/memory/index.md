...
- [0272: the cap's margin is not in the restore axis, the charge shape, or the width ceiling](evidence/0272-summary.txt) — `copy_stats` prices open lead 18 at **1.7-4.0 %** of a cap row (copies) / 2-5 % (whole axis); the allocator knob A/B is flat on nine rows; the nonzero-only row clear is a **wash** (+0.075/+0.122 s on two rows, not shipped); the free `K=12` ceiling (identical ratios) and the `K=10` price (**+415 u-dev** on nine rows) bound the width dial; the charge shape is the allowance in disguise (charge-75 % + ledger-90 % = +0.9 u-dev); every seam-addressable block is value-free *and* wall-free on the cap rows; tree re-verified 0/300 rows differ.

- [0274: the graded frame was wrong — the `.pat` extraction kept the diagonal](evidence/0274-true-frame-diagonal.txt) — `extract.py` copied the raw corpus arrays while the harness stages `pattern_from_jsonl_line` (diagonal dropped, `ssi-scoring/src/loader.rs:46` via `src/main.rs:269`), so every per-row census from iter65 on measured a graph with `n` self-loops; the true frame's max rows are `crudeoil_lee4_09`/`crudeoil_lee4_10`/`arki0013` at **1.20-1.23 s** (not 1.010 s), the corrected hidden bracket is **f in (1.53, 1.61]** with a survival line of 1.24-1.31 s, `12 → 20` sweeps costs 0 s above the `XCH_PLATEAU_MIN_N = 10 000` gate and +0.18 s on `chimera_selby-c16-01/02`, and the board promoted this lane's 12-sweep bat **0.840623** (transfer 4.3× on dev −3.7e-5).

- [0278: the grader's own wall ledger — the frontier is cap-limited, and the charge shape is retired](evidence/0278-grader-wall-ledger.txt) — 26 validations on 2026-09-13: **20 per-matrix TIME-cap kills**, 19 of them after 83-117 s of a 584-638 s run and one after 5.5 s; the day's cheapest full-corpus pass is a rival's 269 s tree scoring 0.925818 while ours take 633 s at 0.840623 (**2.35x wall for 10 % of score**); the promoted tree's three production deltas are the charge discount, `XCH_ALLOC=2` and the dense rung; the killed class is the 23 rows >= 1.10 s that hold 17.9 % of the wall. The charge shape's wall price lands on exactly those rows (ABBA probe A/B: worst row -0.03..-0.05 s, eight largest savings all hot rows, eight largest losses all <= 1.0 s) for 5e-6 dev of worker-frame value, so it is retired: bat **0.790258 / 0.923138** (300/300, 126 tests). [evidence/0278-bigk-retired.txt, evidence/0278-prexch-gate-census.txt]
