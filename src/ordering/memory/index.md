...
- Current shippable candidate is still the 0061 tip: local **0.843358**, hidden **0.86837** (`c7d5fe71`). 0062–0065, 0069, and 0070 failed the hidden 2 s cap. 0071 is leftover-384 on the pooling band with rounds 2–3 only. See [0071](experiments/0071-leftover-384-band-cheap-chain.md).
- 0071 leftover-384 on the pooling band + cheap chain: local **0.843329** (−0.29 bip), worst 1.349 s. See [0071](experiments/0071-leftover-384-band-cheap-chain.md).
- 0070 leftover-384 + full chain on the pooling band: local **0.843148**, hidden **FAILED** (`194629f`, 2 s cap). See [0070](experiments/0070-leftover-384-pooling-band.md).
- 0069 leftover-384 + rounds 2–3 on every medium miss: local **0.843328**, hidden **FAILED** (`c20b927`, 2 s cap). See [0069](experiments/0069-leftover-384-without-chain.md).
- 0065 leftover `max_s` 256→384 **with** chain unlock: local **0.843147**, hidden **FAILED** (`c16ff382`, 2 s cap). See [0065](experiments/0065-widen-existing-leftover-max-s.md).
- 0064 isolated second miss-retry: local **0.843148**, hidden **FAILED** (`71f76302`, 2 s cap). See [0064](experiments/0064-isolated-second-miss-retry.md).
- 0063 large-only 0.90 retraction: local **0.842991**, hidden **FAILED** (`2d954e54`, 2 s cap). See [0063](experiments/0063-drop-large-090-extra-relabel.md).
- 0062 wider leftover (0.90) + second miss-retry: local **0.842991**, hidden **FAILED** (`395313f1`, 2 s cap). See [0062](experiments/0062-wider-margin-and-second-miss-retry.md).
- Preceding 0061 margin-scaled leftover: local **0.843358**, hidden **0.86837** (promoted `c7d5fe71`). See [0061](experiments/0061-margin-scaled-leftover-search.md).
...
- [0071: leftover-384 band cheap chain](experiments/0071-leftover-384-band-cheap-chain.md) leftover-384 on the pooling band, rounds 2–3 only.
- [0070: leftover-384 pooling band](experiments/0070-leftover-384-pooling-band.md) leftover-384 + full chain on the band. Hidden **FAILED** (`194629f`).
- [0069: leftover-384 without chain](experiments/0069-leftover-384-without-chain.md) leftover-384 + rounds 2–3 on every medium miss. Hidden **FAILED** (`c20b927`).
- [0068: extra small well-below LNS](experiments/0068-extra-small-well-below-lns.md) two more 50M streams. Local no-op (`lt_1k` 0.890338). Reverted.
- [0067: terminal pass nnz 125k](experiments/0067-terminal-pass-nnz-125k.md) local no-op. `pooling_sppa9tp` unchanged.
- [0066: AMF dead-zone / relabelled AMF](experiments/0066-amf-dead-zone-well-below.md) local no-op on `gams05`. Not submitted.
- [0065: widen existing leftover `max_s`](experiments/0065-widen-existing-leftover-max-s.md) puts 384 on the 0061 leftover ticket and deletes the second refine. Hidden **FAILED** (`c16ff382`).
- [0064: isolated second miss-retry](experiments/0064-isolated-second-miss-retry.md) reverts well-below to 0.80 and keeps only the `n<10k` second first-round miss-retry. Local **0.843148** (−2.10 bip). Hidden **FAILED** (`71f76302`).
- [0063: drop large-graph 0.90 extra relabel](experiments/0063-drop-large-090-extra-relabel.md) keeps 0062's miss-retry and 0.90 LNS; large extra relabel stays at the hidden-proven 0.80 gate. Hidden **FAILED** (`2d954e54`).
- [0062: wider leftover margin + second miss-retry](experiments/0062-wider-margin-and-second-miss-retry.md) spends leftover tickets at ratio < 0.90 and retries a first-round miss twice on `n<10k`. Dev **0.842991** (−3.67 bip vs tip 0.843358). Hidden **FAILED** (`395313f1`). Isolated second miss-retry −2.10 bip.
- [0061: margin-scaled leftover search](experiments/0061-margin-scaled-leftover-search.md) spends extra first-round / LNS / relabel tickets only on well-below-anchor incumbents. Dev **0.843358** (−3.00 bip vs tip 0.843658). First-round `max_s` on a successful first round loses. Hidden **0.86837**.
