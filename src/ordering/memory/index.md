...
- [0101: terminal work ablation and lean schedule](experiments/0101-terminal-work-ablation-and-lean-schedule.md): full eight-arm/300-input control attributes gains to all three terminal families; one five-candidate TELOS round is byte-identical to the larger schedule on all 300 inputs, with 56% lower summed phase time. A 0.75M/two-round MCS allowance trades 0.2035 relative dev bips for headroom: **0.806332496822684**, 17 wins / 0 losses versus untouched ef9ec4c. MCS bucket reuse rejected after mixed matched results. Official local **300/300 pass, 81 active tests pass**; remote submission pending.

- [0100: linear ranked MCS with bounded continuation](experiments/0100-linear-ranked-mcs-and-bounded-continuation.md): local 0.806370063938585 -> **0.806316086866332**, 10 wins / 0 losses, 300/300 local pass; matched neighbor-preparation kernel 5.8-9.2x faster on three real completions. Not submitted: the predecessor's hidden failure remains unexplained and end-to-end added-work risk needs a smaller policy.
- [0099: component-wise donor mixing](experiments/0099-component-donor-mixing-negative.md): two correctness tests pass, but 20-row disconnected-component screen has zero changed flop counts; removed from production, reference retained under cfg(test).

- [0098: bounded structural terminal candidates and allocation reuse](experiments/0098-bounded-structural-terminal-portfolio.md): fresh ef9ec4c control **0.806787941749786** -> **0.806370063938585**; 17 wins / 0 losses / 283 unchanged, all 300 official local rows pass. STRIP + column-ranked TELOS + four terminal MCS tie variants; reusable MINL scratch and clone admission checks; inherited METIS seed 21 replaced by canonical seed 2. Extra metric donors dropped after a local cap failure and phase profiling. **d21ea6f6 FAILED hidden validation, no score or reason disclosed**; development and hidden scores are not interchangeable.
