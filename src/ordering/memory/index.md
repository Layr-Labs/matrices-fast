[0157: close the last `best_flops` leak, stage 13's alternate-seed chains](experiments/0157-stage13-best-flops-writeback.md):
one monotone `min` write-back. Dev **0.792573 -> 0.792573**, exact flops
identical on all 300 rows, and the `STALE_BEST_FLOPS` guard falls from 4 rows
to **0**. Every one of the 40 downstream `best_flops` reads is an acceptance
test, a `min`, or a pruning bound, so a lower value is work-monotone; the three
delta-ratchets after the stage become harder to arm.
