...
- [0085: stage-6 move sampling, metered by fill](experiments/0085-stage6-move-sampling-metered-by-fill.md): dev 0.826784 -> 0.826637, 4 wins / 0 losses, and the first `lt_1k` movement in five promotions. Up to 32 independent xorshift stream pairs at the small-graph polish, best-of, with the set count solved from a 150M word-operation budget because a `SmallScore` pass costs `words*(n+fill)` and `fill` is bounded only by `n(n+1)/2` - an `(n, nnz)` gate is NOT an absolute-constant bound for it. Also: alternate-seed pool 4 -> 8 under the unchanged 4M ledger (16 adds nothing, 32 costs the tail rule's whole tolerance), pre-symbolic entry refusal on the above-gate PEO branch, and two redundant `is_bijection` passes. Official pending.

- [0086: cross-candidate subtree transplant](experiments/0086-cross-candidate-subtree-transplant.md): on tip `996e8d6`: dev 0.826558 -> **0.825648** (9.10 bips), 300 rows OK, 68 tests, and a corpus maximum **59.2 ms BELOW** that tip's (1.4568 -> 1.3976 s); on the previous tip `4d86414` the identical change measured 0.826784 -> 0.825411 (13.73 bips). Postorder the incumbent's elimination tree, then for each block width in [4096, 512, 128, 32] replace every maximal disjoint subtree's internal order by the order each RETAINED `consider` ordering induces on exactly that subtree's vertices, accepting each block independently on a strict decrease of its own exact contribution. Disjoint subtrees contribute independently to `sum c^2`, so ONE exact score prices every block at once and `k` donors cost `k` scores. Terminal placement, so nothing downstream reads it. Every score is charged `n + nnz` against a 600_000-unit ledger, and the scorer is `permute + etree + column_counts_gnp` - linear in the INPUT and independent of the factor, so unlike a `SmallScore` pass the gate really is a work bound; 600k units is 12.7 ms here and 17.3 ms on the slowest host reported, and `4(n + nnz) <= 600_000` refuses every row above `n + nnz = 150_000` outright. Unmetered ceiling is 87 rows and ~35.6 dev bips. Donor count and ledger trade against each other: at four donors the 600k ledger yields 11.29 bips and at eight only 3.03, because eight donors at the coarsest width exhaust it before the finer widths are reached - which is the whole 13.73 -> 9.10 difference between the two tips. `PEO_ALT_SEEDS = 8` is kept regardless because it is promoted. The declared 20.6 ns/unit rate is UNPINNED and optimistic by ~5x; pinned it is 40-106 ns/unit, so the real bound is 63.6 ms local / 86.7 ms slow-host. Official pending.
