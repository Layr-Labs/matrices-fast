...
- [0110: final subtree-refinement round](experiments/0110-final-subtree-refinement-round.md) — both `subtree_refine` call sites are at stage 3, so whatever a later stage installs ships unrefined; one appended round on the FINISHED incumbent at `n + nnz <= 400_000` is **−2.84 dev bips / ~55 movers / 0 rows worse** on `0fb513b`, and −3.03 / −2.89 on the two frontiers before it. Also: three `gt_10k` generator families closed (relabelled residual-core lottery 0/45, extra AMD+AMF lottery tickets 1/45, ungated METIS/Scotch 0/45).
