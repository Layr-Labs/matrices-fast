[0291: re-arm the shared-prefix basin fork](experiments/0291-basin-fork-rearm.md):
iter81's completion exonerates the fork on the cap — iter78/79/80 cap-failed at
84.1–84.7 s with the fork **absent**, and restoring the four `rgreedy.rs`
`Game` buffer-image behaviours is what made the Benchmark step complete. Restore
the fork, widen its structural fence `600 / 5 000` → **`1 000 / 10 000`** (two
more movers, priced row by row), hold the anchor margin at 20 %. Public
**0.790254 → 0.790161 (−9.3e-5 dev)**, 4 movers, **0 regressions**, all in
`lt_1k`; 300/300 `COUNTS`; determinism re-run byte-identical; release suite
127 passed / 0 failed / 56 ignored. Also carries the negative results:
`probe_eval_audit` still `leak_rows = 0`, and the five `gt_10k` anchor ties are
**optima**, not search failures.

[0290: the simplicial reduction is a null](experiments/0290-simplicial-reduction-negative.md):
a new exact `core_lift` rule — eliminate a vertex whose live neighbourhood is
already a clique, at **any** degree, verified under a charge-before-attempt
pair-check ledger — fires on most rows and shrinks cores by 20–98 %
(`squfl030-150` 13 680 → 180, `emfl050_5_5` 13 175 → 650), and **300/300 flop
records are bit-identical**: SCORE 0.790254 both ways. Widening `order_core`'s
small-core quality gate to `n <= 60 000` on top of it is also identical. A
smaller core is not a better ordering; the core-size axis is closed as a value
lever.

[0289: restore the promoted `Game` buffer lifecycle](experiments/0289-restore-promoted-buffer-lifecycle.md):
iter80 cap-failed as `65c2e9d` / PR #746 at 84.1 s, the fourth same-day endpoint in an 84.1–84.7 s
band. The promoted base and iter80 are identical in `MAX_N`, the sparse `Pristine` representation,
`new_sparse`, `XCH_ALLOC` and the ledger; the only functional divergence was four `rgreedy.rs`
buffer-image changes. Release the 1 GiB sparse image at the promoted 160 MB ceiling, give
`new_sparse` a fresh zeroed image, and drop the reset-first fast path. Public score unchanged at
0.790253862267, 300/300 rows. **Hidden verdict: `849643ab` / PR #747 COMPLETED the Benchmark step in
8 min 02 s — the first completion since iter72 — scoring 0.840510 vs the frontier 0.840511, rejected
by 1e-6.** The four reverted behaviours were the binding cap cause. A refuted `MAX_N` lead is
recorded on the same page.

[0288: restore direct suffix control flow](experiments/0288-restore-direct-suffix-control-flow.md):
iter79 removed both added search devices but cap-failed at the same 84.360-second position. Remove
the retired fork's global closure, scorer conversion, runner-up `Arc`, channel, and branch; restore
the promoted direct phase-4-through-terminal path with unchanged public score 0.790253862267.
