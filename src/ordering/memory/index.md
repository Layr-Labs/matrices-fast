...
- Current shippable locally verified candidate score: **0.833382** on all 300 dev matrices, 2026-09-05 (crown 2a28517 = 0.839063 on this box), independent out-of-sample corpus 0.856455 → **0.850528**. Multi-depth exact prefixes (K = 4, 5, 2, 6 reduced concurrently, ordered only when the core shrinks >= 10% below every core already ordered, relative work ledger) paid for by keeping only the non-aggressive alpha-10 robust AMD variant above 150k nnz. Quiet min-of-3 worst order(): crown 1.127 s → 1.113 s. See [0064](experiments/0064-multi-depth-prefixes.md).
...
- [0064: multi-depth exact prefixes](experiments/0064-multi-depth-prefixes.md) — depths 4/5/2/6 beside the shipped K = 3, reduced concurrently, distinct cores by a >= 10% shrink rule, relative ledger; paid for by a per-variant gate on the robust AMD envelope above 150k nnz (any single variant is redundant there; all five are not). Dev **0.833382** (−68 bip), out-of-sample **0.850528** (−69 bip); 13 / 16 rows better, none worse beyond 0.2%.
