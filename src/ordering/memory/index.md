[0166: budget-side levers — both NEGATIVE](experiments/0166-budget-levers-negative.md):
AMF relabel ceiling 200k→400k moves zero rows (band rows get 1 restart, no
win); gt_10k relabel 500k/36→800k/48 is +4.5e-4 worse via downstream basin
shift — 0007's shape is the optimum. The original well is dry; ride the held
0163 bundle + port rounds.

[0164: flow-cutter bisection — NEGATIVE](experiments/0164-flowcutter-bisection-negative.md)
and [0165: BRKGA random keys — NEGATIVE](experiments/0165-brkga-random-keys-negative.md):
both MatricesLit original-stream picks measured to receipts on the full
corpus — the bisection-ND order moved zero rows (0039's structural verdict,
different engine) and the key-driven memetic nets +7e-6 worse via downstream
basin shift (0004 extends to crossover). Modules + 5 unit tests kept as
instruments; both wirings reverted in-source.

[0163: round-4 port + tail exchange dose 1 — THE SUBMISSION BUNDLE](experiments/0163-round4-99de589-delta-and-tail-dose1.md):
dev 0.790135 -> **0.789841** (−2.94e-4; −1.98e-3 vs the graded base), 125
tests, official 300/300 ×2, worst 1.04 s / lee4_09 min-of-3 1.034 s. Content
parity with the 0.840623 frontier + the 0158 edge + the tail-exchange device
(their tree ships it off). Estimate 0.84042–0.84050 vs bar 0.840523. Held by
the refire daemon for the morning window.

[0162: kernelized reduce — measured NEGATIVE as an ordering variant](experiments/0162-kernelized-reduce-negative.md):
OSS simplicial/almost-simplicial fixpoint (the MatricesLit Rank-1 pick) moved
ZERO of 300 rows despite a real census signal — chordal collapses land on the
unwinnable AMD ties and the kernel prefix is min-degree-class. Instruments
(`reduce_kernel`, `probe_kernel_census`) kept; the open value path is
kernel-core-fed-exact-search (instance reduction).

[0161: overnight chain port (six promotions, final forms)](experiments/0161-overnight-chain-port.md):
dev 0.791243 -> **0.790135** (54/3; −1.69e-3 vs the graded base), 125 active
tests, official 300/300, quiet-machine timing worst 1.042 s / min-of-3
lee4_09 0.901 s. The XCHG_TAIL original device measures real value
(0.789948 at dose 1) but fails the harness cap on lee4_09 — reverted with
receipt; re-enable only with a work-priced gate. Candidate vs hidden frontier
0.840782 — NOT submitted, decision pending.
