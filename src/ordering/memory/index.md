...
- [0076: core-LNS on the K = 3 residual core](experiments/0076-core-lns-on-the-residual-core.md): frontier `a07cc9c` (hidden 0.861203) measures **0.831772** on this box; current candidate (time-neutral, small rows only) **0.831500** (−3.3 bips), lt_1k 0.889730 → 0.888824, other buckets identical. Two chained 30M-op exact LNS streams on the residual core (8 ≤ cn ≤ 1000, core_nnz ≤ 60k) of n ≤ 1000 rows, paid by trimming 60M ops from the row's full-graph LNS tail; 7 better / 1 worse; mean row −0.5 ms. Earlier shapes: additive (−6.1 bips) failed the hidden cap (`f118a2c5`); time-neutral on small + medium rows (−3.8 bips) passed the cap but lost 1.2 bips on hidden (`35f129c9`). Official result pending.
- [0075: exact MinFill on residual cores](experiments/0075-residual-core-minfill.md): current local candidate **0.832118** versus the recorded `0.832286` base; the `1k_10k` bucket moves from `0.865327` to `0.863646`, fill from `0.938912` to `0.938869`. The residual-core probe finds 7 strict movers; the other relabel probes find none. Full trusted 300-matrix run and 66 active tests pass. Official result pending.
...
- [`../probe.rs`](../probe.rs) — TEST-ONLY measurement module (`#[cfg(test)]`,
  ...
  - `probe_core_families` / `probe_core_lns_design` — core-based candidate families measured as
    ...
    drop-1); `probe_core_lns_budget_low`, `probe_core_lns_next`, `probe_core_lns_unpaid` — budget,
    next-lever and unpaid-row pricing of the core walk; `probe_tie_floor` — lower bound
    `n + 3|E| + 2·triangles` vs AMD on every exact tie (certifies only the 16 fill-free ties).
...
- [0076](experiments/0076-core-lns-on-the-residual-core.md) — exact randomized LNS on the K = 3 residual core inside `refine_core` (2 × 30M ops, cheap cores only) + 2/1 polishing margin. small-rows-only time-neutral shape 0.831772 → **0.831500** (−3.3 bips), 7 / 1 / 292, no row does more exact-search work; the additive shape (−6.1 bips) FAILED the hidden cap (`f118a2c5`), the small+medium shape (−3.8 bips) was rejected on hidden score (`35f129c9`). Hidden pending. Negative controls in the same page: relabelled core MinFill, K = 5/4/2/6 ticket portfolio, any core family on n ≥ 10k.
