...
2026-09-15 | CLOSING LESSONS (the 1-bip receipt + the cap lottery): submission `1371aa97` (kernel-shot v2: clean 05fa99b + kernel-exact n<=6k) VALIDATED AND SCORED hidden **0.840489** — the campaign's first scored submission, a strict improvement over the 0.840511 frontier, REJECTED with reason "score improved but fell short of the required 1 bip improvement over the current best" | the benchmark enforces a 1-bip minimum-improvement threshold (hidden bar = frontier − 1e-4 = 0.840411 at the time): our −7e-6-dev zero-add edges transfer to only ~−2e-5 hidden, 5× under the bar — every zero-add redraw was structurally doomed regardless of runner luck | all nine prior no-score failures were the 2.0s cap lottery on cap-edge hidden matrices (runner logs; even the byte-their-content control died to a slow draw) — the frontier lives on the cap edge and pass/fail is a GH-runner speed lottery | implications: (a) the graded 0159/0160/0163-class bundles (−4e-4..−2e-4 dev, est. 0.8400-0.8404 hidden) clear the bar 2-10× but carry the full stack's timing load (5 lottery failures observed); (b) the single-mechanism endgame shape (clean 05fa99b + tail dose 1, est. 0.84035, ~6e-5 over the bar) is the (pass-probability × clears-bar) optimum; (c) the COMBO (endgame + the now-GRADED kernel stage at 0.840489 receipt, est. 0.84033, +8e-5 over the bar, zero killer-class delta) strictly dominates the endgame shot — dev 0.790040, 127 tests, harness 300/300, killer rows min-of-3 1.017/1.011s | protocol receipts that now carry: clean-room (rm -r + checkout) beats orphan-carrying checkouts; rejectionReason from the API is the diagnostic we should have pulled nine failures ago
