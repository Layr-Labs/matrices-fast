...
- [x] **RESOLVED (negative, hidden timeout) — Widen existing leftover miss-retry `max_s` and then unlock the chain.**
      ...
      the subtree chain on a new hidden matrix.
- [x] **RESOLVED (negative, hidden timeout) — Leftover `max_s=384` + rounds 2–3 on every medium miss.**
      Answered by [0069](experiments/0069-leftover-384-without-chain.md):
      Submission `c20b927` failed the 2 s cap. Skip-all-chain is a local loss
      (0.843421). Medium-wide leftover-384 is closed. See [0070](experiments/0070-leftover-384-pooling-band.md).
- [x] **RESOLVED (negative, hidden timeout) — Leftover `max_s=384` + full chain on the pooling band.**
      Answered by [0070](experiments/0070-leftover-384-pooling-band.md):
      Submission `194629f` failed the 2 s cap. The hidden occupant is in
      `4000<=n<8000` / `100k<=nnz<=150k`. See [0071](experiments/0071-leftover-384-band-cheap-chain.md).
- [ ] **IN FLIGHT — Leftover `max_s=384` on the pooling band with rounds 2–3 only.**
      [0071](experiments/0071-leftover-384-band-cheap-chain.md) is the last
      leftover-384 cell: band ∩ cheap chain.
