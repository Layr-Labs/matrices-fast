...
pointed at the core; two chained 30M streams there are worth −6.1 bips on dev at ≤ 0.04 s per row (−3.8 bips once paid for by trimming the same ops from the row's full-graph LNS — the additive shape failed the hidden cap), with
