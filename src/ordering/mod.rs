...
/// Independent stream pairs run at stage 6, best-of. Set 0 is the shipped pass;
/// the rest are `splitmix64` derivatives of the same two constants. Swept
/// post-hoc at 1 / 2 / 4 / 8 / 32 sets over the 92 in-gate dev rows: 0 / 1 / 3 /
/// 4 / 4 winning rows and -0.00 / -0.04 / -0.09 / -0.26 / -0.31 dev bips, at
/// 2.1 ms per set per row.
const CUTOFF_STREAM_SETS: usize = 32;
/// Word-operation budget for the stage-6 sets beyond the shipped one. Measured
/// over the 92 in-gate dev rows, one word operation costs 0.54-1.04 ns on this
/// host (median 0.88), so 150M units is 130-160 ms here and ~300 ms on the
/// slowest host on record. The band's own maximum row is 0.318 s against a
/// 1.36 s corpus maximum, so the budget cannot reach the tail.
...
fn leader_order_pool(pattern: &Pattern, alt_pool: usize) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    if n >= 12 && n <= 300 && pattern.nnz() <= 3_000 {
        // Stage 6 samples 512 paired-swap position tuples and 1024 neutral-walk
        // tuples from two hardcoded xorshift streams. `cutoff_differential` shows
        // that a SECOND application of those same streams wins on 0 of 92 in-gate
        // dev rows, which is local optimality with respect to 1536 fixed tuples
        // and says nothing about any other tuple. Because the iteration count IS
        // the stream length, "more iterations" and "another seed" are the same
        // experiment, so the lever is simply more sampled moves.
        //
        // Stream set 0 is `(CUTOFF_PAIRED_SEED, CUTOFF_PLATEAU_SEED)` started from
        // the incoming permutation, i.e. bit-for-bit the shipped pass; each later
        // set restarts from the current best under the same `SmallScore` strict-`<`
        // admission, so no row can regress and the result is monotone in
        // `CUTOFF_STREAM_SETS`.
        //
        // Measured over the 92 in-gate dev rows: 4 winning rows, -0.86 dev bips
        // through the `lt_1k` bucket, 5.2 s corpus-wide and a 152 ms maximum,
        // against the band's own 0.93 s of slack below the corpus maximum.
        let scoring = SmallScore::new(pattern);
        let (mut best_f, fill) = scoring.flops_and_fill(&best_perm);
        // METER THE SETS, DO NOT COUNT THEM. The gate caps `n` and `nnz`, but a
        // scoring pass costs `words * (n + fill)` word operations and `fill` is
        // bounded only by `n(n+1)/2`, which no input gate can lower: at `n = 300` a
        // sparse pattern that fills in completely is ~30x the corpus's densest
        // in-gate row, and 32 unmetered sets on it would run for seconds. So the
        // set count is solved from an explicit op budget, and set 0 - the shipped
        // pass - always runs, which makes the floor the incumbent's own cost and
        // the added work at most `CUTOFF_STREAM_UNITS` word operations.
        let words = (n + 63) / 64;
        let per_set = (words as u64)
            .saturating_mul(n as u64 + fill)
            .saturating_mul((CUTOFF_PAIRED_DRAWS + CUTOFF_PLATEAU_DRAWS) as u64)
            .max(1);
        ...
        for set in 0..sets {
            let (paired, plateau) = if set == 0 {
                (CUTOFF_PAIRED_SEED, CUTOFF_PLATEAU_SEED)
            } else {
                // A zero xorshift state is absorbing, so it is never handed out.
                (splitmix64(&mut state) | 1, splitmix64(&mut state) | 1)
            };
            let candidate = cutoff_plateau_stream(
                &scoring,
                cutoff_paired_swap_stream(
                    &scoring, best_perm.clone(), paired, CUTOFF_PAIRED_DRAWS,
                ),
                true, plateau, CUTOFF_PLATEAU_DRAWS,
            );
            let f = scoring.flops(&candidate);
            if f < best_f { best_f = f; best_perm = candidate; }
        }
    }
