...
/// A fill-free elimination completion attains the minimum possible flop count.
/// For any elimination order and its chordal completion H, the exact identity is
/// flops(H) = n + 3 * |E(H)| + 2 * triangles(H): each edge is counted at its
/// earlier endpoint, and each triangle at its first eliminated vertex. Since H
/// contains the input graph G, both terms are at least those of G. Exact column
/// counts summing to n + |E(G)| certify H = G and attain this lower bound.
///
/// Counts must come from the trusted symbolic analysis of this same pattern.
/// Lifted cores are simple symmetric graphs with sorted rows and no diagonal.
/// Check those assumptions before certifying; malformed or noncanonical input
/// safely declines the shortcut. Only a possible certificate pays the edge scan.
fn fill_free_core_optimum(pattern: &ScoringPattern, counts: &[u32]) -> bool {
    let n = pattern.n;
    let nnz = pattern.row_idx.len();
    if n.checked_add(1) != Some(pattern.col_ptr.len()) || counts.len() != n || nnz % 2 != 0 {
        return false;
    }
    let factor_nnz: u128 = counts.iter().map(|&c| c as u128).sum();
    if factor_nnz != n as u128 + (nnz / 2) as u128 {
        return false;
    }
    if pattern.col_ptr.first().copied() != Some(0)
        || pattern.col_ptr.last().copied() != Some(nnz)
        || pattern.col_ptr.windows(2).any(|p| p[0] > p[1])
    {
        return false;
    }
    for v in 0..n {
        let row = &pattern.row_idx[pattern.col_ptr[v]..pattern.col_ptr[v + 1]];
        if row.iter().any(|&u| u >= n || u == v)
    ...
    for v in 0..n {
        for &u in &pattern.row_idx[pattern.col_ptr[v]..pattern.col_ptr[v + 1]] {
            if pattern.row_idx[pattern.col_ptr[u]..pattern.col_ptr[u + 1]]
                .binary_search(&v).is_err()
            {
                return false;
            }
        }
...
fn refine_core(
    ...
    if fill_free_core_optimum(core_pat, &counts) {
        return None;
    }
...
/// Retain the sorted, score-unique best seed ledger. An existing score keeps
/// its earlier representative, matching stable append/sort/dedup/truncate.
/// Check admission before cloning a potentially very large permutation.
fn retain_runner_up(ledger: &mut Vec<(u64, Vec<usize>)>, score: u64, perm: &[usize]) {
    let position = match ledger.binary_search_by_key(&score, |(value, _)| *value) {
        Ok(_) => return,
        Err(position) => position,
    };
    if position >= PEO_ALT_SEEDS {
        return;
    }
    // Drop the displaced tail first so a full ledger need not grow its
    // allocation just to truncate that same tail immediately afterward.
    if ledger.len() == PEO_ALT_SEEDS {
        ledger.pop();
    }
    ledger.insert(position, (score, perm.to_vec()));
}
...
fn flush_batch<'a>(
    ...
    for r in results {
        ...
        probe::alt_lineage::note_consider(f, &perm, *best_flops, &best_perm[..]);
        {
            ...
            if f < *best_flops {
                retain_runner_up(&mut r, *best_flops, best_perm);
            } else {
                retain_runner_up(&mut r, f, &perm);
            }
        }
        if f < *best_flops {
            *best_flops = f;
...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    // One arena and one exact last-permutation cache serve this fixed pattern.
    let score_workspace = std::cell::RefCell::new(scoring_ws::FixedPatternScorer::new(&scoring_pat));
    let score = |p: &[usize]| {
        let f = score_workspace.borrow_mut().flops(p);
        #[cfg(test)]
    ...
    // A later phase may have reached the same certified optimum as a
    // fill-free ordering; no remaining strict refinement can improve it.
    if score_workspace.borrow().certifies_minimum(best_flops, original_edges) {
        return best_perm;
    }
    #[cfg(test)]
    ...
    if (SUBTREE_MIN_N..=SUBTREE_CHAIN_MAX_N).contains(&n) && nnz <= 1_500_000 {
        ...
        let parent: Vec<i32> = post_etree
            ...
            .map(|p| p.map_or(-1, |j| j as i32))
            .collect();
        // Share 512M nominal across rounds 4/5, reserving the 25% ceiling
        // before dispatch and refunding unused work after each joined round.
        let mut late_subtree_allowance = 640_000_000i64;
                    ...
                    if f2 < best_flops {
                        ...
                        if improved3 > 0 && is_bijection(&candidate3, n) {
                            ...
                            if f3 < best_flops {
                                ...
                                let improved4 = rgreedy::subtree_refine_limited(
                                     n,
                                     &pattern.col_ptr,
                                     &pattern.row_idx,
                                     &mut candidate4,
                                ...
                                if improved4 > 0 && is_bijection(&candidate4, n) {
                                    ...
                                    if f4 < best_flops {
                                        ...
                                        if n < 100_000 || best_flops != amd_flops {
                                            ...
                                            let improved5 = rgreedy::subtree_refine_limited(
                                                n,
                                                &pattern.col_ptr,
                                                &pattern.row_idx,
                                                &mut candidate5,
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    if n >= 6 && n <= rgreedy::MAX_N && nnz <= 200_000 {
        for (width, budget) in [(8, 16_000_000), (12, 32_000_000)] {
    ...
    if n >= 6 && n <= rgreedy::MAX_N && nnz <= 200_000
        && rgreedy::neutral_change_can_fit(n, nnz, 8, 2_000_000)
    {
    ...
    #[cfg(test)]
    let _tph = std::time::Instant::now();
    // A second, differently grouped rolling trial starts only after the raw
    // rolling pass has finished, preserving every earlier accepted gain.
    ...
    if n >= 6 && n <= rgreedy::MAX_N && nnz <= 200_000
        && rgreedy::neutral_change_can_fit(n, nnz, 8, 16_000_000)
    {
        ...
        if post_seed != best_perm && score(&post_seed) == best_flops {
            if let Some(candidate) = rgreedy::subset_window_descent_neutral_rolling(
                n, &pattern.col_ptr, &pattern.row_idx, &post_seed, 8, 3, 3, 16_000_000,
            ) {
                let candidate_flops = score(&candidate);
                if candidate_flops < best_flops {
                    best_flops = candidate_flops;
...
mod tests {
    use super::*;
    #[test]
    fn fill_free_core_certificate_attains_exhaustive_minimum_flops() {
        let mut certificates = 0;
        let mut graphs_without_certificate = 0;
        for n in 1..=5 {
            ...
            for graph in 0usize..1usize << edges.len() {
                ...
                graphs_without_certificate += usize::from(certified_scores.is_empty());
                for score in certified_scores {
                    assert_eq!(score, minimum, "n={n} graph={graph}");
                    certificates += 1;
                }
            }
        }
        assert!(certificates > 1000);
        assert!(graphs_without_certificate > 0, "nonchordal inputs must not certify");
    }

    #[test]
    fn fill_free_core_certificate_declines_invalid_graph_conventions() {
        let cases = [
            // Diagonal entries, repeated neighbors, missing reverse edges,
            // unsorted canonical edges, odd nnz, an invalid vertex, bad CSC.
            (2, vec![0, 1, 2], vec![0, 1], vec![1, 2]),
            (3, vec![0, 2, 4, 4], vec![1, 1, 0, 0], vec![2, 2, 1]),
            (3, vec![0, 1, 1, 2], vec![1, 0], vec![2, 1, 1]),
            (3, vec![0, 2, 3, 4], vec![2, 1, 0, 0], vec![2, 2, 1]),
            (2, vec![0, 1, 1], vec![1], vec![1, 1]),
            (2, vec![0, 1, 2], vec![2, 0], vec![1, 2]),
            (2, vec![0, 3, 2], vec![1, 0], vec![1, 2]),
            (2, vec![1, 1, 2], vec![1, 0], vec![1, 2]),
            (2, vec![0, 1], vec![], vec![1, 1]),
        ];
        for (n, col_ptr, row_idx, counts) in cases {
            let sp = ScoringPattern { n, col_ptr, row_idx };
            assert!(!fill_free_core_optimum(&sp, &counts));
        }
        let sp = ScoringPattern { n: 2, col_ptr: vec![0, 1, 2], row_idx: vec![1, 0] };
        assert!(!fill_free_core_optimum(&sp, &[1]));
        assert!(!fill_free_core_optimum(&sp, &[2, 2]));
    }

    #[test]
    fn fill_free_core_refinement_has_no_strict_improvement() {
        for (n, edges, perm) in [
            (5, vec![(0, 1), (1, 2), (2, 3), (3, 4)], vec![0, 1, 2, 3, 4]),
            (5, vec![(0, 1), (0, 2), (0, 3), (0, 4)], vec![1, 2, 3, 4, 0]),
            (6, vec![(0, 1), (0, 2), (1, 2), (3, 4)], vec![0, 1, 2, 3, 4, 5]),
        ] {
            ...
            let counts: Vec<u32> = column_counts_gnp(&pp, &et)
                .into_iter().map(|c| c as u32).collect();
            assert!(fill_free_core_optimum(&sp, &counts));
            let flops = flops_of(&sp, &perm);
            assert!(refine_core(n, &sp.col_ptr, &sp.row_idx, &sp, &perm, flops, flops).is_none());
        }
    }

    #[test]
    fn runner_up_admission_matches_stable_ledger_replay() {
        fn check(scores: &[u64]) {
            let mut actual = Vec::new();
            let mut expected = Vec::new();
            let mut actual_best = (u64::MAX, vec![0usize]);
            let mut expected_best = actual_best.clone();
            for (step, &score) in scores.iter().enumerate() {
                // Distinct payloads reveal which stable tie representative
                // survives, independently of any graph or ordering heuristic.
                let candidate = vec![step + 1];
                if score < expected_best.0 {
                    expected.push(expected_best.clone());
                } else {
                    expected.push((score, candidate.clone()));
                }
                expected.sort_by_key(|(value, _)| *value);
                expected.dedup_by_key(|(value, _)| *value);
                expected.truncate(PEO_ALT_SEEDS);
                if score < expected_best.0 {
                    expected_best = (score, candidate.clone());
                }

                if score < actual_best.0 {
                    retain_runner_up(&mut actual, actual_best.0, &actual_best.1);
                    actual_best = (score, candidate);
                } else {
                    retain_runner_up(&mut actual, score, &candidate);
                }
                assert_eq!(actual, expected, "ledger at step {step}, score {score}");
                assert_eq!(actual_best, expected_best, "incumbent at step {step}");
            }
        }

        // The first tie stores a MAX-scoring candidate before a subsequent
        // win displaces the initial MAX incumbent. The earlier candidate
        // must survive that collision. Later events fill/evict the ledger,
        // tie its cutoff, and collide with newly displaced incumbents.
        check(&[
            u64::MAX, u64::MAX - 1, u64::MAX - 1, 100, 105, 104, 103, 102,
            101, 110, 109, 108, 107, 106, 108, 101, 100, 99, 98, 99, 0, 0,
            u64::MAX,
        ]);
        for code in 0..256usize {
            let alphabet = [0, 1, 2, u64::MAX];
            let scores: Vec<_> = (0..4).map(|shift| alphabet[(code >> (2 * shift)) & 3]).collect();
            check(&scores);
        }
        ...
        for step in 0..1024 {
            ...
            state ^= state >> 7;
            state ^= state << 17;
            scores.push(if step % 17 == 0 { u64::MAX } else { state % 257 });
        }
        check(&scores);
    }
