...
///
/// `SSI_INDEP_NO_FORCE` (test-only; the production arm is `false`, so the
/// shipped ordering is unchanged by this seam) prices the generalisation of
/// that move: defer the lift on every row, i.e. always let the lift clear the
/// `INDEP_IMMEDIATE_MARGIN` lead instead of being installed for `n` alone.
#[cfg(test)]
fn indep_force_off() -> bool {
    std::env::var("SSI_INDEP_NO_FORCE").is_ok()
}
