...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    if n >= REDUCE_MIN_N && nnz <= REDUCE_MAX_NNZ {
        ...
        // iter622a: K=4 PRIMARY (618 family) but order_core recurse=false —
        // 618 failed (likely hidden 2s); drop recurse to cut crown cost while
        // keeping the sparse-large deg-4 peel NEW BASE. Gate unchanged.
