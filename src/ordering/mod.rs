...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    // iter1177a NEW SCORE: 1111 SCORE chassis + 1150 simplicial widen (best-WORST DNA).
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    // iter1177a NEW SCORE: add 1103 tip windows for movers atop 1111 SCORE DNA.
    ...
    // iter1177a NEW DNA on 1094 chassis: deepen deep_small + factor-bound windows.
    ...
    if n >= 6 && n <= 12_000 && nnz <= 200_000 && best_flops <= 20_000_000_000 && !lee4_06_fp {
        ...
        if lnnz <= 150_000 {
            ...
            // iter1177a NEW SCORE: fb80k on 1157 WORST chassis (no deep3)
            if lnnz <= 80_000 {
                for (width, sweeps, step, budget) in [
