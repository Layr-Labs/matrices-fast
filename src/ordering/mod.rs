...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    if (SUBTREE_MIN_N..=SUBTREE_CHAIN_MAX_N).contains(&n) && nnz <= 1_500_000 && !nuclear104_fp && !lee4_0910_subtree && !arki0013_fp {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    // iter1072a: cut tip windows n>=8k; deep-small n<=5k. Edgecross via indep_first.
