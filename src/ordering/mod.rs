...
/// Upper `n` of the band whose exact exchange runs the WIDER 14/5/14 cycle
/// (see the width-band note at the exchange site). Above it the ledger
/// truncates a 14-cycle below the complete 12-cycle it replaces.
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    if terminal_exchange {
        ...
        let exchange_width: usize = std::env::var("SSI_EXCHANGE_WIDTH")
            .ok()
            .and_then(|v| v.trim().parse().ok())
            .unwrap_or(if n <= EXCHANGE_WIDE_MAX_N { 14 } else { 12 });
        #[cfg(not(test))]
        let exchange_width: usize = if n <= EXCHANGE_WIDE_MAX_N { 14 } else { 12 };
        ...
        let exchange_sweeps: usize = std::env::var("SSI_EXCHANGE_SWEEPS")
            ...
            .unwrap_or(if n <= EXCHANGE_WIDE_MAX_N { 14 } else { 12 });
        #[cfg(not(test))]
        let exchange_sweeps: usize = if n <= EXCHANGE_WIDE_MAX_N { 14 } else { 12 };
        #[cfg(test)]
