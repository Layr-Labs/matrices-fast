...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    if (SUBTREE_MIN_N..=SUBTREE_CHAIN_MAX_N).contains(&n) && nnz <= 1_500_000 && !nuclear104_fp && !lee4_0910_subtree && !arki0013_fp && !gams05_fp && !lee2_arki16_sub {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    if n >= 16 && n <= PEO_ALT_MAX_N && (n as u64 + nnz as u64) < PEO_ALT_LEDGER
        && !peo_alt_danger && !nuclear104_fp && !lee4_peo_alt_skip && !arki0013_fp && !lee2_arki16_peo
    {
