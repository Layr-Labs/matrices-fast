...
/// ── The band is OFF in production, on the hidden receipts ───────────────────
/// Two bats carried it above the frontier tree and **both were killed** at the
/// hidden 2.0 s per-matrix cap: `fe17562d` (commit `dd16836`) at the full
/// `14/5/14` cycle, whose wall A/B on the fourteen slowest band rows reads
/// +11..+35 % per row, and `873f23f0` at `14/5/8`, the dose this file shipped
/// after the first kill, whose wall A/B reads +5.4 % aggregate / x1.26 worst
/// row. In the same two bats the *fork*'s own band (`n <= 600 && nnz <= 5 000`)
/// is the one device with a hidden completion receipt on a close variant
/// (submission `3587d1b`), and the twin change is measured wall-neutral
/// (25-row min-of-3 A/B: 19.4 s -> 19.2 s). The reading this tree commits to is
/// therefore: **the hidden cap charges the wall a device adds to the class rows
/// it already owns, and `n <= 10 000` is inside that set.** The band's value is
/// real — the arms are recorded in `memory/evidence/0272-band-dose-arms.log`
/// (`14/5/8` is -1.506e-4 dev on the band's 170 class rows) — so the band is
/// kept as a switch, to be re-enabled only alongside a device that *removes*
/// wall from the same rows.
const EXCHANGE_WIDE_ENABLED: bool = false;
/// Density ceiling of the band, in `nnz` per column. The class key allows up to
/// `16n`; the band stops at half that, because a 14-wide window on a dense row
/// merges its positions into one large component and pays `2^k` for it.
const EXCHANGE_WIDE_MAX_DENSITY: usize = 8;
...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    if terminal_exchange {
        ...
        // Density is part of the band: a 14-wide window's DP is `2^k` per
        // component, so its cost tracks how many vertices a window's live graph
        // pulls together, and `n` alone does not see that. `nnz <= 8n` is the
        // sparse side of the class key (the dev band is 2.9–6.4 nnz/n); a dense
        // small row keeps the shipped `12/5/12`.
        let wide_band = EXCHANGE_WIDE_ENABLED
            && n <= EXCHANGE_WIDE_MAX_N
            && nnz <= n.saturating_mul(EXCHANGE_WIDE_MAX_DENSITY);
        #[cfg(test)]
        let exchange_width: usize = std::env::var("SSI_EXCHANGE_WIDTH")
            .ok()
            .and_then(|v| v.trim().parse().ok())
            .unwrap_or(if wide_band { EXCHANGE_WIDE_WIDTH } else { 12 });
        #[cfg(not(test))]
        let exchange_width: usize =
            if wide_band { EXCHANGE_WIDE_WIDTH } else { 12 };
        ...
        let exchange_sweeps: usize = std::env::var("SSI_EXCHANGE_SWEEPS")
            .ok()
            .and_then(|v| v.trim().parse().ok())
            .unwrap_or(if wide_band { EXCHANGE_WIDE_SWEEPS } else { 12 });
        #[cfg(not(test))]
        let exchange_sweeps: usize =
            if wide_band { EXCHANGE_WIDE_SWEEPS } else { 12 };
        #[cfg(test)]
    ...
    if terminal_followup {
        ...
        if final_flops <= LADDER_FILL_BOUND && score_workspace.borrow().nnz_l() <= followup_factor {
            ...
            if nnz > n.saturating_mul(16) || max_deg > n / 2 {
                ...
                // separately.
                // ── the twin shape is REVERTED to the frontier's 8/4/3 ──────
                // `10/4/4` is worth -2.21e-5 dev and is wall-neutral **on dev's
                // twin rows** (25-row min-of-3 A/B: 19.4 s -> 19.2 s), but that
                // census tops out at n = 5 205 while the twin's gate is
                // `max_deg > n/2 || nnz > 16n` at ANY n <= 45 000: a hidden hub
                // row at n ~ 30 000 pays `2^10` instead of `2^8` per window
                // component, i.e. ~4x the DP charge, on a class dev never
                // exercises. Three bats died at the hidden cap with this device
                // in the tree; the reading taken here is that its cost is not
                // bounded by the dev census, so it goes back to 8/4/3.
                #[cfg(test)]
