...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    // 0210c: last re-transplant cut — tighter second-refine gate than 0210b
    // (c401dd13 failed timing). Fire only when post-transplant stages strictly
    // improved AND donors non-empty AND n<12k AND nnz<80k.
