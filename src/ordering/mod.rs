...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    // iter1301a ORTHOGONAL: peo_extract skip popdynm200 (1299 WORST 1.27)
    let popdyn_peo_skip = (22_000..23_000).contains(&n) && (100_000..110_000).contains(&nnz);
    if n >= 16 && n <= 30_000 && nnz <= 180_000 && !popdyn_peo_skip {
        ...
        for _ in 0..8 {
            ...
            // iter1293a ORTHOGONAL: nuclear10a-only LNNZ on 1111 SCORE chassis (NOT 1250)
            let n10a = (17_000..18_000).contains(&n) && (160_000..170_000).contains(&nnz);
            let lnnz_cap = if n10a { 500_000usize } else { peo_extract::MAX_LNNZ };
            let max_lnnz = if lnnz > lnnz_cap as u64 {
    ...
    // iter1301a: peo_large skip dt3 + gams05 (1299 WORST) + popdyn if routed large
    } else if n >= 16 && nnz <= PEO_LARGE_MAX_NNZ && nnz < 1_200_000
        && !((30_000..31_500).contains(&n) && (145_000..160_000).contains(&nnz))
        && !((17_000..18_000).contains(&n) && (240_000..270_000).contains(&nnz))
        && !popdyn_peo_skip {
    ...
    // iter1111a NEW DNA on 1094 chassis: deepen deep_small + factor-bound windows.
