...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    const HUGE_CHAIN_ROUNDS: usize = 3;
    if n > SUBTREE_CHAIN_MAX_N && n <= HUGE_CHAIN_MAX_N && nnz <= HUGE_CHAIN_MAX_NNZ {
        for round in 0..HUGE_CHAIN_ROUNDS {
            ...
            // Break, do not continue: `budget` is PER BLOCK, so a row that
            // does not respond would otherwise pay 16 blocks x 32M x every
            // remaining round for nothing. Submission a5e3fd90 failed hidden
            // validation with no score at all, and unbounded work on an unseen
            // row in this band is the only mechanism in this change that can
            // do that. Stopping at the first non-improving round takes the
            // non-responder cost on the four dev rows outside the gate from
            // 0.17-0.67 s down to 0.03-0.10 s, and the in-gate worst from
            // 0.52 s to 0.29 s.
