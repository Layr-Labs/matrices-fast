...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    if n >= SUBTREE_MIN_N && n + nnz <= FINAL_REFINE_MAX_WORK {
        ...
        // iter147m: skip iter110 rebuild ONLY inside 146b envelope (n<4k nnz<=14k)
        // where tiny rebuild already paid; keep tip mid iter110 elsewhere.
        let skip_mid_110 = (1_000..4_000).contains(&n) && nnz > 0 && nnz <= 14_000;
        if cur_flops < best_flops_before_final && !skip_mid_110 {
