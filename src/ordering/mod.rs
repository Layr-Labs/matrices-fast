...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    // ── terminal exchange (linson007 fb35d11, graded 0.842526) ────────────
    ...
    // exactly under one shared preparation/DP/replay allowance (64M). Width 8,
    // 4 sweeps, offset step 3. Admission is a pure predicate on the supplied
    // pattern (6 <= n <= 12k, nnz <= 200k, nnz <= 16n, max col degree <= n/2)
    // which bounds bitset storage and excludes dense/hub shapes. Acceptance
    // re-scores BOTH sides exactly - never the scalar `best_flops` ledger,
    // which earlier phases can leave stale.
    ...
    // ── terminal followup (linson007 52affcb / submission 07f0e8a, hidden ───
    // 0.842377) — revisit the completion after the greedy search and its
    // exact windows: earlier PEO/watcher rounds saw a different incumbent.
    // Retain the entire winning prefix; every replacement below is a strict
    // exact decrease. The high-flop class keeps its existing searches
    // (ledger precheck: reject an expensive row before scoring again); the
    // fresh admission still requires the exact score and a 150k factor-nnz
    // bound. The dense/hub arm re-runs the 8/4/3 window on the rows the
    // exchange's structural gate excluded; then two PEO rounds on the current
    // incumbent; then wide sparse spans (48/19 and narrow 9/4, 8/3) whose
    // components solve exactly up to 14 vertices and larger components keep
    // their slots. The 0205 shape (late watcher + 48M/32M/64M allowances)
    // failed the hidden cap — the halved 16M/16M/32M shape below is the fix.
