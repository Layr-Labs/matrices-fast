//! Deterministic seed, residual and separator search with exact symbolic acceptance.
use crate::Pattern;

mod advancements;
mod completion;
mod decomposition;
mod joint;
mod ladder;
mod linegraph;
mod parallel;
mod patch;
mod promotions;
mod quality;
mod quotient;
mod recovery_regions;
mod recovery_separator;
mod rgreedy;
mod separator;

mod core_search;
mod graph_orderers;
mod independent;
mod local_search;
mod policy;
mod score;
mod seeds;
mod subtrees;

use feral::ordering::elimination_tree::EliminationTree;
use feral::sparse::csc::CscPattern as ScoringPattern;

use score::*;

fn map_indexed<T, R>(items: &[T], f: impl Fn(usize, &T) -> R) -> Vec<R> {
    items
        .iter()
        .enumerate()
        .map(|(i, item)| f(i, item))
        .collect()
}

struct Candidate {
    perm: Vec<usize>,
    cost: u64,
}

impl Candidate {
    fn consider(&mut self, scoring: &ScoringPattern, candidate: Option<Vec<usize>>) -> bool {
        let before = self.cost;
        accept_if_cheaper(
            candidate,
            scoring.n,
            scoring,
            &mut self.perm,
            &mut self.cost,
        );
        self.cost < before
    }
}

pub fn order(pattern: &Pattern) -> Vec<usize> {
    let n = pattern.n;
    if n == 0 {
        return Vec::new();
    }
    let col_ptr: Vec<i32> = pattern
        .col_ptr
        .iter()
        .map(|&v| i32::try_from(v).expect("column pointer exceeds i32"))
        .collect();
    let row_idx: Vec<i32> = pattern
        .row_idx
        .iter()
        .map(|&v| i32::try_from(v).expect("row index exceeds i32"))
        .collect();
    let core =
        feral_ordering_core::CscPattern::new(n, &col_ptr, &row_idx).expect("invalid pattern");
    let anchor = quotient::order(&core, 10.0, true, false, 3 * quotient::WORK);
    if anchor.is_err() {
        for alpha in [2.0, 0.0] {
            if let Ok(p) = quotient::order(&core, alpha, true, false, quotient::WORK / 2) {
                return p.into_iter().map(|v| v as usize).collect();
            }
        }
        return graph_orderers::rcm_order(&graph_orderers::Graph::new(pattern))
            .into_iter()
            .map(|v| v as usize)
            .collect();
    }
    let perm = anchor
        .unwrap()
        .into_iter()
        .map(|v| v as usize)
        .collect::<Vec<_>>();
    let scoring = ScoringPattern {
        n,
        col_ptr: pattern.col_ptr.clone(),
        row_idx: pattern.row_idx.clone(),
    };
    let (amd_cost, factor_entries) = cost_and_entries(&scoring, &perm);
    if factor_entries == n + pattern.nnz() / 2 {
        return perm;
    }
    let mut best = Candidate {
        perm,
        cost: amd_cost,
    };
    if factor_entries > 8_000_000 {
        best.consider(
            &scoring,
            quotient::order(&core, 10.0, false, false, quotient::WORK)
                .ok()
                .map(|p| p.into_iter().map(|v| v as usize).collect()),
        );
        return best.perm;
    }

    seeds::refine(pattern, &core, &scoring, &mut best);
    let quality_enabled = quality::admits(pattern, amd_cost);
    let donors = if quality_enabled {
        vec![best.perm.clone()]
    } else {
        Vec::new()
    };
    let residual = core_search::refine(pattern, &scoring, &mut best);
    let quality_seeds = quality_enabled.then_some(quality::Seeds {
        core: residual,
        donors,
    });
    local_search::refine(pattern, &scoring, &mut best, amd_cost);
    subtrees::refine(pattern, &scoring, &mut best, amd_cost);
    local_search::cleanup(pattern, &scoring, &mut best);
    let mut small_input_cost = None;
    if let Some(seeds) = quality_seeds {
        if n < 10_000 {
            if factor_entries <= 64 * n
                || best.cost.saturating_mul(20) >= amd_cost.saturating_mul(19)
            {
                advancements::finish(pattern, &scoring, &mut best);
            }
            small_input_cost = Some(best.cost);
            quality::refine_small(pattern, &scoring, &mut best);
        }
        if best.cost.saturating_mul(20) < amd_cost.saturating_mul(19) {
            quality::bounded(pattern, &scoring, &mut best, seeds);
        }
    } else {
        advancements::finish(pattern, &scoring, &mut best);
    }
    if small_input_cost.is_some_and(|cost| cost != best.cost) {
        quality::refine_small(pattern, &scoring, &mut best);
    }
    independent::refine(pattern, &scoring, &mut best);
    best.perm
}
