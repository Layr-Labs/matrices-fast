...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    // iter587a NEW BASE: Scotch shape grid n<=10k — NO KaHIP/pe2 (both CI-dead for chimera).
    if n <= 10_000 && nnz < 50_000 { // iter594a
        // iter588a: lean 4-tuple subset aimed at chimera (CI-safer than 8)
        // iter589a: 2-tuple leanest chimera hunt
        // iter591a: single Scotch tuple
        for (trials, amd_sw, imb, seed) in [
            (10u32, 50u32, 0.25f64, 0xF00Du64),
        ] {
