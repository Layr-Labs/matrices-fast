...
/// Alternate prefix depths (0062 follow-up): K=2/4 on medium matrices only, AMF grid only.
const REDUCE_ALT_MAX_N: usize = 8_000;
const REDUCE_ALT_MAX_NNZ: usize = 300_000;
...
pub fn order(pattern: &Pattern) -> Vec<usize> {
    ...
    if n >= REDUCE_MIN_N && nnz <= REDUCE_MAX_NNZ {
        let reduce_passes: &[(usize, bool)] = if n < REDUCE_ALT_MAX_N && nnz <= REDUCE_ALT_MAX_NNZ {
            &[(3, true), (4, false), (2, false)]
        } else {
            &[(3, true)]
        };
        for &(max_row_deg, do_core_relabel) in reduce_passes {
        ...
        if let Ok(Some(cl)) = lifted {
            ...
            if cn > 0 && cn < n && cl.core_nnz() <= REDUCE_MAX_CORE_NNZ {
                ...
                let core_restarts = if do_core_relabel {
                    relabel_restarts_tuned(
