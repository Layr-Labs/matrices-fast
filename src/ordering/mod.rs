...
/// Require the incumbent to be at least 20% below AMD before paying for a
/// second suffix. This is the largest whole-percent margin that retains the two
/// high-value fork movers at the pre-suffix checkpoint; 21% drops the c8
/// Chimera mover. `gancns` is deliberately retired: its 97-flop improvement
/// costs the largest remaining measured fork dose, while removing it changes
/// the full-corpus score by only +3.014e-6.
const SHARED_BASIN_FORK_MARGIN_PCT: u64 = 20;
/// Test builds may re-point the production margin through
/// `SSI_FORK_MARGIN_PCT` for paired measurements.
...
fn shared_basin_fork_band(n: usize, nnz: usize, best_flops: u64, amd_flops: u64) -> bool {
    ...
    // The margin gate remains a test seam, but production uses the measured
    // structural band. This fork is disjoint from sparse-pristine's large-row
    // expansion and exact merging cannot worsen the incumbent.
    #[cfg(test)]
    ...
    #[cfg(not(test))]
    let margin_pct: u64 = SHARED_BASIN_FORK_MARGIN_PCT;
    n >= 6
    ...
    if terminal_followup {
        ...
        if final_flops <= LADDER_FILL_BOUND && score_workspace.borrow().nnz_l() <= followup_factor {
            ...
            if nnz > n.saturating_mul(16) || max_deg > n / 2 {
                ...
                // separately. Two hidden cap failures later, the fourth sweep
                // is no longer justified: 10/3/4 retains improvements on all
                // three movers and scores 0.790166 on the full 300-row frame
                // (only +2.4e-5 versus 10/4/4), while cutting this exponential
                // site's sweep dose by 25%.
