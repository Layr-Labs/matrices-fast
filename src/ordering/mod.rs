...
pub fn order(pattern: &Pattern) -> Vec<usize> {
    ...
    if n < AMF_SWEEP_MAX_N && nnz < 130_000 {
        // α0.5 / α2.5: distinct dense-deferral thresholds the {1,2,5,16,−1}
        // set never samples. probe_amf_extra_alpha measured 2 unique wins
        // (crudeoil_pooling_dt2 0.8375→0.7353, chp_partload 0.8165→0.8105)
        // and +0.041 s extra-pass worst inside this same nnz cap — the
        // current order() worst already lives here and stays far under 2 s.
