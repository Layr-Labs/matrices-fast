...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    // 0210: terminal conditioned re-transplant. Fire only when post-transplant
    // stages strictly improved AND the phase-14 donor pool was non-empty.
    // Same tip ledger/widths/gates (transplant_probe.rs untouched).
    if best_flops < transplant_entry_flops && !donor_perms.is_empty() {
        if let Some(cand) = transplant_probe::refine_with_donors(
