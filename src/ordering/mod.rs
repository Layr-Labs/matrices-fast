...
/// iter1083a: fill-scale fence (1077 DNA + mid-hub indep structural)
...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    // iter1077a: extra AMF alphas on mid-sparse AMD ties (squfl/emfl/knp zone).
    // Cheap: n<9k nnz<40k — cannot touch lee4 crown. Distinct from tip {1,16,-1}.
    if (2_000..9_000).contains(&n) && nnz < 40_000 {
        for da in [0.5f64, 2.0, 4.0, 8.0] {
            let amf_a = feral_amf::AmfOptions { dense_alpha: da, ..Default::default() };
            consider_cached!(CandidateFamily::Amf, da, None, move || {
                feral_amf::amf_order_opts(&core, &amf_a).map(|(p, ..)| p)
            });
    ...
    if (SUBTREE_MIN_N..=SUBTREE_CHAIN_MAX_N).contains(&n) && nnz <= 1_500_000 && !nuclear104_fp && !lee4_0910_subtree && !arki0013_fp && !gams05_fp {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    if n >= 16 && n <= PEO_ALT_MAX_N && (n as u64 + nnz as u64) < PEO_ALT_LEDGER
        && !peo_alt_danger && !nuclear104_fp && !lee4_peo_alt_skip && !arki0013_fp
    {
    ...
    // iter1077a: cut tip windows n>=8k; deep-small n<=5k. Edgecross via indep_first.
    ...
    // iter1077a: widen deep-small to n<=6k / nnz<=70k for more 1k_10k movers
    // (still far below lee4_06 n=10.4k).
    let deep_small = n >= 8
        && n <= 6_000
        && nnz <= 70_000
        && nnz <= 12 * n
