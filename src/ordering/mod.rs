...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    if n <= window_n && nnz <= window_nnz {
        ...
        /// ── iter41: the fan-out returns at 1/4 the budget ────────────────────
        /// The iter40 "removal is free" A/B was a STALE-BINARY artifact: the
        /// record's `results.tsv:1789241463` row (0.791896 with the list
        /// emptied) is not reproducible — the official sandboxed harness on the
        /// committed tree reads **0.792166** (`0207-harness-tip-4cpu.log`,
        /// results.tsv:1789242561, score.json), and a single-variable probe A/B
        /// in the graded 4-vCPU frame (same binary, same seams, one session)
        /// prices the terminal round exactly:
        ///
        ///   budgets       SCORE      movers  max +wall (row)      corpus +wall
        ///   [] (committed) 0.792166      0    -                    0
        ///   4 x 5e8        0.792079     12    +0.365 (mpbp_07)     +6.1 s
        ///   4 x 1e9        0.792065     14    +0.684 (emfl100_3_3) +13.6 s
        ///   4 x 2e9        0.791896     14    +1.485 (emfl100_3_3) +23.9 s
        ///
        /// [0207-fanoutoff/5e8/1e9/2e9-probe-4cpu.log] The round's price is
        /// LINEAR in its budget and lands hardest on rows it cannot move at all
        /// (mpbp_07, emfl100_3_3, transswitch0300p, squfl020-150, squfl015-080persp
        /// and knp5-44 each pay +0.23..+0.37 s at 5e8 with zero flop gain), which
        /// is why the 2e9 shape died on the hidden cap. 5e8 is the best measured
        /// value-per-peak-second point; the 2e9 value is not worth 4x its price
        /// on the class that decides the cap.
        const SHIPPED_ENGINE_FANOUT: &[i64] = &[500_000_000];
