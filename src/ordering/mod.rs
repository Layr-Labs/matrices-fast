...
fn shared_basin_fork_band(n: usize, nnz: usize, best_flops: u64, amd_flops: u64) -> bool {
    ...
    // ── iter68: the margin gate is DISARMED ─────────────────────────────────
    // iter68 armed `best_flops * 100 < amd_flops * 90` here on a worker-frame
    // receipt showing the fork's cost and its dev value are disjoint (the six
    // dev movers sit at ratio <= 0.842; the two rows it loads hardest with zero
    // dev change are near-anchor rows). That tree completed the hidden corpus
    // at 0.840946 (+3.23e-4) — and the margin is the one change whose entire
    // justification is "the fork has no value on near-anchor rows", which the
    // hidden frame has now contradicted. The gate is kept only as a test seam so
    // a later session can re-price it; the shipped band is the structural one.
    #[cfg(test)]
