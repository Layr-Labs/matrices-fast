...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    // iter1071a: nuclear104-ONLY + lee4_09/10-ONLY subtree skip (nnz 90k..140k).
    // Broader nnz>=90k also hit gams05 (nnz=252k) and regresssed 0.5274->0.5295 in 1070.
    let nuclear104_fp = (38_000..41_000).contains(&n) && (240_000..280_000).contains(&nnz);
    let lee4_0910_subtree = (15_000..20_000).contains(&n) && (100_000..130_000).contains(&nnz);
    if (SUBTREE_MIN_N..=SUBTREE_CHAIN_MAX_N).contains(&n) && nnz <= 1_500_000 && !nuclear104_fp && !lee4_0910_subtree {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    // iter1071a: nuclear104 + lee4_09/10 peo_alt + lee4_06 peo_alt (tip phases:
    // alt ratio-flat ~0.14-0.16s on all three; lee4_06 is post-1069 worst).
    let nuclear104_fp = (38_000..41_000).contains(&n) && (240_000..280_000).contains(&nnz);
    ...
    if n >= 16 && n <= PEO_ALT_MAX_N && (n as u64 + nnz as u64) < PEO_ALT_LEDGER
        && !peo_alt_danger && !nuclear104_fp && !lee4_peo_alt_skip
    {
    ...
    // iter1071a: cut tip windows n>=8k; deep-small n<=5k. Edgecross via indep_first.
