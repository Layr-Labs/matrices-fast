...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    if n >= REDUCE_MIN_N && nnz <= REDUCE_MAX_NNZ {
        ...
        // iter625a NEW BASE: shape-gated K=4 primary — 618/622 FAILED (CI/hidden).
        // Broad gate hit nuclear104 (timing). Keep only mover shapes:
        //   sparse-large transswitch band OR mid-n with nnz/n in {<=5, >=8}
        // (excludes lee4 ~6.4 and nuclear104 n~39k). recurse=false for crown cost.
