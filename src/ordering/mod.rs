...
///
/// THE CAP IS A SANITY BOUND, NOT THE CONTROL. The control is the microsecond
/// budget: `restarts = budget / nnz` already bounds the family's spend on EVERY
/// pattern at `budget` microseconds of worst-case restart time, whatever the
/// structure (see [`RELABEL_BUDGET`] for the cost model). A flat low cap on top
/// of that budget is not a safety property — it is an inconsistency. It grants
/// the full budget to a matrix whose `nnz` sits just above `budget/cap`, and
/// denies the same budget to every SPARSER matrix, which is strictly cheaper per
/// restart. The sparse side of that line is the majority of any realistic corpus
/// and the side where a restart costs the least, so a flat cap systematically
/// spends the least search where search is cheapest.
///
/// The caps below are therefore raised to the point where the budget binds
/// almost everywhere, and only remain at all to bound the pathological case
/// `nnz -> 0` (an almost-empty pattern, where `budget/nnz` would otherwise be
/// unbounded and the per-restart cost is dominated by fixed overhead the nnz
/// model does not see).
#[inline]
...
pub fn order(pattern: &Pattern) -> Vec<usize> {
    ...
    // A relabelled minimum-fill ticket samples the numbering-sensitive tie
    // breaks of the otherwise deterministic min-fill heuristic. It is gated
    // identically to the base min-fill pass, and the AMD floor keeps score
    // monotone; the runtime cost is the only tradeoff.
    if n < MINFILL_MAX_N && nnz < MINFILL_MAX_NNZ {
        let q = relabel(n, 7919);
        let b = permute_pattern(&scoring_pat, &q);
        let rp = Pattern { n, col_ptr: b.col_ptr, row_idx: b.row_idx };
        consider(&|| {
            let p = minfill_order(&rp);
...
fn ndfm_order(pattern: &Pattern) -> Vec<i32> {
    ...
    // Scratch for the separator-minimization pass (see `min_cover_separator`).
    let mut pos_b: Vec<i32> = vec![-1i32; n];

    // Work budget for separator minimization, shared across the whole recursion
    // and charged per edge scanned; exhausting it degrades later subsets to the
    // previous smaller-boundary separator.
    let mut cover_budget: i64 = 24 * pattern.nnz() as i64 + 32 * n as i64;
    ...
    while let Some((nodes, lo, _hi)) = stack.pop() {
        ...
        // The two boundaries are the two sides of the edge cut, so each is a
        // valid vertex separator on its own — but only the TRIVIAL one. Take the
        // MINIMUM vertex cover of the crossing edges instead (König/Hopcroft–
        // Karp); it separates the same two subdomains and is never larger than
        // the smaller boundary. Fall back to that boundary when the cover pass
        // declines (empty side or exhausted budget) or does not actually shrink.
        let trivial = boundary_a.len().min(boundary_b.len());
        let cover = if cover_budget > 0 {
            min_cover_separator(&boundary_a, &boundary_b, &adj, &mut pos_b, &mut cover_budget)
        } else {
            None
        };
        let sep: Vec<usize> = match cover {
            Some(c) if !c.is_empty() && c.len() < trivial => c,
            _ => {
                if boundary_a.len() <= boundary_b.len() {
                    boundary_a
                } else {
                    boundary_b
                }
...
/// Minimum vertex separator extracted from a bipartite edge-cut boundary.
///
/// `side_a` and `side_b` are the two sides of an edge cut: every edge crossing
/// the current bisection has one endpoint in each list. Any VERTEX COVER of that
/// bipartite edge set is a vertex separator — deleting it leaves no crossing edge
/// — and by KÖNIG's theorem a minimum vertex cover of a bipartite graph has
/// exactly the size of a maximum matching, hence at most `min(|A|, |B|)`.
///
/// Taking a whole boundary side (or a whole BFS level) is only the TRIVIAL cover.
/// On mesh-like and KKT-like graphs the trivial cover is routinely 1.5–2× the
/// minimum one, and separator size is the dominant term in nested-dissection
/// fill: a separator is eliminated last and behaves as a near-clique, so its
/// contribution grows like `|S|^2` in fill and `|S|^3` in flops. Shrinking `|S|`
/// is therefore worth more than any tie-break or restart tuning, and unlike a
/// restart lottery it is a property of the GRAPH, not of the vertex numbering.
///
/// Maximum matching is Hopcroft–Karp (`O(E sqrt(V))`) over an index-compacted CSR
/// copy of the crossing edges; the cover is then read off the alternating-
/// reachable set `Z` as `(A \ Z) ∪ (B ∩ Z)`.
///
/// Deterministic: both sides are consumed in the caller's ascending-index order,
/// every scan is index-ordered, and the returned separator is sorted ascending.
///
/// Returns `None` — the caller then keeps its own separator — when either side is
/// empty or the shared work budget is exhausted. The result is a valid cover for
/// ANY matching (an uncovered edge would contradict the reachability of its
/// left endpoint), and callers additionally adopt it only when it is strictly
/// smaller than the separator they already have, so an early-stopped matching can
/// only cost a refinement, never correctness.
fn min_cover_separator(
    side_a: &[usize],
    side_b: &[usize],
    adj: &[Vec<usize>],
    pos_b: &mut [i32],
    ...
    for (j, &v) in side_b.iter().enumerate() {
        pos_b[v] = j as i32;
    }
    let cover = min_cover_inner(side_a, side_b, adj, pos_b, budget);
    for &v in side_b {
        pos_b[v] = -1;
    }
    cover
...
/// Body of [`min_cover_separator`], split out so the `pos_b` scratch is restored
/// on every exit path (including the budget bail-outs).
fn min_cover_inner(
    side_a: &[usize],
    side_b: &[usize],
    adj: &[Vec<usize>],
    ...
    // CSR of the crossing edges, left-indexed and compacted to 0..na / 0..nb.
    let mut ptr: Vec<u32> = Vec::with_capacity(na + 1);
    ptr.push(0);
    let mut edges: Vec<u32> = Vec::new();
    for &u in side_a {
        *budget -= adj[u].len() as i64;
        for &w in &adj[u] {
            let j = pos_b[w];
            if j >= 0 {
                edges.push(j as u32);
        ...
        ptr.push(edges.len() as u32);
    }
    if *budget < 0 {
        return None;
    }

    let mut mate_a: Vec<i32> = vec![-1; na];
    let mut mate_b: Vec<i32> = vec![-1; nb];
    let mut layer: Vec<u32> = vec![u32::MAX; na];
    let mut queue: Vec<usize> = Vec::with_capacity(na);
    // (left vertex, cursor into its CSR row) — an EXPLICIT stack, so a long
    // augmenting path cannot overflow the call stack.
    let mut stack: Vec<(u32, u32)> = Vec::new();

    // Every phase augments at least once, so `na` phases is a hard upper bound
    // (the matching can never exceed `na`); the loop is bounded rather than
    // `loop { }` so a logic error degrades to a fallback instead of hanging.
    for _phase in 0..na {
        // Phase 1: BFS layers over free left vertices.
        queue.clear();
        for i in 0..na {
            if mate_a[i] < 0 {
                layer[i] = 0;
            ...
            *budget -= (e - s) as i64;
            for &je in &edges[s..e] {
                ...
                if m < 0 {
                    ...
                    layer[m as usize] = layer[i] + 1;
                    queue.push(m as usize);
                }
            }
        }
        if !reaches_free {
            break; // no augmenting path: the matching is maximum
        }
        if *budget < 0 {
            return None;
        }

        // Phase 2: vertex-disjoint augmenting paths, restricted to the layers.
        for s0 in 0..na {
            if mate_a[s0] >= 0 || layer[s0] != 0 {
                continue;
            }
            stack.clear();
            stack.push((s0 as u32, ptr[s0]));
            while let Some(&(iv, cur)) = stack.last() {
                let i = iv as usize;
                if cur == ptr[i + 1] {
                    layer[i] = u32::MAX; // exhausted; never revisited this phase
                    stack.pop();
                    continue;
                }
                stack.last_mut().expect("stack non-empty").1 = cur + 1;
                *budget -= 1;
                let j = edges[cur as usize] as usize;
                let m = mate_b[j];
                if m < 0 {
                    // Augmenting path found: rewire every level at once. At each
                    // level the edge just taken is the one before the cursor.
                    for &(lv, lc) in stack.iter() {
                        let r = edges[(lc - 1) as usize] as usize;
                        mate_a[lv as usize] = r as i32;
                        mate_b[r] = lv as i32;
                    }
                    break;
                } else if layer[m as usize] == layer[i] + 1 {
                    stack.push((m as u32, ptr[m as usize]));
                }
            }
            if *budget < 0 {
                return None;
    ...
    // König: Z = the set reachable from free left vertices by alternating paths
    // (left -> right along any edge, right -> left along a matched edge).
    let mut za: Vec<bool> = vec![false; na];
    let mut zb: Vec<bool> = vec![false; nb];
    queue.clear();
    for i in 0..na {
        if mate_a[i] < 0 {
            za[i] = true;
    ...
    // Cover = (A \ Z) ∪ (B ∩ Z). Verified against every crossing edge before it
    // is handed back, so no partially computed state can yield a non-separator.
    for i in 0..na {
        if !za[i] {
            continue;
        }
...
fn nd_order(pattern: &Pattern) -> Vec<i32> {
    ...
    let mut bfs: Vec<usize> = Vec::new();
    // Scratch for the separator-minimization pass: `pos_b` compacts one boundary
    // side to 0..k (always restored to all -1), `incover` flags the chosen cover.
    let mut pos_b: Vec<i32> = vec![-1i32; n];
    let mut incover: Vec<bool> = vec![false; n];

    // Work budget for separator minimization, shared across the whole recursion
    // and charged per edge scanned, so the refinement can never move the
    // worst-case time of a subset-heavy input. It is separate from the split
    // budget below: exhausting it degrades every later subset to its wide
    // separator, which is exactly the previous behaviour.
    let mut cover_budget: i64 = 24 * pattern.nnz() as i64 + 32 * n as i64;
    ...
    while let Some((nodes, lo, _hi)) = stack.pop() {
        ...
        // A BFS level is a WIDE separator. Every edge crossing this bisection
        // runs between level `sep_level-1` and level `sep_level` (BFS levels
        // differ by at most one, and the unreached vertices in `right` are a
        // different component), so ANY vertex cover of that bipartite edge set
        // separates the halves just as well — and the minimum one is typically a
        // fraction of a whole level. Adopt it only when it is strictly smaller
        // and still leaves both halves non-empty, which is what keeps every
        // pushed task strictly smaller than its parent.
        if sep_level >= 2 && !sep.is_empty() && cover_budget > 0 {
            let prev = (sep_level - 1) as u32;
            let mut side_a: Vec<usize> = Vec::new();
            for &u in &left {
                cover_budget -= adj[u].len() as i64;
                if dist[u] == prev
                    && adj[u]
                        .iter()
                        .any(|&w| mark[w] && dist[w] as usize == sep_level)
                {
                    side_a.push(u);
                }
            }
            if cover_budget > 0 {
                if let Some(cover) =
                    min_cover_separator(&side_a, &sep, &adj, &mut pos_b, &mut cover_budget)
                {
                    if cover.len() < sep.len() {
                        for &u in &cover {
                            incover[u] = true;
                        }
