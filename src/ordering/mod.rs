...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    if heavy_arm_enabled() && nnz < METRIC_LIGHT_MAX_NNZ {
        // 0096b: keep 0095's seven; add DegSqrt@10 (the missing shipped-α
        // ticket for the strongest named light-tier win) + DegDivNvSqrtWf@10
        // (new objective, not mid-α pile-on). Same budget/cap/envelope.
