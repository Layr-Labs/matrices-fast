...
const PEO_ALT_WIDE_N: usize = 10_000;
const PEO_ALT_LEDGER_WIDE: u64 = 1_000_000;
/// ── 0191: the chain and the ladder are row-disjoint above the ladder's window ─
/// The 0190 table (chain 4e6 to n <= 50 000, no ladder) and the 71c2c5fe receipt
/// (ladder to 12 000, chain gated at 10 000) are the only two builds that ever
/// finished a hidden run; every one of the six builds that carried BOTH spenders
/// (436d52d2, bc0e0b6c, c13df7a2, 6ad8cc5e, e5a3c6b4, and 55d9ed93 — the last
/// also killed, 9/12/26) died 104-114 s into the Benchmark step. 55d9ed93 was
/// strictly CHEAPER than the surviving frontier on all eight dev rows of
/// 10 000 < n <= 12 000 (5.338 s vs 6.197 s, probe frame), so the separation is
/// not a cost ordering: what the six kills share is that a row above the ladder's
/// own window carries chain work at all. This band is therefore handed to the
/// ladder (which is worth powerflow0300p, n = 11251, 0.11 dev bips there) and the
/// chain resumes above it, where no draw can reach.
const PEO_ALT_SKIP_LO: usize = 10_000;
const PEO_ALT_SKIP_HI: usize = 12_000;
...
/// that row.
const PEO_ALT_MAX_N: usize = 50_000; // iter26: was 10_000 (0187); see 0189
...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    if n >= 16 && n <= PEO_ALT_MAX_N && (n as u64 + nnz as u64) < PEO_ALT_LEDGER
        && !peo_alt_danger
        // 0191: hand the ladder's own upper band to the ladder alone. See the
        // PEO_ALT_SKIP_LO comment: the chain's six kills all carried chain work
        // on a row above the ladder's window, and this band's chain yield is
        // exactly 0.0000 on all eight dev rows while the ladder's draw moves
        // powerflow0300p (n = 11251) there.
        && !(n > PEO_ALT_SKIP_LO && n <= PEO_ALT_SKIP_HI)
    {
        let ledger_cap: u64 = if n > PEO_ALT_WIDE_N { PEO_ALT_LEDGER_WIDE } else { PEO_ALT_LEDGER };
    ...
    if n <= window_n && nnz <= window_nnz {
        ...
        /// had in the 0.40-weighted gt_10k bucket) and PEO_ALT_SKIP_LO/HI keeps
        /// the chain out of it; the chain keeps the frontier's own 50 000 scope
        /// above, at the reduced work allowance, where its dev yield is 0.0000
        /// on all 38 rows but its hidden yield is what 71c2c5fe was missing.
        const SHIPPED_FULL_N: usize = 12_000;
