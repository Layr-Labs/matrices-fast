...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    if n >= SUBTREE_MIN_N && n + nnz <= FINAL_REFINE_MAX_WORK {
        ...
        // iter147k: SKIP iter110 mid rebuild when n>1000 — ticket+146b already
        // paid a rebuild in-envelope; mid mb32/4M was the stacked timing bomb.
        // Keep lt_1k chained rebuild only (cannot see lee killers).
        if cur_flops < best_flops_before_final && n <= 1_000 {
