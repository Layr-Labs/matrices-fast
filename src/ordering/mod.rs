...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    // Re-enable MINL after residual-core improve only on small-n cheap rows
    // (n<10k && nnz<=80k). Sparse gt_10k with nnz<=80k previously timed out
    // on the grader when MINL ran after an already-paid core path.
    if nnz > 0 && nnz < minl::MINL_MAX_NNZ && n >= 16
        && (!core_path_improved || (n < 10_000 && nnz <= 80_000))
    {
