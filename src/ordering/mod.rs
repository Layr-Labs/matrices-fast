...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    // The skip used to be the two-sided band `(3_000..8_000).contains(&n) &&
    // nnz >= 9_000`, whose comment named the three corpus rows each edge was
    // drawn around. It is re-derived here as the upward-closed cone three
    // sibling sites in this file already use for the same purpose
    // (`mod.rs` danger / danger_timing / the residual-core exact guard): the
    // cone contains the band, so every pattern that skips the chains today
    // still skips them and none gains chain work.
