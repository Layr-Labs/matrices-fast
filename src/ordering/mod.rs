...
/// ~constant across the tier. It runs on SPARSE patterns only (`nnz <= 6 n`,
/// the same density guard the no-dense AMD pass uses), from where the crown's
/// lottery stops up to the cutoff below. There used to be a second, DENSE
/// sub-tier at 420k..700k, separated from this one by a 20 000-wide hole; the
/// hole existed to exclude one specific corpus matrix whose 24 seeds all
/// returned 1.0000, and its lower edge sat 1 024 below that matrix's `nnz`.
/// A gap has no cost reading, so both the gap and the sub-tier it served are
/// gone and the remaining ladder is monotone non-increasing in `nnz`.
