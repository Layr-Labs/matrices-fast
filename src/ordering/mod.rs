...
pub fn order(pattern: &Pattern) -> Vec<usize> {
    ...
    if (12..=300).contains(&pattern.n) && pattern.row_idx.len() <= 3000 {
        cutoff_plateau_refine(pattern, cutoff_paired_swap_refine(pattern, best), true)
    } else { best }
}
...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    if n >= REDUCE_MIN_N && nnz <= REDUCE_MAX_NNZ {
        ...
        // All passes are ranked by exact core scores. Only the degree-three
        // path enables recursive refinement; extra depths retain their budget.
        let order_core = |cl: &core_lift::CoreLift, alphas: &[f64], threads: bool, recurse: bool, incumbent: u64| -> Option<(u64, Vec<usize>)> {
            let cn = cl.core_n();
            ...
            let run_pass = |k: usize| -> Option<(u64, Vec<usize>)> {
                ...
                let (p, _proxy): (Vec<i32>, u64) = if k < alphas.len() {
            ...
            let f = cl.prefix_flops + if refined.is_some() {
            ...
            if recurse && (8..=4000).contains(&cn) && cl.core_nnz() <= 30_000 {
                ...
                for q in [(0..cn).rev().collect::<Vec<_>>(), degree_order] {
                    ...
                    for alpha in [2.5, 10.0] {
                        ...
                        if let Ok((p, _)) = feral_amf::amf_order_opts(&rc, &options) {
                            ...
                            if score < best_f {
                                best_f = score;
                                best_cand = core_lift::splice(cl, &cp);
                            }
                        }
                    }
                }
