...
/// Absolute nnz ceiling on the heavy near-regular class. Near-regularity keeps an
/// AMD pass at anchor SPEED, but speed is still linear in size: on an unseen
/// matrix several times larger than anything in the dev corpus (largest here is
/// acopf_case9241pegase_qcqp at nnz 1292408) the extra pass plus its symbolic
/// scoring is unbounded work against a 2 s cap. Bound it by construction.
const HEAVY_GT10K_MAX_NNZ: usize = 1_500_000;
...
fn relabel_restarts_tuned(budget: usize, cap: usize, n: usize, nnz: usize, max_deg: usize) -> usize {
    ...
    } else if n >= 10_000 && nnz > 350_000 && nnz <= HEAVY_GT10K_MAX_NNZ && max_deg * 1_000 <= n {
...
pub fn order(pattern: &Pattern) -> Vec<usize> {
    ...
    let heavy_near_regular =
        n >= 10_000 && nnz > 350_000 && nnz <= HEAVY_GT10K_MAX_NNZ && max_deg * 1_000 <= n;
