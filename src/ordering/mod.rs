...
/// The 0193 receipt was never acted on: the constant stayed at 50 000 and the
...
/// 300-row corpus, graded-closest `SSI_MARK_NOSCORE=1` frame): over the 38 dev
/// rows in that band the gate admits, the chain's own mark sums to **8.11 s**,
/// median 0.21 s/row, on a band where the same record measures its yield at
/// zero. Ten-row same-binary arm pair (50 000 vs 10 000): identical flop
/// counters; worker-frame A/B of the two real binaries: 20 of 21 rows
/// bit-identical, the one mover being the dense twin's row. [0277]
...
/// The anchor margin the fork requires: the incumbent must already be at least
/// this many percent below the AMD anchor for the fork to be entered. See
/// [`shared_basin_fork_band`] for the receipt that chose 10.
const SHARED_BASIN_FORK_MARGIN_PCT: u64 = 10;
