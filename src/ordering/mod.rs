...
/// iter1061a: fill-scale fence (dev-free; caps hidden fill-heavy batches).
...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    // iter1061a BREAKTHROUGH: cut tip window spend on n>=8k (lee4_06 critical path).
    // Reinvest into deep small-n windows only. Timing-first structural leap.
    ...
    // Deep late window on n<=5k sparse non-hub — cannot touch lee4_06 (n=10.4k).
    let deep_small = n >= 8
        && n <= 5_000
        && nnz <= 60_000
        && nnz <= 12 * n
