    ...
    if n >= INDEP_MIN_N && nnz <= INDEP_MAX_NNZ {
        if let Some((core_total, cand)) = indep_first::run(&scoring_pat, INDEP_WORK_LEDGER) {
            if core_total < best_flops && is_bijection(&cand, n) {
                ...
                // iter950a: strip digabel+hydro fitted windows only; KEEP mid_force
                // (nnz-heavy 8k–20k) so lee4_10 stays on the indep immediate path (949 lost it).
                let gasprod_band = n >= 20_000;
                let mid_force = (8_000..20_000).contains(&n) && nnz >= 50_000;
                if gasprod_band || mid_force || f.saturating_mul(INDEP_IMMEDIATE_MARGIN.1) <= best_flops.saturating_mul(INDEP_IMMEDIATE_MARGIN.0) {
                    best_flops = f;
                    best_perm = cand;
