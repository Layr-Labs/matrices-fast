...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    if n >= SUBTREE_MIN_N && n + nnz <= FINAL_REFINE_MAX_WORK {
        ...
        // iter129: cfg_agg 3M, n<15k && nnz<200k (cc6ac19 4M full FAIL; 129b 2M only −2.0 bip).
        // Skips lee4_09/10 crowns; keeps methanol200/mpbp_34/pooling. Leaner than full-gate 4M/6M.
        // Prefer worst<=1.00; >=15 movers; real bip past darth 0.84913 (not −0.01% micro).
