    ...
    if n >= INDEP_MIN_N && nnz <= INDEP_MAX_NNZ {
        if let Some((core_total, cand)) = indep_first::run(&scoring_pat, INDEP_WORK_LEDGER) {
            if core_total < best_flops && is_bijection(&cand, n) {
                let f = score(&cand);
                // iter233a: digabel-band + gasprod-band any-accept on tip (no METIS-arki)
                let digabel_band = (400..=1000).contains(&n);
                let gasprod_band = n >= 20_000;
                if digabel_band || gasprod_band || f.saturating_mul(INDEP_IMMEDIATE_MARGIN.1) <= best_flops.saturating_mul(INDEP_IMMEDIATE_MARGIN.0) {
                    best_flops = f;
                    best_perm = cand;
