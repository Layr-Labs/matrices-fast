...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
        ...
        let mut order_core = |cl: &core_lift::CoreLift, alphas: &[f64], threads: bool, recurse: bool, incumbent: u64| -> Option<(u64, Vec<usize>)> {
                ...
                && !(n >= 1_800 && nnz >= 9_000)
            {
                ...
                let streams: &[(i64, u64)] = if cn <= 400 {
                    ...
                    // iter97: mid-core 3→2 (hidden-load)
                    &[
                        (budget, 0xA1B2_C3D4_E5F6_7788u64),
                        (budget / 2, 0x1234_5678_9ABC_DEF0),
                    ]
                ...
                // iter97: drop chained LNS (hidden-load cut; tip has none)
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    // iter97: tip+minimal late polish for ≥15 movers under hidden 2× load.
    // Mid: 1 stream only; cheap: 3 streams; skip cost>16M; no mid simplicial.
    if n >= 16 && n < 3_000 && nnz <= 12_000 {
        let cost = (n as u64).saturating_mul(nnz as u64);
        if cost > 16_000_000 {
            // skip killers (tighter than iter96 20M)
        } else {
            let late_streams: &[(i64, u64)] = if cost > 8_000_000 {
                &[
                    (8_000_000i64, 0xC0FF_EE00_BADC_0FFEu64),
                ]
            } else {
                &[
                    (20_000_000i64, 0xC0FF_EE00_BADC_0FFEu64),
                    (15_000_000, 0x0D15_EA5E_FEED_FACEu64),
                    (12_000_000, 0xCAFE_BABE_DEAD_BEEFu64),
                ]
            };
            ...
            let (d_rounds, d_budget) = if cost > 8_000_000 {
                ...
                (2usize, (n as i64).saturating_mul(5_000).min(5_000_000))
            };
            if let Some(cand) = rgreedy::adjacent_pair_descent(
                n, &pattern.col_ptr, &pattern.row_idx,
            ...
            // cheap-band simplicial only (mid skipped — hidden load)
            if cost <= 8_000_000 {
                if let Some(cand) = rgreedy::simplicial_promotion(
                    n, &pattern.col_ptr, &pattern.row_idx, &best_perm, 3_000_000,
                ) {
                    if is_bijection(&cand, n) {
                        let f = score(&cand);
                        if f < best_flops { best_flops = f; best_perm = cand; }
