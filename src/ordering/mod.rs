...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    // Keep this optional exact pass away from the hidden timeout-size band.
    // Its public wins are concentrated below 4k nodes and 20k nonzeros.
    if n >= 6 && n <= 4_000 && nnz <= 20_000 {
