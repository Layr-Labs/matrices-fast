...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    if n >= REDUCE_MIN_N && nnz <= REDUCE_MAX_NNZ {
        ...
        // iter96: single core-exact shot (widen+chain kept) — cut hidden work vs dual.
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    // iter74/95b: LATE exact polish — needed for ≥15 movers (strip failed bar).
    // Lean: keep cheap 6-stream + mid 3-stream trim; keep cost>20M skip.
    // Faclay skips elsewhere; prefer ≤1.00 on crown.
    if n >= 16 && n < 3_000 && nnz <= 12_000 {
        let cost = (n as u64).saturating_mul(nnz as u64);
        if cost > 20_000_000 {
            // skip killers
        } else {
            // iter96: lean late polish — mid 2 streams, cheap 4 streams (cut hidden load)
            ...
            for &(budget, rng_seed) in late_streams {
                if let Some((cand, _)) = rgreedy::search(
                    n, &pattern.col_ptr, &pattern.row_idx,
                    &best_perm, best_flops, budget, rng_seed,
                ) {
                    if is_bijection(&cand, n) {
                        let f = score(&cand);
            ...
            if let Some(cand) = rgreedy::adjacent_pair_descent(
                n, &pattern.col_ptr, &pattern.row_idx,
                &best_perm, d_rounds, d_budget,
            ) {
                let f = score(&cand);
                if f < best_flops { best_flops = f; best_perm = cand; }
            }
            let sim_budget: i64 = if cost <= 8_000_000 { 4_000_000 } else { 2_000_000 };
            if let Some(cand) = rgreedy::simplicial_promotion(
                n, &pattern.col_ptr, &pattern.row_idx, &best_perm, sim_budget,
            ) {
                if is_bijection(&cand, n) {
                    let f = score(&cand);
                    if f < best_flops { best_flops = f; best_perm = cand; }
