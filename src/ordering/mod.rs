...
/// 71c2c5fe is byte-identical to them below n = 10 000. What separates the two
/// columns is the *coexistence* of the two spenders, not their per-row prices,
/// so the chain is confined to the scope where a draw can never reach it — the
/// surviving build's own scope — and the wide band is handed to the ladder
/// alone (SHIPPED_FULL_N), whose draw is score-monotone by construction.
...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    // 0192: the gate is the frontier's own (16 <= n <= MAX_N and n + nnz under
    // the 4e6 allowance), with the scope narrowed to the band the one surviving
    // draw-carrying build used. See the PEO_ALT_MAX_N comment for the receipt law.
    if n >= 16 && n <= PEO_ALT_MAX_N && (n as u64 + nnz as u64) < PEO_ALT_LEDGER
        && !peo_alt_danger
    {
        let ledger_cap: u64 = PEO_ALT_LEDGER;
    ...
    if n <= window_n && nnz <= window_nnz {
        ...
        const SHIPPED_FULL_N: usize = 50_000;
