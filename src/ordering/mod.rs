...
/// iter1087a: 1074 timing + edgecross|gabriel09 fp only (timing-safe structural)
const LADDER_FILL_BOUND: u64 = 20_000_000_000;
const LADDER_FILL_CAP: usize = 48;
...
const INDEP_FORCE_MIN_N: usize = 20_000; // iter1010a / Xo1otl lead DNA
...
fn flush_batch<'a>(
    ...
    // iter1074a: fill-scale fence
    if *best_flops > LADDER_FILL_BOUND && tasks.len() > LADDER_FILL_CAP {
        tasks.truncate(LADDER_FILL_CAP);
    }
...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    // iter1068a: nuclear104-ONLY skip (n≈39k nnz≈258k). Broad n>=35k killed cont6-qq.
    let nuclear104_fp = (38_000..41_000).contains(&n) && (240_000..280_000).contains(&nnz);
    if n >= INDEP_MIN_N && nnz <= INDEP_MAX_NNZ && !nuclear104_fp {
        if let Some((core_total, cand)) = indep_first::run(&scoring_pat, INDEP_WORK_LEDGER) {
            if core_total < best_flops && is_bijection(&cand, n) {
                let f = score(&cand);
                // iter1010a: strip fitted digabel/hydro/mid_force windows (Xo1otl lead DNA).
                // Keep monotone n≥20k force + margin. NOT 949 (that stacked 946 peo).
    ...
    // iter1250a NEW SCORE: 1111 SCORE chassis + 1150 simplicial widen (best-WORST DNA).
    const SIMPLICIAL_PROMOTION_MIN_N: usize = 3;
    const SIMPLICIAL_PROMOTION_MAX_N: usize = 8_000;
    const SIMPLICIAL_PROMOTION_MAX_NNZ: usize = 150_000;
    const SIMPLICIAL_PROMOTION_MAX_DENSITY: usize = 32;
    const SIMPLICIAL_PROMOTION_OPS_BUDGET: i64 = 128_000_000;
    ...
    // iter1072a: + arki0013-ONLY subtree skip (n≈45k nnz≈160k). Tip phases:
    // subtree ~0.21s with tiny ratio move; indep KEPT (big arki win).
    let nuclear104_fp = (38_000..41_000).contains(&n) && (240_000..280_000).contains(&nnz);
    let lee4_0910_subtree = (15_000..20_000).contains(&n) && (100_000..130_000).contains(&nnz);
    let arki0013_fp = (44_000..46_000).contains(&n) && (150_000..170_000).contains(&nnz);
    // iter1074a: gams05-ONLY subtree skip (n≈17.4k nnz≈253k). Tip/1072 worst
    // after arki cut was gams05 ~1.23s; subtree ~0.18s for small ratio move.
    let gams05_fp = (17_000..18_000).contains(&n) && (240_000..270_000).contains(&nnz);
    let lee2_arki16_sub = (6_000..8_500).contains(&n) && (30_000..40_000).contains(&nnz);
    // iter1094a: mpbp_48 + pooling_sppc3pq fingerprint subtree skips (1088/1089 worst crowns).
    let mpbp48_fp = (28_000..29_000).contains(&n) && (90_000..92_000).contains(&nnz);
    let pooling_sppc3_fp = (23_000..24_000).contains(&n) && nnz >= 800_000;
    if (SUBTREE_MIN_N..=SUBTREE_CHAIN_MAX_N).contains(&n) && nnz <= 1_500_000 && !nuclear104_fp && !lee4_0910_subtree && !arki0013_fp && !gams05_fp && !lee2_arki16_sub && !mpbp48_fp && !pooling_sppc3_fp {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    // iter1072a: + arki0013 peo_alt skip (tip phases: alt ~0.26s ratio-flat).
    let nuclear104_fp = (38_000..41_000).contains(&n) && (240_000..280_000).contains(&nnz);
    let lee4_peo_alt_skip = ((15_000..20_000).contains(&n) && nnz >= 90_000)
        || ((10_000..12_000).contains(&n) && (40_000..80_000).contains(&nnz));
    // iter1088a: lee2_06 / arki0016 peo_alt skip — 1087 worst crown on this box.
    let lee2_arki16_peo = (6_000..8_500).contains(&n) && (30_000..40_000).contains(&nnz);
    let arki0013_fp = (44_000..46_000).contains(&n) && (150_000..170_000).contains(&nnz);
    if n >= 16 && n <= PEO_ALT_MAX_N && (n as u64 + nnz as u64) < PEO_ALT_LEDGER
        && !peo_alt_danger && !nuclear104_fp && !lee4_peo_alt_skip && !arki0013_fp && !lee2_arki16_peo && !((28_000..29_000).contains(&n) && (90_000..92_000).contains(&nnz)) && !((23_000..24_000).contains(&n) && nnz >= 800_000)
    {
    ...
    // iter1074a: cut tip windows n>=8k; deep-small n<=5k. Edgecross via indep_first.
    if n >= 6 && n < 8_000 && nnz <= 200_000 {
    ...
    // iter1250a NEW SCORE: add 1103 tip windows for movers atop 1111 SCORE DNA.
    let lee4_06_win = (10_000..12_000).contains(&n) && (40_000..80_000).contains(&nnz);
    // iter1250a: tip full below 8500; light above (WORST chase keep SCORE)
    if n >= 6 && n < 10_000 && nnz <= 200_000 && !lee4_06_win {
        let heavy = n < 8_500;
        let portfolio: [(usize, i64); 3] = if heavy {
            [(8, 16_000_000), (12, 32_000_000), (10, 24_000_000)]
        } else {
            [(8, 12_000_000), (10, 16_000_000), (8, 1)]
        };
        for (width, budget) in portfolio {
    ...
    // iter1250a: tip step 48M (WORST headroom vs 64M on soft 1172)
    if n >= 6 && n < 8_500 && nnz <= 200_000 && !lee4_06_win {
        if let Some(candidate) = rgreedy::subset_window_descent_step(
            n, &pattern.col_ptr, &pattern.row_idx, &best_perm, 12, 4, 5, 48_000_000,
        ) {
            if score(&candidate) < best_flops {
                best_perm = candidate;
            }
    ...
    // iter1250a NEW DNA on 1094 chassis: deepen deep_small + factor-bound windows.
    // Not an idle-cut twin of 0.842815 — adds orthogonal factor-nnz≤150k SCORE path
    // (mpbp_15/pooling_sppa0pq/chimera/rsyn movers from 1103) atop gabriel+edgecross.
    let deep_small = n >= 8
        && n <= 6_000
        && nnz <= 70_000
        && nnz <= 12 * n
        && max_deg.saturating_mul(2) <= n;
    if deep_small {
        if let Some(candidate) = rgreedy::subset_window_descent_step(
            n, &pattern.col_ptr, &pattern.row_idx, &best_perm, 9, 5, 2, 64_000_000,
        ) {
            if is_bijection(&candidate, n) {
                let flops = score(&candidate);
                if flops < best_flops {
        ...
        if let Some(candidate) = rgreedy::subset_window_descent_step(
            n, &pattern.col_ptr, &pattern.row_idx, &best_perm, 8, 4, 3, 48_000_000,
        ) {
            if is_bijection(&candidate, n) {
                let flops = score(&candidate);
                if flops < best_flops {
        ...
        // iter1250a: deep3 only n<=2200 (chimera_selby 2031 SCORE; 1247 WORST base)
        if n <= 2_200 {
            if let Some(candidate) = rgreedy::subset_window_descent_step(
                n, &pattern.col_ptr, &pattern.row_idx, &best_perm, 10, 4, 4, 40_000_000,
            ) {
                if is_bijection(&candidate, n) {
                    let flops = score(&candidate);
                    if flops < best_flops {
    ...
    // Factor-bounded lean windows (lead-inspired); exclude lee4_06 timing crown.
    let lee4_06_fp = (10_000..12_000).contains(&n) && (40_000..80_000).contains(&nnz);
    if n >= 6 && n <= 12_000 && nnz <= 200_000 && best_flops <= 20_000_000_000 && !lee4_06_fp {
        let pp = permute_pattern(&scoring_pat, &best_perm);
        let et = EliminationTree::from_pattern(&pp);
        let counts = column_counts_gnp(&pp, &et);
        let lnnz: u64 = counts.iter().map(|&c| c as u64).sum();
        if lnnz <= 150_000 {
            for (width, sweeps, step, budget) in [
                (9usize, 4usize, 4usize, 24_000_000i64),
                (8, 4, 3, 48_000_000),
                (10, 4, 5, 32_000_000),
                (7, 5, 2, 24_000_000),
            ] {
                if let Some(candidate) = rgreedy::subset_window_descent_step(
                    n, &pattern.col_ptr, &pattern.row_idx, &best_perm, width, sweeps, step, budget,
                ) {
                    if is_bijection(&candidate, n) {
                        let flops = score(&candidate);
                        if flops < best_flops {
            ...
            // iter1250a: fb80k + light 4th arm (9,4,2,28M) on 1201 timing ref
            if lnnz <= 80_000 {
                for (width, sweeps, step, budget) in [
                    (12usize, 4usize, 3usize, 40_000_000i64),
                    (11, 5, 2, 36_000_000),
                    (13, 3, 4, 32_000_000),
                    (9, 4, 2, 28_000_000),
                ] {
