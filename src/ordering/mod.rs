...
/// Test-arm margin for the retired basin fork. Twenty percent retains the two
/// high-value public movers while dropping `gancns`; production no longer
/// admits the fork after its 0%, 10%, 14%, and 20% forms all cap-failed.
#[cfg(test)]
const SHARED_BASIN_FORK_MARGIN_PCT: u64 = 20;
/// Test builds may re-point this margin through `SSI_FORK_MARGIN_PCT` after
/// explicitly enabling the arm with `SSI_SHARED_BASIN_FORK=1`.
...
/// Historical basin-fork admission: the ported structural band AND an anchor
/// margin. Production retired this arm after iter77; the implementation remains
/// available for explicit test measurements.
///
...
fn shared_basin_fork_band(n: usize, nnz: usize, best_flops: u64, amd_flops: u64) -> bool {
    // The fork is retired in production. Keep it as an explicit opt-in test
    // arm so its value and cost can still be reproduced from one binary.
...
#[cfg(not(test))]
#[inline(always)]
fn shared_basin_fork_band(_n: usize, _nnz: usize, _best_flops: u64, _amd_flops: u64) -> bool {
    false
}
