...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    if n >= 6 && n <= rgreedy::MAX_N && nnz <= 200_000 {
        for (width, budget) in [(8, 16_000_000), (10, 24_000_000)] {
    ...
    if n >= 6 && n <= rgreedy::MAX_N && nnz <= 200_000 {
        ...
        // Four additive window packages exceeded the hidden cap. Exchange the
        // inherited width-12/32M union pass for this width-8/32M signature pass:
        // the total terminal allowance stays at the promoted parent's 136M,
        // while the smaller exponential window is faster and scores lower.
        if let Some(candidate) = rgreedy::subset_window_descent_step(
