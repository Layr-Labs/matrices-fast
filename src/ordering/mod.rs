...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    // iter1232a NEW SCORE: 1111 SCORE chassis + 1150 simplicial widen (best-WORST DNA).
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    // iter1232a NEW SCORE: add 1103 tip windows for movers atop 1111 SCORE DNA.
    let lee4_06_win = (10_000..12_000).contains(&n) && (40_000..80_000).contains(&nnz);
    // iter1232a: tip full below 8500; light above (WORST chase keep SCORE)
    ...
    // iter1232a: tip step 48M (WORST headroom vs 64M on soft 1172)
    ...
    // iter1232a NEW DNA on 1094 chassis: deepen deep_small + factor-bound windows.
    ...
    if deep_small {
        ...
        // iter1232a NEW SCORE: third deep_small arm on 1163 fb80k chassis
        if let Some(candidate) = rgreedy::subset_window_descent_step(
    ...
    if n >= 6 && n <= 12_000 && nnz <= 200_000 && best_flops <= 20_000_000_000 && !lee4_06_fp {
        ...
        if lnnz <= 150_000 {
            ...
            // iter1232a: fb80k + 4th arm 16M (1226 SCORE @ lower cost)
            if lnnz <= 80_000 {
                for (width, sweeps, step, budget) in [
