...
pub fn order(pattern: &Pattern) -> Vec<usize> {
    ...
    if n < AMF_SWEEP_MAX_N && nnz < 130_000 {
        // Three-alpha unlabelled AMF sweep. Extra 0.5/2.5 tickets (0057,
        // 85dcf2a) failed hidden — do not add a fourth. probe_amf_swap_alpha
        // showed swapping 16.0 for 0.5 keeps the two unique wins
        // (crudeoil_pooling_dt2, chp_partload) with zero conservative
        // losses, same ticket count as promoted H2.
