...
fn subtree_cfg_for(n: usize, nnz: usize) -> rgreedy::SubCfg {
    ...
    if n < 1_000 {
        ...
        // else (blocks x budget x streams). Sparse small patterns use four
        // deeper blocks: a measured shape sweep found that short elimination
        // trees benefit more from finishing a few high-ranked local searches
        // than from starting many shallow ones. Denser small patterns retain
        // the promoted 16-by-2M shape to protect the hidden time cap.
