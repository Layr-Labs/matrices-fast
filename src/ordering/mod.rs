            ...
            if cn > 0 && cn < n && cl.core_nnz() <= REDUCE_MAX_CORE_NNZ {
                ...
                if let Some((_, k)) = pick {
                    if let Some((core_flops, cp)) = &results[k] {
                        ...
                        // Refine the residual core with one existing k5/k4 cycle.
                        // Each kernel receives 16M units. Construction and exact
                        // scoring overhead are measured in the full pipeline.
