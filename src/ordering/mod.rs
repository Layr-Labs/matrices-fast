...
/// Sparse-large band of the extra-depth reductions. The dense band requires `nnz >= 6 n`, which
/// excludes a class of large graphs between four and six times the dimension. An extra depth on
/// that class is worth 6.7% and 9.9% on two of them, but its cost scales with the input, so the
/// band carries its own nonzero cap: the depth is only ever attempted where the work it can do is
/// bounded, rather than where it happened to measure cheap. Every row that gains sits below the
/// cap (275_106, 331_010, 277_562); every row above it gained nothing (476_332, 480_012, 557_994,
/// and larger). The existing reduce-work budget, clique-pair budget and extra-core ledger all
/// still apply unchanged on top of this.
...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    if n >= REDUCE_MIN_N && nnz <= REDUCE_MAX_NNZ {
        ...
        for &depth in REDUCE_EXTRA_DEPTHS.iter() {
            ...
            let sparse_large = nnz > REDUCE_EXTRA_MIN_NNZ
                && nnz <= REDUCE_SPARSE_MAX_NNZ
                && nnz >= REDUCE_SPARSE_DENSITY * n;
