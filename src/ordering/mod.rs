...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    // VOL-TX2 (re-priced on ab30c0e 2026-09-10 — NEW ticket on this base):
    // chained second transplant pass, conditioned on a strict phase-14 win
    // this row (five2 pattern — not a terminal re-run: the recipient is
    // phase-14's output, and downstream stages still run after, consuming
    // whatever this finds). Same call, same ledger, same gates; strict
    // admit + bijection check.
