...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    if n <= 12_000 {
        // Shipped windows/budgets; `SSI_TERM_LADDER` and `SSI_TERM_FULL_N`
        // (test builds only) re-price them without changing production.
        const SHIPPED_FULL_N: usize = 7_000;
        const SHIPPED_LADDER: [(i64, u64); 2] = [
            (200_000_000i64, 0x9E37_79B9_7F4A_7C15u64),
            (200_000_000, 0xD1B5_4A32_D192_ED03),
        ];
        /// Wide rows (7 000 < n <= 12 000): one draw, a quarter of the budget.
        const SHIPPED_WIDE_LADDER: [(i64, u64); 1] =
