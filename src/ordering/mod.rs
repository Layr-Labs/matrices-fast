...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    // Hidden fail e0d3727: n<80k/nnz<200k put two extra exact scores on
    // arki0013/methanol400/nuclear10a-class rows. Keep the only mover
    // (sonet21v6 n=8232 nnz=40744) and stay out of gt_10k.
    if n >= 1_000 && n < 10_000 && nnz <= 50_000 {
        consider(&mut best_flops, &mut best_perm, &|| {
