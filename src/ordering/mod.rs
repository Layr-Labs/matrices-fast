...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    #[cfg(test)]
    let strip_gate = strip_gate && probe::terminal_phase_enabled(1);
   ...
   // Terminal completion cleanup leaves every existing descent seed intact.
    ...
    #[cfg(test)]
    parallel::phase_mark("15a.strip", _tph, best_flops);
    #[cfg(test)]
    let _tph = std::time::Instant::now();
    ...
    #[cfg(not(test))]
    let telos_enabled = true;
    #[cfg(test)]
    let telos_enabled = probe::terminal_phase_enabled(2);
    if telos_enabled && telos::enabled(n, nnz) {
        best_flops = score(&best_perm);
        // The small schedule uses only the leader. No displaced permutations
        // are copied, and rejected shapes do not pay another symbolic score.
        #[cfg(not(test))]
        ...
        #[cfg(test)]
        let (telos_perm, telos_flops) = telos::refine_for_probe(
            &scoring_pat,
            nnz,
            max_deg,
            &best_perm,
    ...
    // Terminal MCS tie alternatives: use several static structural ranks in
    // both directions. Keeping them outside the incumbent-dependent PEO chains
    // makes this a true strict best-of the finished result. A strict win earns
    // another round on its smaller completion; the walk stops immediately at
    // a fixed point and is capped at two rounds and a shared structural work
    // allowance. Large completions can be refused before the first round.
    #[cfg(not(test))]
    let terminal_ranks_enabled = true;
    #[cfg(test)]
    let terminal_ranks_enabled = probe::terminal_phase_enabled(4);
    if terminal_ranks_enabled
        && n >= 16 && n <= peo_extract::MAX_N && nnz <= peo_extract::MAX_INPUT_NNZ
    {
        #[cfg(not(test))]
        let (rank_rounds, rank_ledger) = (2, 750_000u64);
        #[cfg(test)]
        let (rank_rounds, rank_ledger) = if probe::terminal_phase_enabled(16) {
            (4, 1_500_000u64)
        } else {
            (2, 750_000u64)
        };
        let mut rank_work = 0u64;
        for _rank_round in 0..rank_rounds {
            let pp = permute_pattern(&scoring_pat, &best_perm);
            let et = EliminationTree::from_pattern(&pp);
            let counts = column_counts_gnp(&pp, &et);
            let lnnz: u64 = counts.iter().map(|&c| c as u64).sum();
            if lnnz > PEO_OVERSIZE_MAX_LNNZ as u64 {
                break;
            }
            let work = n as u64 + nnz as u64 + lnnz;
            if rank_work + work > rank_ledger {
                break;
            }
            rank_work += work;
            let mut cur_flops: u64 = counts
                ...
                .sum();
            let round_entry = cur_flops;
            let Some(cands) = peo_extract::ranked_candidates_bounded(
                n,
                &pp.col_ptr,
                &pp.row_idx,
                &et.parent,
