...
fn leader_order(pattern: &Pattern) -> Vec<usize> {
    ...
    if n >= REDUCE_MIN_N && nnz <= REDUCE_MAX_NNZ {
        ...
        // iter618a: K=4 PRIMARY on nnz≥150k OR n≥22k — keeps 617 wins
        // (nuclear10a 164k, popdynm n22k, transswitch) while skipping lee4_09/10
        // (nnz 102–121k, n 16–18k) that paid timing with no score.
        if nnz >= 150_000 || n >= 22_000 {
            ...
            if let Ok(Some(cl4)) = lifted4 {
                    ...
                    && !seen_core_n.contains(&cn4)
                {
                    ...
                    if let Some((f, p)) = order_core(&cl4, &REDUCE_ALPHAS, true, true, best_flops) {
                        if f < best_flops {
                            best_flops = f;
                            best_perm = p;
                        }
