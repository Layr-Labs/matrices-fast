...
pub(super) fn deficiency(g: &[u64], w: usize, v: usize, words: &mut Vec<(usize, u64)>) -> usize {
    words.clear();
    words.extend(
        g[v * w..(v + 1) * w]
            .iter()
...
impl Dense {
    ...
    pub(super) fn elim(&mut self, v: usize) -> u64 {
        let w = self.w;
        let ns = self.nb(v);
        let row = row_words(&self.a[v * w..(v + 1) * w]);
    ...
    pub(super) fn init_def(&mut self) {
        let mut words = Vec::with_capacity(self.w);
        for v in 0..self.k {
            self.def[v] = deficiency(&self.a, self.w, v, &mut words) as i64;
        }
    }
    ...
    pub(super) fn defelim(&mut self, v: usize) -> u64 {
