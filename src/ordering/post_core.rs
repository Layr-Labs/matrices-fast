...
where S:Fn(&[usize])->u64,P:Fn(&[usize])->ScoringPattern,N:Fn()->u64 {
    ...
    if pat.n<6 ||pat.n>30_000 ||pat.nnz()>180_000 {return None;}
    let initial=score(incumbent);if initial>20_000_000_000 ||factor()>300_000 {return None;}
    ...
    for round in 0..2 {
        ...
        if best>20_000_000_000 ||factor()>300_000 {break;}
        ...
        let Some(candidate)=peo_extract::priority_candidate_with_limits(pat.n,
            ...
            30_000,180_000,300_000) else {break};
        if !is_bijection(&candidate,pat.n) {break;}
        let f=score(&candidate);if f>=best {break;}best=f;p=candidate;
    }
    if let Some(candidate)=refine_bounded(pat,&p,2,30_000,180_000,300_000,
