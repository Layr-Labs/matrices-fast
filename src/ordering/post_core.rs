...
/// Research-only single original-incidence tie policy under smaller work caps.
#[cfg(test)]
...
where S:Fn(&[usize])->u64,P:Fn(&[usize])->ScoringPattern,N:Fn()->u64 {
    ...
    const MAX_FACTOR:usize=750_000;
    ...
    let mut candidates=peo_extract::original_candidates_with_limits(pat.n,
        ...
        incumbent,&[3],MAX_N,MAX_INPUT,MAX_FACTOR)?;
    let mut p=candidates.pop()?;
    if !is_bijection(&p,pat.n) ||score(&p)>=initial {return None;}
    if let Some(q)=refine_bounded(pat,&p,2,MAX_N,MAX_INPUT,MAX_FACTOR,
