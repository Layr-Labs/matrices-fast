...
fn probe_margin_cascade() {
    ...
    let b: i64 = std::env::var("PROBE_CASCADE_BUDGET")
        .ok()
        .and_then(|v| v.parse().ok())
...
/// Does the per-block `budget` knob actually BIND on the dev corpus, or do the
/// block searches terminate early and leave it unspent?
///
/// This is the question that decides whether requested work is a legitimate
/// safety bound. `subtree_refine` charges at most `budget` word-ops per block
/// per stream, so `budget * max_blocks * streams` is an UPPER bound on the work
/// a pass can do — but an upper bound is only useful if it is tight. If wall
/// time keeps growing as `budget` doubles, the budget binds, dev time predicts
/// hidden time, and capping requested work caps wall time. If wall time
/// plateaus, the dev blocks are finishing early for reasons of their own and a
/// dev-measured wall time says nothing about a hidden matrix whose blocks do
/// saturate the same nominal budget.
#[test]
#[ignore]
fn probe_budget_saturation() {
    let corpus = crate::corpus::corpus();
    let budgets: [i64; 6] = [
        4_000_000,
        8_000_000,
    ...
    println!("SAT_HEADER\tname\tbucket\tn\tnnz\tblocks\t[secs_at_budget]*6");
    for (name, pat) in &corpus {
        let n = pat.n;
        if n == 0 || !(SUBTREE_MIN_N..=SUBTREE_MAX_N).contains(&n) || pat.nnz() > 1_500_000 {
            continue;
        }
        let sp = scoring_pattern(pat);
        let base = order(pat);

        // One shared setup: every budget searches the SAME postordered tree, so
        // the only difference between the columns is the budget itself.
        let permuted = permute_pattern(&sp, &base);
        let etree = EliminationTree::from_pattern(&permuted);
        let post = etree.postorder();
        let seed: Vec<usize> = post.iter().map(|&j| base[j]).collect();
        let post_pattern = permute_pattern(&sp, &seed);
        let post_etree = EliminationTree::from_pattern(&post_pattern);
        let ccounts: Vec<u32> = column_counts_gnp(&post_pattern, &post_etree)
            .into_iter()
            .map(|c| c as u32)
        ...
        for &budget in &budgets {
            ...
            row.push_str(&format!("\t{:.4}", t0.elapsed().as_secs_f64()));
        }
        println!("{row}");
    }
}
