...
/// Tabu engagement check: tabu_pair_descent vs RAW AMD on in-band rows.
/// Terminal seat nulled (0 movers) — expected if memory never engages on
/// converged incumbents (first sweep replays pair). If tabu beats AMD
/// anywhere, memory works on weak recipients (then join vs crown for a
/// portfolio seat); if not, tenure adds nothing anywhere. Offline only.
#[test]
#[ignore]
fn probe_tabu_weak() {
    let corpus = crate::corpus::corpus();
    ...
    for (name, pat) in &corpus {
        let n = pat.n;
        if n <= 32 || n > 1_000 || pat.nnz() > 60_000 {
            continue;
        }
        rows += 1;
        let (cp, ri) = core_of(pat);
        ...
        let sp = scoring_pattern(pat);
        let base = flops_of(&sp, &amd);
        match rgreedy::tabu_pair_descent(
            n,
            &pat.col_ptr,
            &pat.row_idx,
            &amd,
            ...
            Some(cand) => {
                fired += 1;
                if !is_bijection(&cand, n) {
                    println!("TABUW\t{name}\tNON-BIJECTION");
                    continue;
                }
                let f = flops_of(&sp, &cand);
                if f < base {
                    won += 1;
                    println!(
                        "TABUW\t{name}\tn={n}\t{base}->{f} (-{:.1} row-bips)",
    ...
    println!("TABUW SUMMARY rows={rows} fired={fired} won={won}");
}

/// SA machinery check: sa_swap_anneal vs RAW AMD on in-band rows
/// (n<=300, nnz<=12k). Terminal null is expected if basins are tight;
/// this decides whether the annealer itself works. Offline only.
#[test]
#[ignore]
fn probe_sa_weak() {
    let corpus = crate::corpus::corpus();
    ...
    for (name, pat) in &corpus {
        ...
        match rgreedy::sa_swap_anneal(
            ...
            Some(cand) => {
                fired += 1;
                if !is_bijection(&cand, n) {
                    println!("SAW\t{name}\tNON-BIJECTION");
                    continue;
                }
                let f = flops_of(&sp, &cand);
                if f < base {
                    won += 1;
                    println!(
                        "SAW\t{name}\tn={n}\t{base}->{f} (-{:.1} row-bips)",
                        (base - f) as f64 * 10000.0 / base as f64
                    );
                }
            }
    ...
    println!("SAW SUMMARY rows={rows} fired={fired} won={won}");
}

/// Ball-ruin machinery check: ball_window_reconstruct vs RAW AMD.
/// Terminal null needs the usual disambiguation (broken vs converged).
...
fn probe_rb_weak() {
    ...
    for (name, pat) in &corpus {
        ...
        let sp = scoring_pattern(pat);
        let base = flops_of(&sp, &amd);
        let mut cur = amd.clone();
        let mut cur_f = base;
        for attempt in 0..16 {
            match rgreedy::ball_window_reconstruct(
                n,
                &pat.col_ptr,
                &pat.row_idx,
                &cur,
                ...
                Some(cand) => {
                    fired += 1;
                    if !is_bijection(&cand, n) {
                        println!("RBW\t{name}\tNON-BIJECTION");
                        continue;
                    }
                    let f = flops_of(&sp, &cand);
                    if f < cur_f {
        ...
        if cur_f < base {
            won += 1;
            println!(
                "RBW\t{name}\tn={n}\t{base}->{cur_f} (-{:.1} row-bips)",
                (base - cur_f) as f64 * 10000.0 / base as f64
            );
        }
    }
    println!("RBW SUMMARY rows={rows} fired={fired} won={won}");
}

/// Path-relink machinery check: relink AMD -> RCM offline (no order()
/// needed). If interpolation steps beat AMD anywhere, the path code works
/// and the terminal null means interpolation is weaker than segment
/// assembly (transplant wins 39 from the same pool); if nothing beats AMD
/// even here, the path code is broken. Offline only.
#[test]
#[ignore]
fn probe_pr_weak() {
    let corpus = crate::corpus::corpus();
    ...
    for (name, pat) in &corpus {
        ...
        let rcm: Vec<usize> = rcm_order(pat).into_iter().map(|x| x as usize).collect();
        if rcm.len() != n {
            continue;
        }
        let sp = scoring_pattern(pat);
        let base = flops_of(&sp, &amd);
        let donors = vec![(0u64, rcm)];
        match rgreedy::path_relink(
            n,
            &pat.col_ptr,
            &pat.row_idx,
            &amd,
            ...
            Some(cand) => {
                fired += 1;
                if !is_bijection(&cand, n) {
                    println!("PRW\t{name}\tNON-BIJECTION");
                    continue;
                }
                let f = flops_of(&sp, &cand);
                if f < base {
                    won += 1;
                    println!(
                        "PRW\t{name}\tn={n}\t{base}->{f} (-{:.1} row-bips)",
                        (base - f) as f64 * 10000.0 / base as f64
                    );
                }
            }
    ...
    println!("PRW SUMMARY rows={rows} fired={fired} won={won}");
}
