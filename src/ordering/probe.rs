...
/// Post-chain simplicial-promotion rerun probe (same pipeline-gap argument as
/// probe_postchain_polish): the shipped simplicial pass runs before the
/// subtree chain. Rerun it on the final incumbent for the chain band
/// (n >= 1000) and report the full-corpus delta. Mirrors the shipped gates.
#[test]
#[ignore]
fn probe_postchain_simplicial() {
    const SP_MAX_N: usize = 6_000;
    const SP_MAX_NNZ: usize = 100_000;
    const SP_MAX_DENSITY: usize = 24;
    const SP_OPS: i64 = 64_000_000;
    ...
    for (name, pat) in &corpus {
        ...
        let mut out_f = inc_f;
        if n >= 1_000
            && (3..=SP_MAX_N).contains(&n)
            && nnz > 0
            && nnz <= SP_MAX_NNZ
            && nnz <= n.saturating_mul(SP_MAX_DENSITY)
        {
    ...
    let pol_s = aggregate(&pol.0, &pol.1);
    println!("\nshipped order(): {cur_s:.6}");
    println!("+ post-chain simplicial: {pol_s:.6}  d {:.6}", pol_s - cur_s);
    println!("improved {} matrices", improved.len());
    improved.sort_by(|a, b| {
        (a.4 as f64 / a.3 as f64)
            .partial_cmp(&(b.4 as f64 / b.3 as f64))
            .unwrap()
    });
    for (name, n, nnz, cf, xf) in improved.iter().take(15) {
        println!(
            "  {name:34} n={n:<7} nnz={nnz:<8} {cf} -> {xf} ({:.4})",
            *xf as f64 / *cf as f64
        );
