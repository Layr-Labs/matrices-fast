...
/// Kernel census: how much smaller is the simplicial/almost-simplicial fixpoint
/// core than the production K=3 core, per row, and what does the kernel reduce
/// cost? Test-only.
#[test]
#[ignore]
fn probe_kernel_census() {
    let corpus = crate::corpus::corpus();
    let mut tot_k3 = 0usize;
    let mut tot_k = 0usize;
    let mut rows_shrunk = 0usize;
    let mut worst_secs = 0.0f64;
    for (name, pat) in &corpus {
        let sp = scoring_pattern(pat);
        let k3 = core_lift::reduce(&sp, 3, 60_000, 3_000_000);
        let k = core_lift::reduce_kernel(&sp, 3, 12, 60_000, 3_000_000, 40_000_000);
        let (Some(k3), Some(k)) = (k3, k) else { continue };
        let t0 = std::time::Instant::now();
        let _ = core_lift::reduce_kernel(&sp, 3, 12, 60_000, 3_000_000, 40_000_000);
        let secs = t0.elapsed().as_secs_f64();
        worst_secs = worst_secs.max(secs);
        tot_k3 += k3.core_n();
        tot_k += k.core_n();
        let shrink = 100 - (k.core_n() * 100) / k3.core_n().max(1);
        if shrink >= 5 {
            rows_shrunk += 1;
            println!(
                "KERNEL\t{name}\tn={}\tnnz={}\tK3core={}\tKcore={}\tshrink={shrink}%\tkernel={secs:.3}s",
                pat.n, pat.nnz(), k3.core_n(), k.core_n()
            );
        }
    }
    println!(
        "KERNEL-TOTAL\tk3={tot_k3}\tkernel={tot_k}\trows_shrunk>=5%={rows_shrunk}\tworst_call={worst_secs:.3}s"
    );
}
