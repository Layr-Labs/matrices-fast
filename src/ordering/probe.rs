...
/// `gt_10k` residual-core census + relabelled-core lottery value probe (r11).
///
/// The shipped relabelled-core ticket block (`mod.rs`, "Small residual cores
/// permit a few independent tie-break orders") is gated
/// `(1_000..10_000).contains(&n) && nnz <= 50_000`, so it has never run on a
/// `gt_10k` row. This probe (a) censuses the K=3 residual core of every row
/// above `SSI_LOT_MIN_N` and (b) prices an i.i.d. relabelled core lottery
/// there, ranked on the EXACT core objective and spliced by the split identity
/// `prefix_flops + flops(core, p)`.
///
/// Env: `SSI_LOT_MIN_N` (10_000), `SSI_LOT_SEEDS` (0 = census only),
/// `SSI_LOT_MAX_CN` (60_000), `SSI_LOT_MAX_CORE_NNZ` (3_000_000),
/// `SSI_PROBE_ONLY` (comma-separated row names).
#[test]
#[ignore]
fn probe_gt10k_core_lottery() {
    let corpus = match std::env::var("SSI_CORPUS_FILE") {
        Ok(path) if !path.trim().is_empty() => {
            ssi_scoring::load_corpus_jsonl(std::path::Path::new(&path))
                .unwrap_or_else(|_| crate::corpus::corpus())
    ...
    let min_n = env_usize("SSI_LOT_MIN_N", 10_000);
    let seeds = env_usize("SSI_LOT_SEEDS", 0);
    let max_cn = env_usize("SSI_LOT_MAX_CN", 60_000);
    let max_core_nnz = env_usize("SSI_LOT_MAX_CORE_NNZ", 3_000_000);
    let only: Option<std::collections::HashSet<String>> = std::env::var("SSI_PROBE_ONLY")
        .ok()
        .map(|v| v.split(',').map(|x| x.trim().to_string()).collect());

    const ALPHAS: [f64; 4] = [0.5, 2.5, 5.0, 10.0];
    let mut log_now = 0.0f64;
    let mut log_new = 0.0f64;
    let mut count = 0usize;
    let mut wins = 0usize;
    let mut total_lot = 0.0f64;
    let mut worst_lot = 0.0f64;
    let mut worst_name = String::new();
    println!("row\tn\tnnz\tcn\tcore_nnz\tprefix_share\tratio_now\tratio_lot\tgain_pct\torder_s\tlot_s\twinner");
    for (name, pat) in &corpus {
        let n = pat.n;
        if n < min_n {
            continue;
        ...
        let nnz = pat.nnz();
        let sp = scoring_pattern(pat);
        let (cp0, ri0) = core_of(pat);
        let anchor = feral_ordering_core::CscPattern::new(n, &cp0, &ri0).unwrap();
        let base = flops_of(
            &sp,
            &feral_amd::amd_order(&anchor)
                .unwrap()
                .into_iter()
                .map(|x| x as usize)
                .collect::<Vec<_>>(),
        ...
        let cur = flops_of(&sp, &perm);
        let ratio_now = cur as f64 / base;

        let lifted = core_lift::reduce(
            &sp,
            REDUCE_ROW_DEG,
            REDUCE_MAX_CORE_N,
            REDUCE_MAX_CORE_EDGES,
        );
        let Some(cl) = lifted else {
            println!("{name}\t{n}\t{nnz}\t-\t-\t-\t{ratio_now:.6}\t-\t-\t{order_s:.3}\t-\t-");
            log_now += ratio_now.ln();
            log_new += ratio_now.ln();
            count += 1;
            continue;
        };
        let cn = cl.core_n();
        let cnnz = cl.core_nnz();
        let share = cl.prefix_flops as f64 / cur.max(1) as f64;
        let mut best = cur;
        let mut winner = String::from("-");
        let mut lot_s = 0.0f64;
        if seeds > 0 && cn >= 8 && cn <= max_cn && cnnz <= max_core_nnz {
            let core_pat = ScoringPattern {
                n: cn,
                col_ptr: cl.core_col_ptr.clone(),
                row_idx: cl.core_row_idx.clone(),
            ...
            for s in 1..=seeds as u64 {
                let q = relabel(cn, s);
                let rl = permute_pattern(&core_pat, &q);
                let rp: Vec<i32> = rl.col_ptr.iter().map(|&x| x as i32).collect();
                let ri: Vec<i32> = rl.row_idx.iter().map(|&x| x as i32).collect();
                let Some(rcore) = feral_ordering_core::CscPattern::new(cn, &rp, &ri) else {
                    continue;
                };
                for k in 0..=ALPHAS.len() {
                    let raw: Option<Vec<i32>> = if k < ALPHAS.len() {
                        let o = feral_amf::AmfOptions { dense_alpha: ALPHAS[k], ..Default::default() };
                        feral_amf::amf_order_opts(&rcore, &o).ok().map(|(p, ..)| p)
                    } else {
                        feral_amd::amd_order_opts(&rcore, &feral_amd::AmdOptions::default())
                            .ok()
                            .map(|(p, ..)| p)
                    };
                    let Some(raw) = raw else { continue };
                    let core_perm: Vec<usize> = raw.into_iter().map(|v| q[v as usize]).collect();
                    if !is_bijection(&core_perm, cn) {
                        continue;
                    }
                    let f = cl.prefix_flops + flops_of(&core_pat, &core_perm);
                    if f < best {
                        best = f;
                        winner = if k < ALPHAS.len() {
                            format!("amf{}@s{}", ALPHAS[k], s)
                        } else {
                            format!("amd@s{s}")
                        };
                    }
            ...
            lot_s = t1.elapsed().as_secs_f64();
        }
        let ratio_lot = best as f64 / base;
        let gain = (cur as f64 - best as f64) / cur.max(1) as f64 * 100.0;
        if best < cur {
            wins += 1;
        }
        total_lot += lot_s;
        ...
        log_now += ratio_now.ln();
        log_new += ratio_lot.ln();
        count += 1;
        println!(
            "{name}\t{n}\t{nnz}\t{cn}\t{cnnz}\t{share:.3}\t{ratio_now:.6}\t{ratio_lot:.6}\t{gain:.3}\t{order_s:.3}\t{lot_s:.3}\t{winner}"
        );
    }
    let geo_now = (log_now / count as f64).exp();
    let geo_new = (log_new / count as f64).exp();
    println!("\nROWS = {count}   WINS = {wins}");
    println!("GEO_NOW = {geo_now:.6}   GEO_LOT = {geo_new:.6}");
    println!(
        "DEV_BIPS = {:.5}   (0.40 * (geo_now - geo_lot) * 10000)",
        0.40 * (geo_now - geo_new) * 10_000.0
    );
    println!("LOT_TOTAL = {total_lot:.2} s   LOT_WORST = {worst_lot:.3} s on {worst_name}");
}

/// Price EXTRA relabelled-lottery tickets on `gt_10k` rows (r11).
///
/// Production draws `relabel_restarts_tuned(...)` AMD tickets at seeds
/// `1..=restarts`, and the same seeds for AMF only when
/// `nnz <= RELABEL_AMF_MAX_NNZ`. Both counts are set by `budget / nnz`, so the
/// bucket with the largest `nnz` — `gt_10k` — is the one the law starves, and
/// six of the sixteen `gt_10k` rows at AMD ratio >= 0.94 sit above the AMF
/// ceiling and draw ZERO fill-objective tickets.
///
/// This probe continues each lottery past where production stops and reports
/// the marginal value curve and the marginal per-ticket cost.
///
/// Env: `SSI_TIX_MIN_N` (10_000), `SSI_TIX_EXTRA` (8), `SSI_PROBE_ONLY`.
#[test]
#[ignore]
fn probe_gt10k_tickets() {
    let corpus = match std::env::var("SSI_CORPUS_FILE") {
        Ok(path) if !path.trim().is_empty() => {
            ssi_scoring::load_corpus_jsonl(std::path::Path::new(&path))
                .unwrap_or_else(|_| crate::corpus::corpus())
    ...
    let min_n = env_usize("SSI_TIX_MIN_N", 10_000);
    let extra = env_usize("SSI_TIX_EXTRA", 8);
    let only: Option<std::collections::HashSet<String>> = std::env::var("SSI_PROBE_ONLY")
        .ok()
        .map(|v| v.split(',').map(|x| x.trim().to_string()).collect());

    let mut log_now = 0.0f64;
    let mut log_amd = 0.0f64;
    let mut log_amf = 0.0f64;
    let mut log_both = 0.0f64;
    let mut count = 0usize;
    println!("row\tn\tnnz\tmax_deg\tprod_amd_tix\tprod_amf_tix\tratio_now\tamd_r\tamd_s\tamd_first\tamf_r\tamf_s\tamf_first\tbest_r");
    for (name, pat) in &corpus {
        let n = pat.n;
        if n < min_n {
            continue;
        ...
        let nnz = pat.nnz();
        let max_deg = (0..n).map(|v| pat.col_ptr[v + 1] - pat.col_ptr[v]).max().unwrap_or(0);
        let (b, c) = relabel_budget_and_cap(n);
        let prod_amd = relabel_restarts_tuned(b, c, n, nnz, max_deg);
        let prod_amf = if nnz <= RELABEL_AMF_MAX_NNZ {
            if n >= 10_000 && nnz >= 100_000 { prod_amd.min(8) } else { prod_amd }
        } else {
            0
        };
        let sp = scoring_pattern(pat);
        ...
        // Extra AMD tickets, continuing production's seed stream.
        let mut best_amd = cur;
        let mut amd_first = 0usize;
        let t = Instant::now();
        for r in prod_amd..prod_amd + extra {
            let seed = r as u64 + 1;
            let q = relabel(n, seed);
            let bp = permute_pattern(&sp, &q);
            let bcp: Vec<i32> = bp.col_ptr.iter().map(|&x| x as i32).collect();
            let bri: Vec<i32> = bp.row_idx.iter().map(|&x| x as i32).collect();
            let Some(bcore) = feral_ordering_core::CscPattern::new(n, &bcp, &bri) else { continue };
            let Ok((pb, ..)) = feral_amd::amd_order_opts(&bcore, &amd_configs[r % amd_configs.len()]) else { continue };
            let p: Vec<usize> = pb.into_iter().map(|x| q[x as usize]).collect();
            if !is_bijection(&p, n) { continue; }
            let f = flops_of(&sp, &p);
            if f < best_amd {
                best_amd = f;
                if amd_first == 0 { amd_first = r - prod_amd + 1; }
            }
        }
        let amd_s = t.elapsed().as_secs_f64();

        // Extra AMF tickets, from seed 1 where production draws none.
        let mut best_amf = cur;
        let mut amf_first = 0usize;
        let t = Instant::now();
        for r in prod_amf..prod_amf + extra {
            let seed = r as u64 + 1;
            let q = relabel(n, seed);
            let bp = permute_pattern(&sp, &q);
            let bcp: Vec<i32> = bp.col_ptr.iter().map(|&x| x as i32).collect();
            let bri: Vec<i32> = bp.row_idx.iter().map(|&x| x as i32).collect();
            let Some(bcore) = feral_ordering_core::CscPattern::new(n, &bcp, &bri) else { continue };
            let o = feral_amf::AmfOptions { dense_alpha: amf_alphas[r % amf_alphas.len()], ..Default::default() };
            ...
            if f < best_amf {
                best_amf = f;
                if amf_first == 0 { amf_first = r - prod_amf + 1; }
            }
        }
        let amf_s = t.elapsed().as_secs_f64();

        let best = best_amd.min(best_amf);
        let (ra, rf, rb) = (best_amd as f64 / base, best_amf as f64 / base, best as f64 / base);
        log_now += ratio_now.ln();
        log_amd += ra.ln();
        log_amf += rf.ln();
        log_both += rb.ln();
        count += 1;
        println!(
            "{name}\t{n}\t{nnz}\t{max_deg}\t{prod_amd}\t{prod_amf}\t{ratio_now:.6}\t{ra:.6}\t{amd_s:.3}\t{amd_first}\t{rf:.6}\t{amf_s:.3}\t{amf_first}\t{rb:.6}"
        );
    }
    let g = |l: f64| (l / count as f64).exp();
    println!("\nROWS = {count}   EXTRA_TICKETS = {extra}");
    println!("GEO_NOW = {:.6}  GEO_AMD = {:.6}  GEO_AMF = {:.6}  GEO_BOTH = {:.6}", g(log_now), g(log_amd), g(log_amf), g(log_both));
    println!("DEV_BIPS_AMD = {:.5}", 0.40 * (g(log_now) - g(log_amd)) * 10_000.0);
    println!("DEV_BIPS_AMF = {:.5}", 0.40 * (g(log_now) - g(log_amf)) * 10_000.0);
    println!("DEV_BIPS_BOTH = {:.5}", 0.40 * (g(log_now) - g(log_both)) * 10_000.0);
}

/// Price the PARTITIONER family on `gt_10k` rows it is gated out of (r11).
///
/// METIS default stops at `n < 130_000 && nnz < 320_000`, tuned METIS at
/// `21_000 / 120_000`, Scotch at `12_000 / 200_000` and KaHIP at `6_000 /
/// 50_000`, so the largest `gt_10k` rows — the ones with the most unspent
/// envelope — receive NO nested-dissection candidate at all, only the
/// minimum-degree / minimum-fill family. Nested dissection is the one
/// asymptotically different objective in the portfolio.
///
/// Env: `SSI_PART_MIN_N` (10_000), `SSI_PROBE_ONLY`.
#[test]
#[ignore]
fn probe_gt10k_partitioners() {
    let corpus = match std::env::var("SSI_CORPUS_FILE") {
        Ok(path) if !path.trim().is_empty() => {
            ssi_scoring::load_corpus_jsonl(std::path::Path::new(&path))
                .unwrap_or_else(|_| crate::corpus::corpus())
    ...
    let min_n: usize = std::env::var("SSI_PART_MIN_N").ok().and_then(|v| v.parse().ok()).unwrap_or(10_000);
    let only: Option<std::collections::HashSet<String>> = std::env::var("SSI_PROBE_ONLY")
        .ok()
        .map(|v| v.split(',').map(|x| x.trim().to_string()).collect());
    let mut log_now = 0.0f64;
    let mut log_best = 0.0f64;
    let mut count = 0usize;
    println!("row\tn\tnnz\tin_metis\tratio_now\tmd_r\tmd_s\tmt_r\tmt_s\tmh_r\tmh_s\tsc_r\tsc_s\tbest_r\tbest_who");
    for (name, pat) in &corpus {
        let n = pat.n;
        if n < min_n { continue; }
        if let Some(set) = &only { if !set.contains(name) { continue; } }
        let nnz = pat.nnz();
        let sp = scoring_pattern(pat);
        let (cp0, ri0) = core_of(pat);
        let anchor = feral_ordering_core::CscPattern::new(n, &cp0, &ri0).unwrap();
        let base = flops_of(
            &sp,
            &feral_amd::amd_order(&anchor).unwrap().into_iter().map(|x| x as usize).collect::<Vec<_>>(),
        ) as f64;
        let cur = flops_of(&sp, &order(pat));
        let ratio_now = cur as f64 / base;
        let in_metis = (n < METIS_MAX_N && nnz < METIS_MAX_NNZ) as usize;

        let mut best = cur;
        let mut who = String::from("-");
        let mut run = |label: &str, f: &dyn Fn() -> Option<Vec<i32>>, best: &mut u64, who: &mut String| -> (f64, f64) {
            let t = Instant::now();
            let r = f();
            let s = t.elapsed().as_secs_f64();
            match r {
                Some(p) => {
                    let p: Vec<usize> = p.into_iter().map(|x| x as usize).collect();
                    if !is_bijection(&p, n) { return (f64::NAN, s); }
                    let fl = flops_of(&sp, &p);
                    if fl < *best { *best = fl; *who = label.to_string(); }
                    (fl as f64 / base, s)
                }
                None => (f64::NAN, s),
            }
        ...
        let (md_r, md_s) = {
            let core = feral_ordering_core::CscPattern::new(n, &cp0, &ri0).unwrap();
            run("metis", &|| feral_metis::metis_order_full(&core, &feral_metis::MetisOptions::default()).ok().map(|(p, _, _)| p), &mut best, &mut who)
        };
        let (mt_r, mt_s) = {
            let core = feral_ordering_core::CscPattern::new(n, &cp0, &ri0).unwrap();
            let o = feral_metis::MetisOptions { niparts: 16, fm_passes: 20, ..Default::default() };
            run("metis_tuned", &|| feral_metis::metis_order_full(&core, &o).ok().map(|(p, _, _)| p), &mut best, &mut who)
        };
        let (mh_r, mh_s) = {
            let core = feral_ordering_core::CscPattern::new(n, &cp0, &ri0).unwrap();
            let o = feral_metis::MetisOptions { niparts: 32, fm_passes: 30, ..Default::default() };
            run("metis_hitrial", &|| feral_metis::metis_order_full(&core, &o).ok().map(|(p, _, _)| p), &mut best, &mut who)
        };
        let (sc_r, sc_s) = {
            let core = feral_ordering_core::CscPattern::new(n, &cp0, &ri0).unwrap();
            run("scotch", &|| feral_scotch::scotch_order(&core).ok(), &mut best, &mut who)
        };
        let best_r = best as f64 / base;
        log_now += ratio_now.ln();
        log_best += best_r.ln();
        count += 1;
        println!("{name}\t{n}\t{nnz}\t{in_metis}\t{ratio_now:.6}\t{md_r:.6}\t{md_s:.3}\t{mt_r:.6}\t{mt_s:.3}\t{mh_r:.6}\t{mh_s:.3}\t{sc_r:.6}\t{sc_s:.3}\t{best_r:.6}\t{who}");
    }
    let g = |l: f64| (l / count as f64).exp();
    println!("\nROWS = {count}");
    println!("GEO_NOW = {:.6}   GEO_BEST = {:.6}", g(log_now), g(log_best));
    println!("DEV_BIPS = {:.5}", 0.40 * (g(log_now) - g(log_best)) * 10_000.0);
}

/// Price a FINAL subtree-refinement round appended after the whole pipeline (r11).
///
/// Production runs the subtree chain (`n <= SUBTREE_CHAIN_MAX_N`) and the
/// terminal deep pass (`n <= 80_000 && nnz <= 250_000`) at stage 3, BEFORE
/// core-lift, the completion watcher, the small-graph polish, the terminal PEO
/// chain and the alternate-seed chains. Every later stage can replace the
/// incumbent, and the replacement is never refined. This probe re-runs the two
/// existing configs on the FINAL `order()` result, ungated, and prices the
/// result with the harness's own bucketed aggregation.
///
/// Refinement is strictly monotone (each acceptance is a strict decrease of the
/// exact objective), so only TIME is at stake.
///
/// Env: `SSI_REF_MIN_N` (0), `SSI_REF_ARM` (`both`|`chain`|`deep`), `SSI_PROBE_ONLY`.
#[test]
#[ignore]
fn probe_gt10k_refine() {
    let corpus = match std::env::var("SSI_CORPUS_FILE") {
        Ok(path) if !path.trim().is_empty() => {
            ssi_scoring::load_corpus_jsonl(std::path::Path::new(&path))
                .unwrap_or_else(|_| crate::corpus::corpus())
    ...
    let min_n: usize = std::env::var("SSI_REF_MIN_N").ok().and_then(|v| v.parse().ok()).unwrap_or(0);
    let arm = std::env::var("SSI_REF_ARM").unwrap_or_else(|_| "both".to_string());
    let only: Option<std::collections::HashSet<String>> = std::env::var("SSI_PROBE_ONLY")
        .ok()
        .map(|v| v.split(',').map(|x| x.trim().to_string()).collect());
    let mut log_now = [0.0f64; 3];
    let mut log_new = [0.0f64; 3];
    let mut counts_b = [0usize; 3];
    let mut wins = 0usize;
    let mut post_breaks = 0usize;
    let mut tot_s = 0.0f64;
    let mut worst_s = 0.0f64;
    let mut worst_name = String::new();
    println!("row\tn\tnnz\tin_chain\tin_deep\tratio_now\tchain_r\tchain_s\tdeep_r\tdeep_s\tbest_r\tgain_pct\tsetup_s");
    for (name, pat) in &corpus {
        let n = pat.n;
        if n < min_n || n == 0 { continue; }
        if let Some(set) = &only { if !set.contains(name) { continue; } }
        let nnz = pat.nnz();
        let sp = scoring_pattern(pat);
        let (cp0, ri0) = core_of(pat);
        ...
        let base = amd_flops as f64;
        let best_perm = order(pat);
        let cur = flops_of(&sp, &best_perm);
        let ratio_now = cur as f64 / base;
        let in_chain = ((SUBTREE_MIN_N..=SUBTREE_CHAIN_MAX_N).contains(&n) && nnz <= 1_500_000) as usize;
        let in_deep = ((SUBTREE_MIN_N..=80_000).contains(&n) && nnz <= 250_000) as usize;

        // Postorder setup, exactly as both production call sites build it.
        ...
        let post_etree = EliminationTree::from_pattern(&post_pattern);
        let counts: Vec<u32> = column_counts_gnp(&post_pattern, &post_etree).into_iter().map(|c| c as u32).collect();
        let parent: Vec<i32> = post_etree.parent.iter().map(|p| p.map_or(-1, |j| j as i32)).collect();
        let setup_s = t_setup.elapsed().as_secs_f64();
        // Postorder must be objective-neutral; both production call sites assume it.
        if flops_of(&sp, &base_cand) != cur { post_breaks += 1; }

        let mut best = cur;
        let mut run = |cfg: rgreedy::SubCfg, best: &mut u64| -> (f64, f64) {
            let mut cand = base_cand.clone();
            let t = Instant::now();
            let improved = rgreedy::subtree_refine(n, &pat.col_ptr, &pat.row_idx, &mut cand, &counts, &parent, cfg);
            let s = t.elapsed().as_secs_f64();
        ...
        let (chain_r, chain_s) = if arm != "deep" { run(subtree_cfg_for(n, nnz), &mut best) } else { (ratio_now, 0.0) };
        let (deep_r, deep_s) = if arm != "chain" { run(terminal_deep_subtree_cfg(n, nnz, cur, amd_flops), &mut best) } else { (ratio_now, 0.0) };
        let best_r = best as f64 / base;
        let gain = (cur as f64 - best as f64) / cur.max(1) as f64 * 100.0;
        if best < cur { wins += 1; }
        let b = bucket(n);
        log_now[b] += ratio_now.ln();
        log_new[b] += best_r.ln();
        counts_b[b] += 1;
        let row_s = setup_s + chain_s + deep_s;
        tot_s += row_s;
        if row_s > worst_s { worst_s = row_s; worst_name = name.clone(); }
        println!("{name}\t{n}\t{nnz}\t{in_chain}\t{in_deep}\t{ratio_now:.6}\t{chain_r:.6}\t{chain_s:.3}\t{deep_r:.6}\t{deep_s:.3}\t{best_r:.6}\t{gain:.3}\t{setup_s:.3}");
    }
    let s_now = aggregate(&log_now, &counts_b);
    let s_new = aggregate(&log_new, &counts_b);
    println!("\nARM = {arm}   WINS = {wins}   POSTORDER_BREAKS = {post_breaks}");
    for b in 0..3 {
        if counts_b[b] == 0 { continue; }
        println!("  {:<8} n={:<4} geo_now {:.6} -> {:.6}", BUCKET_NAMES[b], counts_b[b], (log_now[b]/counts_b[b] as f64).exp(), (log_new[b]/counts_b[b] as f64).exp());
    }
    println!("SCORE_NOW = {s_now:.9}   SCORE_NEW = {s_new:.9}");
    println!("DEV_BIPS = {:.5}", (s_now - s_new) * 10_000.0);
    println!("ADDED_TOTAL = {tot_s:.2} s   ADDED_WORST = {worst_s:.3} s on {worst_name}");
}
