...
fn probe_retained_independent_stress() {
    ...
    for (name,pat) in fixtures {
        ...
        for pair in 0..2 { for offset in 0..2 {
            ...
            set_medium_batch_enabled(if budget_mode {false} else if frontier_mode {arm==0} else {cap_mode ||state_mode ||arm==1});
        ...
        if !cap_mode &&!state_mode &&!frontier_mode &&!budget_mode &&medium_drops[1]==0 {
