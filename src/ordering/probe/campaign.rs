...
fn probe_runtime_batch_campaign() {
    let text=std::fs::read_to_string("/tmp/matrices-fast-lex-campaign-seeds.tsv").unwrap();
    ...
    assert!(cache.is_empty());assert!((aggregate(&base,&counts)-0.790519213141).abs()<1e-11);
...
fn probe_retained_independent_stress() {
    ...
    for (name,pat) in fixtures {
        ...
        for pair in 0..2 { for offset in 0..2 {
            ...
            set_medium_batch_enabled(cap_mode ||arm==1);take_medium_batch_drops();
    ...
    peo_extract::set_indexed_priority(true);
    post_core::set_lex_enabled(true);
    set_medium_batch_enabled(true);take_medium_batch_drops();
    if cap_mode {
