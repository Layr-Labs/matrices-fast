...
#[test]
#[ignore]
fn probe_original_linked_block_stress() {
    for (block,links) in [(32usize,3usize),(32,4),(24,4),(48,3)] {
        let groups=16_384/block+1;let n=groups*block;let mut edges=Vec::new();
        let mut state=0x378a_1597_d46f_u64^block as u64^links as u64;
        for group in 0..groups {
            let base=group*block;
            for k in 0..block {
                if k+1<block {edges.push((base+k,base+k+1));}
                for _ in 0..links {
                    state^=state<<13;state^=state>>7;state^=state<<17;
                    let u=state as usize%block;
                    if u!=k {edges.push((base+k.min(u),base+k.max(u)));}
                }
            }
            if group!=0 {edges.push((base-block,base));edges.push((base-block+1,base+1));}
        }
        ...
        let mut minimum=[f64::MAX;2];let mut flags=[0;2];let mut fill=0;
        for pair in 0..2 {for offset in 0..2 {
            let arm=(pair+offset)%2;set_original_large_research(Some(arm==1));set_original_second_research(Some(false));
            take_completion_win_flags();let start=Instant::now();let p=order(&pat);
            minimum[arm]=minimum[arm].min(start.elapsed().as_secs_f64());let flag=take_completion_win_flags();
            assert!(is_bijection(&p,n));let q=ws.flops(&sp,&p);assert!(q<=af);
            if arm==0 {fill=ws.nnz_l();}
            if pair!=0 {assert_eq!(p,outputs[arm]);assert_eq!(flag,flags[arm]);}
            outputs[arm]=p;f[arm]=q;flags[arm]=flag;
        }}
        assert!(f[1]<=f[0]);assert_eq!(flags[1]&8,0);
        if flags[1]&4==0 {assert_eq!(outputs[0],outputs[1]);}
        println!("ORIGINAL_BLOCK_STRESS\tblock{block}_links{links}\t{n}\t{}\t{af}\t{}\t{}\t{fill}\t{:.6}\t{:.6}\t{}",
            pat.nnz(),f[0],f[1],minimum[0],minimum[1],flags[1]);
    }
...
fn probe_original_structured_mesh_stress() {
    ...
    for variant in 0..4 {for (w,h) in [(128usize,128usize),(96,192)] {
        ...
        for pair in 0..2 {for offset in 0..2 {
            let arm=(pair+offset)%2;set_original_large_research(Some(depth_mode ||arm==1));set_original_second_research(Some(depth_mode));
