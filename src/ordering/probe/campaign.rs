...
fn probe_funded_kernel_corpus() {
    ...
    let result=aggregate(&sums,&counts);assert!((result-0.791703147252).abs()<1e-11);
...
fn apply_step(pat: &Pattern, p: &mut Vec<usize>, f: &mut u64,
    ...
    if *f > 20_000_000_000 || ws.nnz_l() > 150_000 { return; }
    match step {
        Step::Gate(_) => {},
        ...
        Step::Priority(policy, rounds) => for _ in 0..rounds {
            ...
            if let Some(candidate) = peo_extract::priority_candidate_bounded(pat.n,
...
fn probe_retained_independent_stress() {
    ...
    for (name,pat) in fixtures {
        ...
        for pair in 0..2 { for offset in 0..2 {
            ...
            indep_first::set_extra_enabled(arm==1);
            let t = Instant::now(); let p = order(&pat);
        ...
        assert!(flops[1]<=flops[0],"{name}");
        if old_factor>200_000 ||flops[0]>20_000_000_000 {
