...
fn probe_selected_original_incidence_bonus() {
    ...
    let mut cache:std::collections::BTreeMap<String,Vec<usize>>=
        std::fs::read_to_string("/tmp/matrices-fast-state-gated-completion-campaign-seeds.tsv")
        .unwrap().lines().map(|line| {
            let (name,p)=line.split_once('\t').unwrap();
            (name.to_owned(),p.split(',').map(|v|v.parse().unwrap()).collect())
        }).collect();
    let flags:std::collections::BTreeMap<String,u8>=std::fs::read_to_string(root.join("0217-completion-state-flags.tsv"))
        .unwrap().lines().skip(1).map(|line| {let (name,f)=line.split_once('\t').unwrap();
            (name.to_owned(),f.parse().unwrap())}).collect();
    let refs:std::collections::BTreeMap<String,u64>=std::fs::read_to_string(root.join("0217-final-screen.tsv"))
    ...
    for (name,pat) in crate::corpus::corpus() {
        ...
        let admitted=flags[&name]&1!=0 &&amd<=post_core::AMD_WORK_LIMIT;
        ...
        println!("SELECTED_ORIGINAL_ROW\t{name}\t{}\t{}\t{amd}\t{initial}\t{fill}\t{f}\t{elapsed:.6}\t{}",
            pat.n,pat.nnz(),flags[&name]);
    }
    assert!(cache.is_empty());assert!((aggregate(&base,&counts)-0.790176632480).abs()<1e-11);
    assert!((aggregate(&sums,&counts)-0.790098120936).abs()<1e-11);
    std::fs::write("/tmp/matrices-fast-selected-original-bonus-campaign-seeds.tsv",output).unwrap();
...
fn probe_retained_independent_stress() {
    ...
    for (name,pat) in fixtures {
        ...
        for pair in 0..2 { for offset in 0..2 {
            ...
            set_medium_batch_enabled(if frontier_mode {arm==0} else {cap_mode ||state_mode ||arm==1});
        ...
        if !cap_mode &&!state_mode &&!frontier_mode &&medium_drops[1]==0 {
