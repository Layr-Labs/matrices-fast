...
pub(super) fn refine(pattern: &Pattern, p: &[usize]) -> Option<Vec<usize>> {
    if !(64..=80_000).contains(&pattern.n)
        || pattern.nnz() > 500_000
    {
        return None;
    }
...
fn refine_impl(
    ...
    if tree.cost < cost {
        let out = tree.output();
        #[cfg(test)]
        assert_eq!(super::flops_of(&sp, &out), tree.cost);
        Some(out)
    } else {
        None
    }
...
mod tests {
    use super::*;
    #[test]
    fn lifted_tree_cost_matches_independent_symbolic_score() {
        let n = 1024;
        let mut a = vec![Vec::new(); n];
        for v in 0..n {
            for u in [
                if v % 32 < 31 { v + 1 } else { v },
                if v + 32 < n { v + 32 } else { v },
            ] {
                if u != v {
                    a[v].push(u);
                    a[u].push(v);
        ...
        let mut successes = 0;
        for shift in [0, 31] {
            let p: Vec<_> = (0..n).map(|v| (v * 257 + shift) % n).collect();
            if let Some(q) = refine(&pattern, &p) {
                assert!(super::super::is_bijection(&q, n));
                assert!(super::super::flops_of(&sp, &q) < super::super::flops_of(&sp, &p));
                successes += 1;
            }
        }
        assert!(successes > 0);
        let base: Vec<_> = (0..n).map(|v| (v * 257) % n).collect();
        let improved = super::super::regions::refine(&pattern, &base)
            .expect("region improvement on shuffled grid");
        assert!(super::super::flops_of(&sp, &improved) < super::super::flops_of(&sp, &base));
        let donor: Vec<_> = (0..n).collect();
        if let Some(q) = super::super::regions::crossover(&pattern, &base, &donor) {
            assert!(super::super::flops_of(&sp, &q) < super::super::flops_of(&sp, &base));
        }
        let mut b = vec![Vec::new(); 80];
        for u in 0..80 {
            for v in u + 1..80 {
                if (u >= 64 && v >= 64) || v == u + 1 || (u * 31 + v * 17) % 29 == 0 {
                    b[u].push(v);
                    b[v].push(u);
                }
            }
        ...
        let mut p: Vec<_> = (0..64).map(|v| (v * 17) % 64).collect();
        p.extend(64..80);
        if let Some(q) = boundary(&local, &p, 64) {
            assert!(q[..64].iter().all(|&v| v < 64));
            assert!(q[64..].iter().all(|&v| v >= 64));
        }
    }
}
