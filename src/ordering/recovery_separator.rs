...
fn greedy_options(
    ...
    for step in 0..k {
        candidates.clear();
        candidates.extend((0..k).filter(|&v| live[v]));
        let count = if mode == 0 { 1 } else { shortlist };
        if candidates.len() > count {
            candidates.select_nth_unstable_by_key(count, |&v| (degree[v], v));
...
fn propose(
    ...
    let (children, g, total) = match prepared {
        ...
        None => tree.compress_reusing_map(root, vertices, &mut work)?,
    };
    let k = vertices.len();
    let original = tree.original(vertices);
...
pub(super) fn recovered(
    ...
    let full: Vec<usize> = if cfg.full_tasks >= tasks.len() {
        ...
        let cheap = super::parallel::map_indexed(&tasks, cfg.threads, |_, (root, vertices)| {
            propose_cheap(&tree, *root, vertices, cfg, allowance)
        });
