...
impl SixWindow {
    fn new(game: &Game<'_>, verts: [usize; 6], work: &mut TripleWork) -> Option<Self> {
        ...
        // Width queries never join distinct components. Keep the subset order
        // ascending so each union's remove-lowest-bit predecessor is ready.
        let mut needed = [0usize; 63];
        let mut needed_count = 0;
        for mask in 1usize..64 {
            if components[..component_count].iter().any(|&c| mask & c as usize == mask)
                && mask.count_ones() > 1 {
                needed[needed_count] = mask;
                needed_count += 1;
            }
        }
        ...
        for word in 0..game.w {
            ...
            for i in 0..6 { unions[1 << i] = rows[i][word]; }
            for &mask in &needed[..needed_count] {
        ...
        Some(Self { internal_union, component_width, components, component_count })
    }
    ...
    fn solve(&self) -> ([usize; 6], u64, u64) {
        let mut sequences = [[0usize; 6]; 6];
        let mut lengths = [0usize; 6];
        let mut optimum = 0;
        for c in 0..self.component_count {
            let component = self.components[c] as usize;
            let length = component.count_ones() as usize;
            lengths[c] = length;
            if length == 1 {
                let pivot = component.trailing_zeros() as usize;
                sequences[c][0] = pivot;
                let width = self.width(0, pivot);
                optimum += width * width;
                continue;
            }
            let mut best = [u64::MAX; 64];
            // Six base-eight digits need eighteen bits.
            let mut path = [u32::MAX; 64];
            best[0] = 0;
            path[0] = 0;
            for mask in 0usize..component {
                if mask & component != mask { continue; }
                let mut available = component & !mask;
                while available != 0 {
            ...
            optimum += best[component];
            for i in 0..length {
                sequences[c][i] = ((path[component] >> (3 * (length - i - 1))) & 7) as usize;
            }
        }
        ...
        if optimum < incumbent {
            // Independent components commute, so merge their optimal sequences
            // by smallest available head for global lexicographic tie-breaking.
            let mut positions = [0usize; 6];
            for slot in &mut order {
                let mut chosen = 0;
                let mut pivot = usize::MAX;
                for c in 0..self.component_count {
                    if positions[c] < lengths[c] && sequences[c][positions[c]] < pivot {
                        chosen = c;
                        pivot = sequences[c][positions[c]];
                    }
                }
                *slot = pivot;
                positions[chosen] += 1;
...
mod five_window_tests {
    ...
    fn six_window_matches_explicit_elimination_oracle() {
        ...
        for sample in 0..18 {
            ...
            for u in 0..n {
                for v in u + 1..n {
                    let include = match sample {
                        // Separate triples sharing live external neighbors:
                        // those neighbors must not couple component DP states.
                        16 => u >= 2 && (v >= 8 || (u - 2) / 3 == (v - 2) / 3),
                        17 => false,
                        _ => xs64(&mut rng) % 16 <= sample,
                    };
                    if include {
                        edges.push((u, v));
                        graph[u][v] = true;
