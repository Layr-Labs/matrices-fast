...
impl<'a> Game<'a> {
    ...
    fn eliminate(&mut self, v: usize) -> u64 {
        let w = self.w;
        self.tmp.copy_from_slice(&self.adj[v * w..(v + 1) * w]);
        self.nlist.clear();
        for k in 0..w {
            let mut bits = self.tmp[k];
            while bits != 0 {
                self.nlist
        ...
        for i in 0..self.nlist.len() {
            ...
            let mut degree = 0;
            for k in 0..w {
                self.adj[u * w + k] |= self.tmp[k];
                degree += self.adj[u * w + k].count_ones();
            }
            self.adj[u * w + (u >> 6)] &= !(1u64 << (u & 63));
            self.adj[u * w + (v >> 6)] &= !(1u64 << (v & 63));
            degree -= 2;
    ...
    fn deficiency(&mut self, v: usize) -> u32 {
        ...
        for k in 0..w {
            ...
            while bits != 0 {
                ...
                let mut count = 0;
                for j in 0..w {
                    count += (self.tmp[j] & !self.adj[u * w + j]).count_ones();
                }
                missing += count - 1;
            }
        }
...
pub(crate) fn search(
    ...
    search_partial(n, &adj, n, seed, cost, budget, rng, 255, 2)
}
...
pub(crate) fn subtree_refine(
    ...
    let split = n >= 10_000 && cfg.rank_blocks && cfg.max_blocks == 64 && cfg.streams == 2;
    ...
    if cfg.rank_blocks {
        ...
        blocks.extend(
            ranked
                .into_iter()
                .take(if split { 96 } else { cfg.max_blocks })
                .map(|(a, b, _)| (a, b)),
        );
    }
    if blocks.is_empty() {
        ...
        let handles: Vec<_> = (0..threads)
            .map(|t| {
                scope.spawn(move || {
                    ...
                    for rank in (t..blocks.len()).step_by(threads) {
                        let (a, b) = blocks[rank];
                        let len = b + 1 - a;
                        ...
                        for k in usize::from(split && rank >= 32)..cfg.streams.max(1) {
                            let mut rng = if n >= 10_000 && k == 1 {
                                0xD1B5_4A32_D192_ED03
                            } else {
                                0x9E37_79B9_7F4A_7C15u64.wrapping_mul(2 * k as u64 + 1)
                                    ^ (k as u64) << 32
                            };
                            ...
                            let (mask, mode) = match k % 4 {
                                0 => (255, 2),
                                1 => (255, 0),
                                2 => (255, 1),
                                _ => (0b0011_0111, 2),
                            };
                            if let Some(result) = search_partial(
                                m,
                                &adj[..needed],
                                len,
                                &seed,
                        ...
                        if let Some((order, _)) = best {
                            if order.len() == len {
                                output
                                    .push((a, order.iter().map(|&i| verts[i]).collect::<Vec<_>>()));
                            }
                        }
                    }
                    output
