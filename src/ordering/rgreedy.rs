...
/// ties. VOL-RR5: THREE sequential windows per attempt (tunneling —
/// intermediate states are not exact-filtered, so the chain can pass
/// through worse states a hill-climb would reject). One shared TripleWork
/// budget across windows (aborts protect); game rebuilt per window.
