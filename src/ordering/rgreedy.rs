...
/// Ruin-and-reconstruct local search with greedy min-fill repair.
///
/// NEW generator (VOL-RR): destroys one deterministic strided window of the
/// incumbent (k = 16/32/64/128 by attempt mod 4) and rebuilds it greedily by
/// live deficiency (min-fill), lowest-index tie-breaks. Beyond-5-window
/// escape hatch: the shipped five-descent already owns every 5-window
/// exactly, so a fresh reconstruction only adds value at k >> 5; min-fill
/// repair (not min-degree) is what makes the rebuild structurally distinct
/// from what AMD-descended basins already contain. Fully deterministic
/// (fixed stride schedule, no RNG); budget-capped via TripleWork; returns
/// Some iff the rebuild differs from the seed (caller exact-scores/admits).
pub(crate) fn ruin_window_reconstruct(
    ...
    // Deterministic strided ruin location (fixed odd stride, no RNG).
    let start = attempt
        .wrapping_mul(9973)
        .wrapping_add(12345)
        % (n - k);
    let adj0 = Game::build_adj(n, col_ptr, row_idx)?;
    let mut game = Game::new(n, &adj0)?;
    // new() leaves the live-list empty; reset() populates it (mirrors the
    // descents, which reset per sweep).
    game.reset();
    if game.ops > budget {
        return None;
    }
    ...
    // Greedy min-fill reconstruction of the window set: at each step,
    // eliminate the remaining vertex of smallest live deficiency.
    let mut remaining: Vec<usize> = seed[start..start + k].to_vec();
    let mut order: Vec<usize> = Vec::with_capacity(k);
    while !remaining.is_empty() {
        let mut best_i = 0;
        let mut best_c = game.deficiency(remaining[0]);
    ...
    candidate.extend_from_slice(&seed[start + k..]);
    if candidate == seed {
        None
    } else {
        Some(candidate)
