...
fn refine_window<const NEUTRAL_SIGNATURES: bool>(
    game: &Game<'_>,
    window: &mut [usize],
    work: &mut TripleWork,
    ...
    while unseen != 0 {
        ...
        // Each descent has one immutable policy and its own engine. Neutral
        // results never enter a strict memo; both kernels retain union charges.
        let solution = if policy.permits_neutral() {
            if NEUTRAL_SIGNATURES && vertices.len() >= 5 && signature_cost < union_cost {
                let engine = engine.get_or_insert_with(|| {
                    SignatureEngine::new_neutral_largest(game.n)
                });
                engine.set_charge_model(ChargeModel::UnionParity);
...
pub(crate) fn subset_window_descent(
    ...
    subset_window_descent_config::<true>(
        n,
        col_ptr,
        row_idx,
        seed,
...
fn subset_window_descent_config<const NEUTRAL_SIGNATURES: bool>(
    n: usize,
    col_ptr: &[usize],
    row_idx: &[usize],
    seed: &[usize],
    ...
    for sweep in 0..sweeps {
        ...
        while start + 1 < n {
            ...
            match refine_window::<NEUTRAL_SIGNATURES>(
                &game,
...
mod tests {
    ...
    fn neutral_window_handles_full_width_codes_and_component_interleaving() {
        ...
        assert!(engine.is_none(), "small components retain the cheaper union kernel");
    }
    ...
    fn compare_neutral_signature_component(
        game: &Game<'_>,
        vertices: &[usize],
        engine: &mut SignatureEngine,
    ) {
        let need = (1usize << vertices.len())
            * (16 * vertices.len() + 6 * game.w + 24);
        for budget in [0, need as i64 - 1, need as i64, need as i64 + 17] {
            let mut union_work = TripleWork { remaining: budget };
            let mut signature_work = TripleWork { remaining: budget };
            let expected = solve_component_policy::<true>(game, vertices, &mut union_work);
            let actual = engine.solve_component(game, vertices, &mut signature_work);
            assert_eq!(actual, expected, "vertices={vertices:?}, budget={budget}");
            assert_eq!(signature_work.remaining, union_work.remaining);
        }
    }

    #[test]
    fn neutral_signature_matches_union_on_exhaustive_small_graphs_and_live_halos() {
        for bits in 0u32..64 {
            ...
            let p = Pattern::from_edges(4, &edges);
            let pristine = Game::build_adj(p.n, &p.col_ptr, &p.row_idx).unwrap();
            let mut game = Game::new(p.n, &pristine).unwrap();
            game.reset();
            let mut engine = SignatureEngine::new_neutral_largest(p.n);
            engine.set_verify_on_hit(true);
            compare_neutral_signature_component(&game, &[3, 0, 2, 1], &mut engine);
            compare_neutral_signature_component(&game, &[1, 2, 0, 3], &mut engine);
            compare_neutral_signature_component(&game, &[3, 0, 2, 1], &mut engine);
            assert!(engine.stats().verified_hits > 0);
        }
        for salt in 0..4 {
            let n = 73;
            let edges: Vec<_> = (0..n).flat_map(|a| {
                (a + 1..n).filter(move |&b| (a * 19 + b * 7 + salt) % 29 < 3)
                    .map(move |b| (a, b))
            }).collect();
            let p = Pattern::from_edges(n, &edges);
            let pristine = Game::build_adj(n, &p.col_ptr, &p.row_idx).unwrap();
            ...
            game.eliminate(0);
            game.eliminate(1);
            let mut engine = SignatureEngine::new_neutral_largest(n);
            engine.set_verify_on_hit(true);
            // A,A,B,A exercises verified hits around a different histogram and
            // DP width, without reusing any post-zeta scratch accidentally.
            for vertices in [
                &[6, 2, 5, 3, 9, 4, 8, 7][..],
                &[6, 2, 5, 3, 9, 4, 8, 7][..],
                &[6, 2, 5, 3, 9, 4][..],
                ...
                compare_neutral_signature_component(&game, vertices, &mut engine);
            }
        }
    }

    #[test]
    fn neutral_signature_clique_cache_is_isolated_and_preserves_local_ties() {
        let n = 17;
        let edges: Vec<_> = (0..n)
            .flat_map(|a| (a + 1..n).map(move |b| (a, b))).collect();
        let p = Pattern::from_edges(n, &edges);
        ...
        game.reset();
        game.eliminate(0);
        let vertices = vec![14, 1, 13, 2, 12, 3, 11, 4, 10, 5, 9, 6, 8, 7];
        let reverse: Vec<_> = vertices.iter().copied().rev().collect();
        let mut strict = SignatureEngine::new(n);
        let mut neutral = SignatureEngine::new_neutral_largest(n);
        strict.set_verify_on_hit(true);
        neutral.set_verify_on_hit(true);
        for input in [&vertices, &vertices, &reverse] {
            let mut strict_work = TripleWork { remaining: i64::MAX };
            let mut neutral_work = TripleWork { remaining: i64::MAX };
            let old = strict.solve_component(&game, input, &mut strict_work).unwrap();
            let new = neutral.solve_component(&game, input, &mut neutral_work).unwrap();
            assert_eq!(old.0, *input);
            assert_eq!(new.0, input.iter().copied().rev().collect::<Vec<_>>());
            assert_eq!((new.1, new.2), (old.1, old.2));
            assert_eq!(neutral_work.remaining, strict_work.remaining);
            compare_neutral_signature_component(&game, input, &mut neutral);
        }
        assert_eq!(strict.stats().verified_hits, 2);
        assert!(neutral.stats().verified_hits >= 2);
        neutral.reset_memo();
        compare_neutral_signature_component(&game, &vertices, &mut neutral);
        let mut work = TripleWork { remaining: i64::MAX };
        assert_eq!(neutral.solve_component(&game, &vertices, &mut work).unwrap().0, reverse);
    }

    #[test]
    fn neutral_signature_rolling_matches_union_under_partial_budgets() {
        // Padding activates the signature cost gate; small synthetic examples
        // alone would compare the union kernel to itself.
        let n = 4096;
        let mut edges = vec![
            (0, 1), (0, 2), (0, 3), (1, 4), (1, 5), (2, 4), (2, 5),
        ];
        ...
        let seed: Vec<_> = (0..n).collect();
        let _ = super::super::window_signatures::take_probe_stats();
        for budget in [0, 1_400_000, 1_500_000, 1_600_000, 2_000_000, 4_000_000] {
            let expected = subset_window_descent_config::<false>(
                n, &p.col_ptr, &p.row_idx, &seed, 8, 3, 3, budget,
                ChargeModel::UnionParity, WindowPolicy::NeutralRollingLargest,
            );
            let actual = subset_window_descent_neutral_rolling(
                n, &p.col_ptr, &p.row_idx, &seed, 8, 3, 3, budget,
            );
            assert_eq!(actual, expected, "budget={budget}");
        }
        let stats = super::super::window_signatures::take_probe_stats();
        assert!(stats.probes > 0, "the neutral signature integration must be exercised");
    }
