...
/// Refine the current leader and, inside a conservative cost gate, at most two
/// already-scored displaced portfolio candidates.  Each seed descends locally;
/// only its final strict improvement is compared with the global leader.
pub(super) fn refine(
    sp: &ScoringPattern,
    nnz: usize,
    max_degree: usize,
    ...
    if nnz == 0 || nnz >= MAX_NNZ || sp.n < 16 || sp.n > MAX_N {
